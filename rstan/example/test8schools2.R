...
ss1 <- sampling(m, data = c("J", "y", "sigma"), iter = iter, n_chains = 4, equal_step_sizes = FALSE, refresh = 100) 
...
yss <- stan(sfile, data = dat, iter = iter, n_chains = 4, sample_file = '8schools.csv', save_dso = FALSE)
save.image()

ss4 <- stan(fit = ss, data = dat, init = 0) 

initfun <- function(chain_id = 1) {
  list(mu = rnorm(1), theta = rnorm(J), tau = rexp(1))
} 
