...
especially the \Stan modeling language.  Details about \Stan could be found in 
\emph{Stan Modeling Language: User's Guide and Reference
Manual}(\cite{StanManual:2012}), so we focus on how to use \RStan using an
...
\item Translate the \Stan model to \Cpp code using \stanc 
...
allows end-to-end use of \Stan in \R. 
\begin{enumerate}
\item Translate the \Stan model to \Cpp code 
...
goes well. \RStan also provides functionality to fit a model by several 
...
the split Rhats.  The inference based on the samples after 
...
the information regarding the sampling procedure, for example, the initials
values for all parameters, the seed for random number generating,
and parameters used for the sampler. 

\section{More details about using package \pkg{rstan}}
...
\code{traceplot}. A common feature for most of these functions are that they allow us 
...
In this plot, an credible interval (typically 80\%) for all the samples
...
specified. If we include the warmup samples by setting \code{inc\_warmup = TRUE} (the 
...
for calling \code{extract}, the samples after warmup period 
...
and the compile the \Cpp code to dynamic shared object (DSO),
which is loaded into \R ang executed to draw samples. 
...
When reusing a previous fitted model, we could specify different 
...
To get the initial values we could use \code{get\_inits}. 
