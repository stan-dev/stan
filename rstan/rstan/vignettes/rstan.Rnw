...
Manual} (\citealt{StanManual:2013}), so we focus on how to use \RStan by an
...
\Stan. Refer to \cite{StanManual:2013} for how to install a \Cpp compiler if there 
not exists one. 
...
As of RStan 1.1.2, the log posterior function (up to a 
additive constant in general) can be evaluated through two
methods of a stanfit object. These two functions are \code{log\_prob}
and \code{grad\_log\_prob}. Both functions take parameters
...
parameters are $K-1$. To get the number of unconstrained parameters, 
function \code{get\_num\_upars} for a stanfit object can be used. 
To transform parameters between the constrained space and 
the unconstrained, we can use functions \code{transform\_pars} 
and \code{constrained\_pars}. The former takes a list of parameters 
as input and transforms to unconstrained space, and the latter 
does the inversion. 
...
\code{transform\_pars}    & transform parameter to unconstrained space \\ 
