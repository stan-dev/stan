...
Function \code{stan} returns an S4 object of class \code{stanfit}, for which
...
using \code{stan\_model}, which in this case includes the step of compiling
...
\code{summary}, \code{plot}, \code{traceplot}, \code{get.stancode}, etc. 
Using these functions, we first can diagnose the convergence
...
We can exclude the warmup area by specifying \code{inc.warmup = FALSE}
...
When the model is the same, we could reuse the previous compiled 
DSO. In function \code{stan}, if parameter \code{fit} is specified
...
DSO for models, so that they could be used across \R sessions. 
Saving the DSO is optional by specify parameter \code{save.dso}, which
...
\code{get.stancode}. All the initial values and seeds are 
...
\code{get.stancode}     & extract the \Stan modeling language code of the model  \\
\code{get.stanmodel}     & extract the \code{stanmodel} object \\ 
...
\RStan provides a convenient and end to end interface to \Stan. 
