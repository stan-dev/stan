...
TEST_F(performance, values_from_tagged_version) {
  ...
  EXPECT_FLOAT_EQ(-65.195801, first_run[0])
    << "lp__: index 0";

  EXPECT_FLOAT_EQ(1, first_run[1])
    << "accept_stat__: index 1";

  EXPECT_FLOAT_EQ(0.958363, first_run[2])
    << "stepsize__: index 2";
  ...
  EXPECT_FLOAT_EQ(65.224403, first_run[6])
    << "E__: index 6";

  EXPECT_FLOAT_EQ(1.3288701, first_run[7])
    << "beta.1: index 7";

  EXPECT_FLOAT_EQ(-0.51497102, first_run[8])
    << "beta.2: index 8";
