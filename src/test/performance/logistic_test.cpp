...
TEST_F(performance, values_from_tagged_version) {
  ...
  EXPECT_FLOAT_EQ(-65.227997, first_run[0])
    << "lp__: index 0";

  EXPECT_FLOAT_EQ(0.97589898, first_run[1])
    << "accept_stat__: index 1";

  EXPECT_FLOAT_EQ(0.80600202, first_run[2])
    << "stepsize__: index 2";
  ...
  EXPECT_FLOAT_EQ(65.4617, first_run[6])
    << "energy__: index 6";

  EXPECT_FLOAT_EQ(1.3497699, first_run[7])
    << "beta.1: index 7";

  EXPECT_FLOAT_EQ(-0.56483799, first_run[8])
    << "beta.2: index 8";
