...
TEST_F(performance, values_from_tagged_version) {
  ...
  EXPECT_FLOAT_EQ(-66.992599, first_run[0]) << "lp__: index 0";

  EXPECT_FLOAT_EQ(0.97065002, first_run[1]) << "accept_stat__: index 1";

  EXPECT_FLOAT_EQ(0.77005202, first_run[2]) << "stepsize__: index 2";

  EXPECT_FLOAT_EQ(3, first_run[3]) << "treedepth__: index 3";

  EXPECT_FLOAT_EQ(7, first_run[4]) << "n_leapfrog__: index 4";

  EXPECT_FLOAT_EQ(0, first_run[5]) << "divergent__: index 5";

  EXPECT_FLOAT_EQ(67.834, first_run[6]) << "energy__: index 6";

  EXPECT_FLOAT_EQ(0.915712, first_run[7]) << "beta.1: index 7";

  EXPECT_FLOAT_EQ(-0.334984, first_run[8]) << "beta.2: index 8";
