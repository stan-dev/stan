...
public:
  ...
  std::stringstream ss;
  stan::interface::recorder::csv recorder;
};



TEST_F(StanInterfaceRecorder, csv_vector_double) {
  ASSERT_EQ("", ss.str());
  std::vector<double> x;
  std::string expected;
...
TEST_F(StanInterfaceRecorder, csv_string) {
  ASSERT_EQ("", ss.str());
  std::string x;
  std::string expected;
...
TEST_F(StanInterfaceRecorder, csv_noargs) {
  ASSERT_EQ("", ss.str());
...
TEST_F(StanInterfaceRecorder, csv_is_recording) {
  ...
  stan::interface::recorder::csv null_recorder(0, "");
