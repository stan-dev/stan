...
TEST(McmcStepsizeAdaptation, set_mu) {
  ...
  double old_mu = 0.5;
  adaptation.mu() = old_mu;
  EXPECT_EQ(old_mu, adaptation.mu());
}
...
TEST(McmcStepsizeAdaptation, set_delta) {
  ...
  EXPECT_EQ(old_delta, adaptation.delta());
...
TEST(McmcStepsizeAdaptation, set_gamma) {
  ...
  EXPECT_EQ(old_gamma, adaptation.gamma());
...
TEST(McmcStepsizeAdaptation, set_kappa) {
  ...
  EXPECT_EQ(old_kappa, adaptation.kappa());
...
TEST(McmcStepsizeAdaptation, set_t0) {
  ...
  EXPECT_EQ(old_t0, adaptation.t0());
...
class exposed_adaptation : public stan::mcmc::stepsize_adaptation {
 public:
  exposed_adaptation(const double counter, const double s_bar,
                     const double x_bar, const double mu, const double delta,
                     const double gamma, const double kappa, const double t0) {
    counter_ = counter;
    s_bar_ = s_bar;
    x_bar_ = x_bar;
    mu_ = mu;
...
TEST(McmcStepsizeAdaptation, learn_stepsize_1) {
  exposed_adaptation adaptation(273, 0.00513820936953773, 0.886351350781597,
                                2.488109515349, 0.5, 0.05, 0.75, 10);
...
TEST(McmcStepsizeAdaptation, learn_stepsize_2) {
  exposed_adaptation adaptation(79, 0.0209457573689391, 0.821976337071218,
                                4.13018724667137, 0.5, 0.05, 0.75, 10);
  double epsilon, adapt_stat;
...
TEST(McmcStepsizeAdaptation, learn_stepsize_3) {
  exposed_adaptation adaptation(79, 0.0206811620891896, 0.825842987938587,
                                4.13018305456267, 0.5, 0.05, 0.75, 10);
  double epsilon, adapt_stat;
