...
TEST_F(McmcChains, blocker_mean) {
  ...
  for (int j = 0; j < chains.num_params(); j++) {
    ASSERT_FLOAT_EQ(means1(j), chains.mean(0, j)) << "1: chain, param mean";
    ASSERT_FLOAT_EQ(means1(j), chains.mean(j)) << "1: param mean";
  }

  chains.add(blocker2);
  Eigen::VectorXd means2 = blocker2.samples.colwise().mean();
  for (int j = 0; j < chains.num_params(); j++) {
    ASSERT_FLOAT_EQ(means2(j), chains.mean(1, j)) << "2: chain, param mean";
...
TEST_F(McmcChains, blocker_sd) {
  ...
  for (int j = 0; j < chains.num_params(); j++) {
    ...
    ASSERT_NEAR(sd(x), chains.sd(j), 1e-8) << "2: param sd. index: " << j;
  }
  ...
  for (int j = 0; j < chains.num_params(); j++) {
    ...
    ASSERT_NEAR(sd(x1), chains.sd(0, j), 1e-8) << "3: chain sd 1 with warmup";
    ASSERT_NEAR(sd(x2), chains.sd(1, j), 1e-8) << "3: chain sd 2 with warmup";
    ASSERT_NEAR(sd(x), chains.sd(j), 1e-8) << "3: param sd with warmup";
  }
  ...
  for (int i = 0; i < chains.num_params(); i++) {
    for (int j = i; j < chains.num_params(); j++) {
      if (++n % 13 == 0) {  // test every 13th value
        ...
        ASSERT_FLOAT_EQ(chains.covariance(0, i, j),
                        chains.covariance(0, name1, name2));
        ASSERT_FLOAT_EQ(chains.covariance(1, j, i),
                        chains.covariance(1, name2, name1));
        ASSERT_FLOAT_EQ(chains.covariance(i, j),
                        chains.covariance(name1, name2));
      }
    }
  }
}
  ...
  for (int i = 0; i < chains.num_params(); i++) {
    for (int j = i; j < chains.num_params(); j++) {
      if (++n % 13 == 0) {  // test every 13th value
        ...
        std::string name1 = chains.param_name(i);
        std::string name2 = chains.param_name(j);
        ASSERT_FLOAT_EQ(chains.correlation(0, i, j),
                        chains.correlation(0, name1, name2));
        ASSERT_FLOAT_EQ(chains.correlation(1, j, i),
                        chains.correlation(1, name2, name1));
        ASSERT_FLOAT_EQ(chains.correlation(i, j),
                        chains.correlation(name1, name2));
      }
    }
  }
}
...
TEST_F(McmcChains, blocker_quantile) {
  ...
  EXPECT_FLOAT_EQ(chains.quantile(0, index, 0.1),
                  chains.quantile(0, name, 0.1));
  EXPECT_FLOAT_EQ(chains.quantile(0, index, 0.3),
                  chains.quantile(0, name, 0.3));
  EXPECT_FLOAT_EQ(chains.quantile(0, index, 0.5),
                  chains.quantile(0, name, 0.5));
...
TEST_F(McmcChains, blocker_split_potential_scale_reduction_rank) {
  ...
  rhat_bulk << 1.0078,1.0109,0.99919,1.001,1.00401,1.00992,1.00182,1.00519,1.00095,1.00351,1.00554,1.00075,1.00595,1.00473,1.00546,1.01304,1.00166,1.0074,1.00178,1.00588,1.00406,1.00129,1.00976,1.0013,1.00193,1.00104,0.99938,1.00025,1.01082,1.0019,1.00354,1.0043,1.00111,1.00281,1.00436,1.00515,1.00325,1.0089,1.00222,1.00118,1.00191,1.00283,1.0003,1.00216,1.00335,1.00133,1.00023,1.0109;
  Eigen::VectorXd rhat_tail(48);
  rhat_tail << 1.00097,1.00422,1.00731,1.00333,1.00337,0.99917,1.00734,1.00633,1.00074,1.00906,1.01019,1.00074,1.00447,1.00383,1.00895,1.00389,1.00052,1.00188,1.00236,1.00284,1.00414,1.00303,1.00327,1.00295,1.00037,1.0044,1.00488,1.00178,1.00475,1.00082,1.00413,1.01303,1.0024,1.01148,1.00098,1.00078,1.00712,1.00595,1.00124,1.00112,1.00381,1.0006,1.00188,1.00225,1.0026,1.0009,1.00209,1.00464;

  for (int index = 4; index < chains.num_params(); index++) {
    double computed_bulk_rhat, computed_tail_rhat;
    std::tie(computed_bulk_rhat, computed_tail_rhat) = chains.split_potential_scale_reduction_rank(index);
