...
struct vectorized_multi_normal_prec_fun {
  ...
  vectorized_multi_normal_prec_fun(int K, int L) : K_(K), L_(L) { }
...
TEST(MultiNormalPrec, TestGradFunctionalVectorized) {
  ...
  test_grad(vectorized_multi_normal_prec_fun(3, 1), x);
  }
  ...
  test_grad(vectorized_multi_normal_prec_fun(3, 2), x);
  ...
  test_grad(vectorized_multi_normal_prec_fun(1, 1), u);
}
