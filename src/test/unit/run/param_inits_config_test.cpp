#include <stan/run/config_param_inits.hpp>
#include <gtest/gtest.h>


TEST(ParamInitsConfigTest, DefaultConstructor) {
  auto config_param_inits = stan::run::config_param_inits::create().build();
  EXPECT_EQ(stan::run::init_radius_config().value(), config_param_inits.init_radius());
  EXPECT_EQ(nullptr, config_param_inits.init_params());
}

TEST(ParamInitsConfigTest, Constructor_2) {
  auto config_param_inits = stan::run::config_param_inits::create()
    .init_radius(5.1).build();
  EXPECT_FLOAT_EQ(5.1, config_param_inits.init_radius());
}

TEST(ParamInitsConfigTest, Constructor_bad) {
  EXPECT_THROW(
	       auto config_param_inits = stan::run::config_param_inits::create()
	       .init_radius(-5.1).build(),
	       std::invalid_argument);
}


TEST(ParamInitsConfigTest, setters_bad) {
  auto config_param_inits = stan::run::config_param_inits::create();
  EXPECT_THROW(config_param_inits.init_radius(-1), std::invalid_argument);
}
