...
TEST(StanUi, write_model_nostream) {
  EXPECT_NO_THROW(stan::ui::write_model(0, model_name, prefix));
  EXPECT_NO_THROW(stan::ui::write_model(0, model_name));
}
...
TEST(StanUi, write_model_noprefix) {
  ...
  EXPECT_NO_THROW(stan::ui::write_model(&ss, model_name));
...
TEST(StanUi, write_model) {
  ...
  EXPECT_NO_THROW(stan::ui::write_model(&ss, model_name, prefix));
  EXPECT_EQ(expected_output, ss.str());
}
