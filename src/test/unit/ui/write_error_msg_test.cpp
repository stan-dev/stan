...
TEST(StanUi, write_error_msg_nostream) {
  EXPECT_NO_THROW(stan::ui::write_error_msg(0, err1));
  EXPECT_NO_THROW(stan::ui::write_error_msg(0, err2));
}

TEST(StanUi, write_error_msg) {
  std::stringstream ss;
  EXPECT_NO_THROW(stan::ui::write_error_msg(&ss, err1));
  EXPECT_TRUE(ss.str().find(err_msg1) != std::string::npos)
    << "The message should have err_msg1 inside it";
  
  ss.str("");
  EXPECT_NO_THROW(stan::ui::write_error_msg(&ss, err2));
