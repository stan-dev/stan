...
void expect_layout_matches(const Eigen::VectorXd& full,
  ...
  std::vector<char> is_data(layout.total_size_, 0);
  size_t src_offset = 0;
  for (size_t i = 0; i < layout.sizes_offsets_.size(); ++i) {
    const size_t offset = layout.sizes_offsets_[i].second;
    const size_t block_size = layout.sizes_offsets_[i].first;
  ...
  for (size_t i = 0; i < layout.total_size_; ++i) {
    if (!is_data[i]) {
...
TEST(serializer_opencl_layout, constructor_alignment) {
  std::vector<size_t> sizes{1, 7, 3};
  stan::io::serializer_layout layout(sizes, 4);
  ASSERT_EQ(layout.sizes_offsets_.size(), sizes.size());
  EXPECT_EQ(layout.sizes_offsets_[0].second, 0U);
  EXPECT_EQ(layout.sizes_offsets_[1].second, 4U);
  EXPECT_EQ(layout.sizes_offsets_[2].second, 12U);
  EXPECT_EQ(layout.sizes_offsets_[0].first, 1U);
  EXPECT_EQ(layout.sizes_offsets_[1].first, 7U);
  EXPECT_EQ(layout.sizes_offsets_[2].first, 3U);
  EXPECT_EQ(layout.align_elems_, 4U);
  EXPECT_EQ(layout.total_size_, 15U);
}

TEST(serializer_opencl_layout, constructor_no_padding) {
  std::vector<size_t> sizes{2, 3, 5};
  stan::io::serializer_layout layout(sizes, 1);
  ASSERT_EQ(layout.sizes_offsets_.size(), sizes.size());
  EXPECT_EQ(layout.sizes_offsets_[0].second, 0U);
  EXPECT_EQ(layout.sizes_offsets_[1].second, 2U);
  EXPECT_EQ(layout.sizes_offsets_[2].second, 5U);
  EXPECT_EQ(layout.total_size_, 10U);
}
...
TEST(serializer_opencl_layout, allocate_buffer_shape) {
  ...
  auto values = stan::io::allocate_serializer_buffer(layout.total_size_,
                                                     CL_MEM_READ_ONLY);
  EXPECT_EQ(values.rows(), static_cast<int>(layout.total_size_));
...
TEST(serializer_opencl_layout, copy_to_buffer) {
  ...
  stan::io::serializer_layout layout(sizes, 4);
  stan::math::matrix_cl<double> values = stan::io::allocate_serializer_buffer(
      layout.total_size_, CL_MEM_READ_ONLY);
...
TEST(serializer_opencl_layout, copy_to_buffer_many_blocks) {
  ...
  Eigen::VectorXd full = stan::math::from_matrix_cl<Eigen::VectorXd>(values);
  ASSERT_EQ(full.size(), static_cast<Eigen::Index>(layout.total_size_));
...
TEST(serializer_opencl_layout, serialize_to_opencl_roundtrip) {
  ...
  ASSERT_EQ(full_vals.size(), static_cast<Eigen::Index>(layout.total_size_));
  expect_layout_matches(full_vals, layout, params);

  EXPECT_EQ(params_opencl.adj().rows(), static_cast<int>(layout.total_size_));
