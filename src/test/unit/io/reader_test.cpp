...
TEST(io_reader, eigen_input) {
  ...
  int z = reader.integer();
  EXPECT_EQ(1, z);
  EXPECT_EQ(0U, reader.available());
}

TEST(io_reader, zeroSizeVecs) {
...
TEST(io_reader, scalar_lub_constrain) {
  ...
  EXPECT_FLOAT_EQ(3.0 + 2.0 * inv_logit_3,
                  reader.scalar_lub_constrain(3.0, 5.0));
  EXPECT_FLOAT_EQ(-3.0 + 5.0 * inv_logit_m1,
                  reader.scalar_lub_constrain(-3.0, 2.0));
  EXPECT_FLOAT_EQ(-15.0 + 30.0 * inv_logit_0,
                  reader.scalar_lub_constrain(-15.0, 15.0));
}
TEST(io_reader, scalar_lub_constrain_jacobian) {
  ...
  EXPECT_FLOAT_EQ(0.0 + 1.0 * inv_logit_m2,
                  reader.scalar_lub_constrain(0.0, 1.0, lp));
  EXPECT_FLOAT_EQ(3.0 + 2.0 * inv_logit_3,
                  reader.scalar_lub_constrain(3.0, 5.0, lp));
  EXPECT_FLOAT_EQ(-3.0 + 5.0 * inv_logit_m1,
...
TEST(io_reader, scalar_offset_multiplier_exception) {
  ...
  EXPECT_THROW(reader.scalar_offset_multiplier_constrain(-2.0, -2.0),
               std::domain_error);
}
...
TEST(io_reader, scalar_offset_multiplier_constrain) {
  ...
  EXPECT_FLOAT_EQ(3.0 + 5.0 * 3.0,
                  reader.scalar_offset_multiplier_constrain(3.0, 5.0));
  EXPECT_FLOAT_EQ(-3.0 + 2.0 * -1.0,
                  reader.scalar_offset_multiplier_constrain(-3.0, 2.0));
  EXPECT_FLOAT_EQ(-15.0,
                  reader.scalar_offset_multiplier_constrain(-15.0, 15.0));
}
TEST(io_reader, scalar_offset_multiplier_constrain_jacobian) {
  ...
  double lp = -7.2;
  EXPECT_FLOAT_EQ(-2.0,
                  reader.scalar_offset_multiplier_constrain(0.0, 1.0, lp));
  EXPECT_FLOAT_EQ(3.0 + 5.0 * 3.0,
                  reader.scalar_offset_multiplier_constrain(3.0, 5.0, lp));
  EXPECT_FLOAT_EQ(-3.0 + 2.0 * -1.0,
                  reader.scalar_offset_multiplier_constrain(-3.0, 2.0, lp));
  EXPECT_FLOAT_EQ(-15.0,
                  reader.scalar_offset_multiplier_constrain(-15.0, 15.0, lp));
...
TEST(io_reader, positive_ordered_constrain) {
  ...
  Eigen::Matrix<double, Eigen::Dynamic, 1> phi(
      reader.positive_ordered_constrain(4));
  EXPECT_FLOAT_EQ(v0, phi[0]);
  EXPECT_FLOAT_EQ(v1, phi[1]);
  EXPECT_FLOAT_EQ(v2, phi[2]);
  EXPECT_FLOAT_EQ(v3, phi[3]);
...
TEST(io_reader, positive_ordered_constrain_jacobian) {
  ...
  Eigen::Matrix<double, Eigen::Dynamic, 1> phi(
      reader.positive_ordered_constrain(4, lp));
  EXPECT_FLOAT_EQ(v0, phi[0]);
  EXPECT_FLOAT_EQ(v1, phi[1]);
  EXPECT_FLOAT_EQ(v2, phi[2]);
  EXPECT_FLOAT_EQ(v3, phi[3]);
...
TEST(io_reader, corr_matrix_constrain_jacobian) {
  ...
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> R(
      reader.corr_matrix_constrain(3U, lp));
  EXPECT_EQ(3, R.rows());
  EXPECT_EQ(3, R.cols());
  EXPECT_EQ(9, R.size());
  EXPECT_EQ(4U, reader.available());
...
TEST(io_reader, cov_matrix_constrain) {
  ...
  stan::io::reader<double> reader(theta, theta_i);
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> S(
      reader.cov_matrix_constrain(3U));
  EXPECT_EQ(3, S.rows());
  EXPECT_EQ(3, S.cols());
  EXPECT_EQ(9, S.size());
...
TEST(io_reader, cov_matrix_constrain_jacobian) {
  ...
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> S(
      reader.cov_matrix_constrain(3U, lp));
...
TEST(io_reader, cholesky_factor_cov) {
  std::vector<int> theta_i;
  std::vector<double> theta;
  for (int i = 0; i < 100.0; ++i)
    theta.push_back(static_cast<double>(i));
...
TEST(io_reader, cholesky_factor_cov_asymmetric) {
  std::vector<int> theta_i;
  std::vector<double> theta;
  for (int i = 0; i < 100.0; ++i)
    theta.push_back(static_cast<double>(i));
...
TEST(io_reader, cholesky_factor_cov_exception) {
  std::vector<int> theta_i;
  std::vector<double> theta;
  for (int i = 0; i < 100.0; ++i)
    theta.push_back(static_cast<double>(i));
  theta[0] = -6.3;
  stan::io::reader<double> reader(theta, theta_i);
  EXPECT_THROW(reader.cholesky_factor_cov(2, 2), std::domain_error);
  EXPECT_THROW(reader.cholesky_factor_cov(0, 0), std::domain_error);

  theta[0] = 1;
  EXPECT_THROW(reader.cholesky_factor_cov(2, 3), std::domain_error);
}
TEST(io_reader, cholesky_factor_cov_constrain) {
  std::vector<int> theta_i;
  std::vector<double> theta;
  for (int i = 0; i < 8; ++i)
    theta.push_back(-static_cast<double>(i));
  ...
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> L(
      reader.cholesky_factor_cov_constrain(3U, 3U));
...
TEST(io_reader, cholesky_factor_cov_constrain_asymmetric) {
  std::vector<int> theta_i;
  std::vector<double> theta;
  for (int i = 0; i < 8; ++i)
    theta.push_back(-static_cast<double>(i));
  stan::io::reader<double> reader(theta, theta_i);
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> L(
      reader.cholesky_factor_cov_constrain(3U, 2U));
  EXPECT_NO_THROW(stan::math::check_cholesky_factor(
      "test_cholesky_factor_constrain", "L", L));
  EXPECT_EQ(3, L.rows());
  EXPECT_EQ(2, L.cols());
...
TEST(io_reader, cholesky_factor_cov_constrain_jacobian) {
  std::vector<int> theta_i;
  std::vector<double> theta;
  for (int i = 0; i < 8; ++i)
    theta.push_back(-static_cast<double>(i));
  ...
  double lp = 1.9;
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> L(
      reader.cholesky_factor_cov_constrain(3U, 3U, lp));
  EXPECT_NO_THROW(stan::math::check_cholesky_factor(
      "test_cholesky_factor_constrain", "L", L));
  EXPECT_EQ(3, L.rows());
  EXPECT_EQ(3, L.cols());
...
TEST(io_reader, cholesky_factor_cov_constrain_jacobian_asymmetric) {
  std::vector<int> theta_i;
  std::vector<double> theta;
  for (int i = 0; i < 12; ++i)
    theta.push_back(-static_cast<double>(i));
  ...
  double lp = 1.9;
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> L(
      reader.cholesky_factor_cov_constrain(4U, 3U, lp));
  EXPECT_NO_THROW(stan::math::check_cholesky_factor(
      "test_cholesky_factor_constrain", "L", L));
  EXPECT_EQ(4, L.rows());
  EXPECT_EQ(3, L.cols());
...
TEST(io_reader, cholesky_factor_corr) {
  std::vector<int> theta_i;
...
TEST(io_reader, cholesky_factor_corr_exception) {
  std::vector<int> theta_i;
  std::vector<double> theta(9);
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> S;
  ...
  theta[8] = 1;
  stan::io::reader<double> reader(theta, theta_i);
  EXPECT_THROW(reader.cholesky_factor_corr(3), std::domain_error);
}
TEST(io_reader, cholesky_factor_corr_constrain) {
  std::vector<int> theta_i;
  std::vector<double> theta;
  for (int i = 0; i < 8; ++i)
    theta.push_back(-static_cast<double>(i));
  stan::io::reader<double> reader(theta, theta_i);
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> L(
      reader.cholesky_factor_corr_constrain(3U));
...
TEST(io_reader, UnitVectorThrows) {
  std::vector<double> theta;
  std::vector<int> theta_i;
  stan::io::reader<double> reader(theta, theta_i);
