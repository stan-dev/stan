...
TEST(deserializer_scalar, read) {
  ...
  theta_init.push_back(1.0);
  theta_init.push_back(2.0);
  stan::math::var_value<Eigen::VectorXd> theta = Eigen::Map<Eigen::VectorXd>(theta_init.data(), theta_init.size());
  stan::io::deserializer<stan::math::var_value<Eigen::VectorXd>> deserializer(theta, theta_i);
...
TEST(deserializer_scalar, complex_read) {
  ...
  theta_init.push_back(1.0);
  theta_init.push_back(2.0);
  theta_init.push_back(3.0);
  theta_init.push_back(4.0);
  stan::math::var_value<Eigen::VectorXd> theta = Eigen::Map<Eigen::VectorXd>(theta_init.data(), theta_init.size());
  stan::io::deserializer<stan::math::var_value<Eigen::VectorXd>> deserializer(theta, theta_i);
...
TEST(deserializer, read_int) {
  ...
  Eigen::Matrix<double, Eigen::Dynamic, 1> theta_init(2);
  theta_init[0] = 1.0;
  theta_init[1] = 2.0;
  theta_i[0] = 1;
  stan::math::var_value<Eigen::VectorXd> theta = Eigen::Map<Eigen::VectorXd>(theta_init.data(), theta_init.size());
...
TEST(deserializer_vector, read) {
  ...
  for (size_t i = 0; i < 100U; ++i)
    theta_init.push_back(static_cast<double>(i));
  stan::math::var_value<Eigen::VectorXd> theta = Eigen::Map<Eigen::VectorXd>(theta_init.data(), theta_init.size());
  stan::io::deserializer<stan::math::var_value<Eigen::VectorXd>> deserializer(theta, theta_i);
...
TEST(deserializer_vector, read_varmat) {
  std::vector<int> theta_i;
  std::vector<double> theta_init;
  for (size_t i = 0; i < 100U; ++i)
    theta_init.push_back(static_cast<double>(i));
  ...
  auto y
      = deserializer.read<stan::math::var_value<Eigen::Matrix<double, Eigen::Dynamic, 1>>>(4);
  EXPECT_EQ(4, y.rows());
  EXPECT_EQ(1, y.cols());
  EXPECT_EQ(4, y.size());
...
TEST(deserializer_row_vector, read_varmat) {
  std::vector<int> theta_i;
  std::vector<double> theta_init;
  for (size_t i = 0; i < 100U; ++i)
    theta_init.push_back(static_cast<double>(i));
  ...
  auto y = deserializer.read<stan::math::var_value<Eigen::Matrix<double, 1, Eigen::Dynamic>>>(4);
  EXPECT_EQ(4, y.cols());
  EXPECT_EQ(1, y.rows());
  EXPECT_EQ(4, y.size());
  EXPECT_FLOAT_EQ(7.0, y.val()[0]);
...
TEST(deserializer_matrix, read) {
  ...
  using varmat
      = stan::math::var_value<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>>;
  varmat y = deserializer.read<varmat>(3, 2);
  EXPECT_EQ(3, y.rows());
  EXPECT_EQ(2, y.cols());
...
TEST(deserializer_vector, unit_vector_constrain) {
  ...
  stan::math::var lp = 0;
  auto reference
      = stan::math::unit_vector_constrain(theta);
  ...
  for (size_t i = 0; i < phi.size(); ++i) {
    EXPECT_FLOAT_EQ(reference.val()(i), phi[i].val());
  }
}
...
TEST(deserializer_vector, unit_vector_jacobian) {
  ...
  auto reference
      = stan::math::unit_vector_constrain(theta, lp_ref);
  Eigen::Matrix<stan::math::var, Eigen::Dynamic, 1> phi(
      deserializer.read_constrain_unit_vector<
...
TEST(deserializer_vector, simplex_jacobian) {
  ...
  stan::math::var lp_ref = 0.0;
  auto reference
      = stan::math::simplex_constrain(theta, lp_ref);
  Eigen::Matrix<stan::math::var, Eigen::Dynamic, 1> phi(
      deserializer.read_constrain_simplex<
