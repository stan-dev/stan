...
class mock_writer : public stan::callbacks::writer {
 public:
  ...
  void operator()(const std::vector<double>& x) override { data.push_back(x); }
};
...
TEST(output_controller, configure_and_write_streaming) {
  ...
  controller.configure_output(
      "samples", {stan::callbacks::OutputFormat::MATRIX, "memory", 100, 3});
  ...
  auto* matrix_writer
      = dynamic_cast<stan::callbacks::matrix_writer*>(writer.get());
...
TEST(output_controller, write_metadata) {
  ...
  controller.configure_output(
      "model_info", {stan::callbacks::OutputFormat::JSON, "model.json"});
...
TEST(output_controller, get_json_writer) {
  ...
  controller.configure_output(
      "metric", {stan::callbacks::OutputFormat::JSON, "metric.json"});
...
TEST(output_controller, get_json_writer_wrong_type) {
  ...
  EXPECT_THROW(controller.get_json_writer<std::ofstream>("samples"),
               std::runtime_error);
}
...
TEST(output_controller, multiple_formats) {
  ...
  controller.configure_output(
      "samples", {stan::callbacks::OutputFormat::MATRIX, "memory", 100, 3});
  controller.configure_output(
      "diagnostics", {stan::callbacks::OutputFormat::CSV, "diagnostics.csv"});
  controller.configure_output(
      "metric", {stan::callbacks::OutputFormat::JSON, "metric.json"});
  ...
  EXPECT_NE(dynamic_cast<stan::callbacks::matrix_writer*>(sample_writer.get()),
            nullptr);
  EXPECT_NE(
      dynamic_cast<stan::callbacks::stream_writer*>(diagnostic_writer.get()),
      nullptr);
...
TEST(output_controller, unconfigured_output) {
  ...
  EXPECT_THROW(controller.get_json_writer<std::ofstream>("metric"),
               std::runtime_error);
}
...
TEST(output_controller, invalid_format) {
  ...
  EXPECT_THROW(controller.configure_output(
                   "samples", {static_cast<stan::callbacks::OutputFormat>(999),
                               "invalid"}),
               std::runtime_error);
