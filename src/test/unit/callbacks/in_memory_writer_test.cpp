...
class InMemoryWriterTest : public ::testing::Test {
 protected:
  void SetUp() override {
    // Parse the header
    header_tokens = split_csv_line(
        "theta, "
        "mu,y_rep.1,y_rep.2,y_rep.3,y_rep.4,y_rep.5,y_rep.6,y_rep.7,y_rep.8,y_"
        "rep.9,y_rep.10");
...
TEST_F(InMemoryWriterTest, DispatcherIntegration) {
  ...
  disp.register_channel(
      InfoType::SAMPLE_RAW,
      std::unique_ptr<stan::callbacks::Channel>(new WriterChannel(&writer)));
