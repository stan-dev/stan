...
TEST(MathErrorHandling,CheckLessOrEqual_nan) {
  ...
  EXPECT_THROW(check_less_or_equal(function, nan, lb, "x", &result),
               std::domain_error);
  EXPECT_THROW(check_less_or_equal(function, x, nan, "x", &result),
               std::domain_error);
  EXPECT_THROW(check_less_or_equal(function, nan, nan, "x", &result),
               std::domain_error);
  ...
  x_vec   << -1, 0, 1;
  low_vec << -2, -1, 0;
  EXPECT_THROW(check_less_or_equal(function, x_vec, nan, "x", &result),
               std::domain_error);

  for (int i = 0; i < x_vec.size(); i++) {
    x_vec   << -1, 0, 1;
    x_vec(i) = nan;
    EXPECT_THROW(check_less_or_equal(function, x_vec, low_vec, "x", &result),
                 std::domain_error);
  }
