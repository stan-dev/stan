...
TEST(MathErrorHandling,CheckGreaterOrEqual_Matrix_one_indexed_message) {
  const char* function = "check_greater_or_equal(%1%)";
  double result;
  double x;
  double low;
  ...
  try {
    check_greater_or_equal(function, x_vec, low, "x", &result);
  ...
  try {
    check_greater_or_equal(function, x_vec, low_vec, "x", &result);
    FAIL() << "should have thrown";
  } catch (std::domain_error& e) {
    message = e.what();
  } catch (...) {
  ...
  try {
    check_greater_or_equal(function, x, low_vec, "x", &result);
    FAIL() << "should have thrown";
  } catch (std::domain_error& e) {
    message = e.what();
  } catch (...) {
...
TEST(MathErrorHandling,CheckGreaterOrEqual_nan) {
  ...
  EXPECT_THROW(check_greater_or_equal(function, nan, lb, "x", &result),
               std::domain_error);
  EXPECT_THROW(check_greater_or_equal(function, x, nan, "x", &result),
               std::domain_error);
  EXPECT_THROW(check_greater_or_equal(function, nan, nan, "x", &result),
               std::domain_error);
  ...
  x_vec   << -1, 0, 1;
  low_vec << -2, -1, 0;
  EXPECT_THROW(check_greater_or_equal(function, x_vec, nan, "x", &result),
               std::domain_error);
