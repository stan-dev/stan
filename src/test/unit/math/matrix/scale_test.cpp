...
TEST(MathMatrix,scale) {
  ...
  expect_matrix_eq(v_m * m * v_m, scale(m,v));
}
TEST(MathMatrix,scale2) {
  ...
  rv << 1, 2, 3;
  expect_matrix_eq(v_m * m * v_m, scale(m,rv));
}

TEST(MathMatrix,scaleException) {
  ...
  EXPECT_THROW(scale(m2,v), std::domain_error);
  EXPECT_THROW(scale(m2,v2), std::domain_error);
}
