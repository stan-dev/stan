...
TEST_F(ProbDistributionsGaussianDLMInputs, PoliciesF) {
  ...
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF_sz1, GG, V, W, m0, C0), std::domain_error);
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF_sz1, GG, V_vec, W, m0, C0), std::domain_error);
  Matrix<double, Dynamic, Dynamic> FF_sz2 = MatrixXd::Random(2, 4);
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF_sz2, GG, V, W, m0, C0), std::domain_error);
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF_sz2, GG, V_vec, W, m0, C0), std::domain_error);
  // finite and NaN
...
TEST_F(ProbDistributionsGaussianDLMInputs, PoliciesG) {
  ...
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG_sz1, V, W, m0, C0), std::domain_error);
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG_sz1, V_vec, W, m0, C0), std::domain_error);
  Matrix<double, Dynamic, Dynamic> GG_sz2 = MatrixXd::Random(2, 3);
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG_sz2, V, W, m0, C0), std::domain_error);
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG_sz2, V_vec, W, m0, C0), std::domain_error);
  // finite and NaN
...
TEST_F(ProbDistributionsGaussianDLMInputs, PoliciesW) {
  ...
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG, V, W_sz, m0, C0), std::domain_error);
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG, V_vec, W_sz, m0, C0), std::domain_error);
  // not square
  Matrix<double, Dynamic, Dynamic> W_notsq = MatrixXd::Identity(2, 3);
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG, V, W_notsq, m0, C0), std::domain_error);
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG, V_vec, W_notsq, m0, C0), std::domain_error);
...
TEST_F(ProbDistributionsGaussianDLMInputs, PoliciesVMatrix) {
  ...
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG, V2, W, m0, C0), std::domain_error);
  // not square
  Matrix<double, Dynamic, Dynamic> V3 = MatrixXd::Identity(2, 3);
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG, V3, W, m0, C0), std::domain_error);
  // positive semi-definite is okay
...
TEST_F(ProbDistributionsGaussianDLMInputs, PoliciesVVector) {
  ...
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG, V_badsz, W, m0, C0), std::domain_error);
  // positive semi-definite is okay
...
TEST_F(ProbDistributionsGaussianDLMInputs, Policiesm0) {
  ...
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG, V, W, m0_sz, C0), std::domain_error);
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG, V_vec, W, m0_sz, C0), std::domain_error);
  // finite and NaN
...
TEST_F(ProbDistributionsGaussianDLMInputs, PoliciesC0) {
  ...
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG, V, W, m0, C0_sz), std::domain_error);
  EXPECT_THROW(gaussian_dlm_obs_log(y, FF, GG, V_vec, W, m0, C0_sz), std::domain_error);
  // negative
...
TEST(ProbDistributionsGaussianDLM,LoglikeUU_fvar_double) {
  ...
  y << fvar<double>(-0.286804393606091,1.0), fvar<double>(1.30654039013044,1.0),
    fvar<double>(0.184631538931975,1.0), fvar<double>(1.76116251447979,1.0), 
    fvar<double>(1.64691178557684,1.0), fvar<double>(0.0599998209370169,1.0), 
    fvar<double>(-0.498099220647035,1.0), fvar<double>(1.77794756092381,1.0), 
    fvar<double>(-0.435458550812876,1.0), fvar<double>(1.17332931763075,1.0);
  double ll_expected = -16.2484978375184;
...
TEST(ProbDistributionsGaussianDLM,LoglikeMM_fvar_double) {
  ...
  V << fvar<double>(7.19105866377728,1.0),
    ...
    fvar<double>(-0.311731853764732,1.0),
    fvar<double>(3.27048576782842,1.0),
    fvar<double>(0.457616661474554,1.0),
    fvar<double>(4.87333111936296,1.0),
    fvar<double>(0.457616661474554,1.0),
  ...
  W << fvar<double>(2.24277594357501,1.0),
    ...
    fvar<double>(-1.65863136283477,1.0),
    fvar<double>(6.69010664813895,1.0);
  Eigen::Matrix<fvar<double>, Eigen::Dynamic, Eigen::Dynamic> C0(2, 2);
  C0 << fvar<double>(82.1224673418328,1.0),
    fvar<double>(0, 1.0),
    fvar<double>(0,1.0),
    fvar<double>(56.0195157304406,1.0);
  ...
  y << fvar<double>(4.05787944965558,1.0), 
    fvar<double>(2.129936403626,1.0), 
    fvar<double>(4.7831157467878,1.0),
    fvar<double>(-3.24787355040931,1.0), 
    fvar<double>(3.29106435886992,1.0), 
    fvar<double>(-5.3704927108258,1.0),
    fvar<double>( -0.816249625704044,1.0),
    fvar<double>(1.48037050701867,1.0), 
    fvar<double>(-2.68345235365616,1.0),
    fvar<double>(2.44624163805141,1.0),
    fvar<double>(0.409922815875619,1.0),
    fvar<double>(4.24853291677921,1.0),
    fvar<double>(3.29113479311716,1.0),
    fvar<double>(-0.49506486892086,1.0),
    fvar<double>(-2.23350858809309,1.0),
    fvar<double>(-1.47295668380559,1.0),
    fvar<double>(2.32945737887854,1.0),
    fvar<double>(4.81422683437484,1.0), 
    fvar<double>(-3.30712917135304,1.0),
    ...
    fvar<double>(-2.53407127990878,1.0),
    fvar<double>(-1.0641380744013,1.0),
    fvar<double>(-2.38506878287814,1.0),
    fvar<double>(0.690976145192563,1.0),
    fvar<double>(-3.25066033978687,1.0),
    fvar<double>(1.32299515908216,1.0),
...
TEST(ProbDistributionsGaussianDLM,LoglikeUU_fvar_fvar_double) {
  ...
  y << fvar<fvar<double> >(-0.286804393606091,1.0), fvar<fvar<double> >(1.30654039013044,1.0),
    fvar<fvar<double> >(0.184631538931975,1.0), fvar<fvar<double> >(1.76116251447979,1.0), 
    fvar<fvar<double> >(1.64691178557684,1.0), fvar<fvar<double> >(0.0599998209370169,1.0), 
    fvar<fvar<double> >(-0.498099220647035,1.0), fvar<fvar<double> >(1.77794756092381,1.0), 
    fvar<fvar<double> >(-0.435458550812876,1.0), fvar<fvar<double> >(1.17332931763075,1.0);
  double ll_expected = -16.2484978375184;
...
TEST(ProbDistributionsGaussianDLM,LoglikeMM_fvar_fvar_double) {
  ...
  C0 << fvar<fvar<double> >(82.1224673418328,1.0),
    fvar<fvar<double> >(0, 1.0),
    fvar<fvar<double> >(0,1.0),
    fvar<fvar<double> >(56.0195157304406,1.0);
