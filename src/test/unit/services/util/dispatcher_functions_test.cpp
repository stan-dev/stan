...
public:
  ...
  void SetUp() override {
    ...
    dispatcher = stan::services::util::configure_dispatcher(std::move(output_streams));
  }
};
...
TEST_F(DispatcherFunctionsTest, WriteSampleHeader) {
  ...
  stan::services::util::write_sample_header(s, sampler, model, dispatcher, num_model_values);
...
TEST_F(DispatcherFunctionsTest, WriteSample) {
  ...
  std::string expect("lp__,accept_stat__,stepsize__,treedepth__,n_leapfrog__,divergent__,energy__,x,y\n");
  ...
  // Now write a sample
  stan::services::util::write_sample(rng, s, sampler, model, dispatcher, logger, num_model_values);
...
TEST_F(DispatcherFunctionsTest, WriteDiagnostic) {
  ...
  std::string expect("lp__,accept_stat__,stepsize__,treedepth__,n_leapfrog__,divergent__,energy__,x,y,p_x,p_y,g_x,g_y\n");
  EXPECT_EQ(header, expect);
...
TEST_F(DispatcherFunctionsTest, WriteTiming) {
  ...
  stan::services::util::write_timing(warmup_time, sampling_time, dispatcher, logger);
  
  // Check sample stream
  std::string sample_output = sample_stream->str();
  EXPECT_TRUE(sample_output.find("10.5 seconds (Warm-up)") != std::string::npos);
  EXPECT_TRUE(sample_output.find("20.3 seconds (Sampling)") != std::string::npos);
...
TEST_F(DispatcherFunctionsTest, HMCNUTSDiagEAdapt) {
  ...
  int result = stan::services::sample::hmc_nuts_diag_e_adapt(
    model, context, nullptr, seed, chain, init_radius,
