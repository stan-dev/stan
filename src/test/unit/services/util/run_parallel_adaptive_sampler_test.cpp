 ...
 public:
  ServicesUtil()
      : num_chains(2),
        ...
        num_thin(1),
        refresh(0),
        init_buffer(0), term_buffer(0), window(0),
        save_warmup(false) {
    for(int i=0; i != num_chains; ++i) {
      cont_vector.emplace_back(std::vector<double>({0.0, 0.0}));
      rng.emplace_back(stan::services::util::create_rng(0, 1));
      sampler.emplace_back(model, rng.back());
  ...
  std::vector<stan::mcmc::adapt_diag_e_nuts<stan_model, boost::ecuyer1988>> sampler;
  int num_chains, num_warmup, num_samples, num_thin, refresh;
  unsigned int init_buffer, term_buffer, window;
  bool save_warmup;
};

TEST_F(ServicesUtil, all_zero) {
  stan::services::util::run_parallel_adaptive_sampler(num_chains,
...
TEST_F(ServicesUtil, num_warmup_num_samples_refresh) {
  ...
  window = 25;
  for(int i=0; i != num_chains; ++i)
    sampler[i].set_window_params(num_warmup, init_buffer, term_buffer, window,
                                 logger);
  stan::services::util::run_parallel_adaptive_sampler(num_chains,
      sampler, model, cont_vector, num_warmup, num_samples, num_thin, refresh,
      save_warmup, rng, interrupt, logger, sample_writer, diagnostic_writer);
