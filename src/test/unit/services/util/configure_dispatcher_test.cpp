...
TEST_F(ConfigureDispatcherTest, BasicFunctionality) {
  ...
  dispatcher.dispatch(stan::callbacks::info_type::SAMPLE,
                      std::string("sample_message"));
  dispatcher.dispatch(stan::callbacks::info_type::DIAGNOSTIC,
  ...
  EXPECT_TRUE(diagnostic_stream->str().find("diagnostic_message")
              != std::string::npos);
...
TEST_F(ConfigureDispatcherTest, NullStream) {
  ...
  EXPECT_THROW(stan::services::util::configure_dispatcher(output_streams),
               std::runtime_error);
}
...
TEST_F(ConfigureDispatcherTest, MoveSemantics) {
  ...
  dispatcher2.dispatch(stan::callbacks::info_type::SAMPLE,
