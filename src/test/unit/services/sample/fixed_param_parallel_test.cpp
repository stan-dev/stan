...
TEST_F(ServicesSamplesFixedParam, call_count) {
  ...
  int return_code = stan::services::sample::fixed_param(
      model, num_chains, context, seed, chain, init_radius, num_iterations, 1, refresh,
...
TEST_F(ServicesSamplesFixedParam, output_sizes) {
  ...
  stan::services::sample::fixed_param(model, num_chains, context, seed, chain, init_radius,
                                      num_iterations, 1, refresh, interrupt,
                                      logger, init, parameter, diagnostic);

  for (int i = 0; i < num_chains; ++i) {
    std::vector<std::vector<std::string> > parameter_names = parameter[i].vector_string_values();
    std::vector<std::vector<double> > parameter_values = parameter[i].vector_double_values();
    std::vector<std::vector<std::string> > diagnostic_names = diagnostic[i].vector_string_values();
    std::vector<std::vector<double> > diagnostic_values = diagnostic[i].vector_double_values();
...
TEST_F(ServicesSamplesFixedParam, parameter_checks) {
  ...
  int return_code = stan::services::sample::fixed_param(model, num_chains,
   context, seed, chain, init_radius, num_iterations, 1, refresh,
   interrupt, logger, init, parameter, diagnostic);
for (int i = 0; i < num_chains; ++i) {
  std::vector<std::vector<std::string> > parameter_names = parameter[i].vector_string_values();
  std::vector<std::vector<double> > parameter_values = parameter[i].vector_double_values();
