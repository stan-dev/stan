 ...
 public:
  ServicesStandaloneGQ()
      : data_var_context([](){
          ...
          data_stream.close();
          return data_context;    
      }()), interrupt(), logger_ss(), logger(logger_ss, logger_ss, logger_ss, logger_ss, logger_ss),
...
TEST_F(ServicesStandaloneGQ, genDraws_bernoulli) {
  ...
  std::vector<stan::callbacks::unique_stream_writer<std::stringstream, deleter_noop>> sample_writer;
  sample_writer.reserve(num_chains);
  std::vector<Eigen::MatrixXd> draws_vec;
  for (int i = 0; i < num_chains; i++) {
    sample_writer.emplace_back(std::unique_ptr<std::stringstream, deleter_noop>(&sample_ss[i]), "");
    draws_vec.push_back(bern_csv.samples.middleCols<1>(7));
  }

  int return_code = stan::services::standalone_generate(
      model, num_chains, draws_vec, 12345, interrupt, logger,
      sample_writer);
