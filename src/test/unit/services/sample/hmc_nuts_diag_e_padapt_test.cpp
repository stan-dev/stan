#include <stan/services/sample/hmc_nuts_diag_e_padapt.hpp>
...
class ServicesSampleHmcNutsDiagEAdapt : public testing::Test {
 public:
  ServicesSampleHmcNutsDiagEAdapt() : model(context, &model_log)
                                      //logger(std::cout, std::cout, std::cout, std::cerr, std::cerr)
  {}
...
TEST_F(ServicesSampleHmcNutsDiagEAdapt, call_count) {
  ...
  int return_code = stan::services::sample::hmc_nuts_diag_e_padapt(
...
TEST_F(ServicesSampleHmcNutsDiagEAdapt, parameter_checks) {
  ...
  stan::services::sample::hmc_nuts_diag_e_padapt(
      model, context, random_seed, chain, init_radius, num_warmup, num_samples,
      num_thin, save_warmup, refresh, stepsize, stepsize_jitter, max_depth,
      delta, gamma, kappa, t0, init_buffer, term_buffer, window, interrupt,
      logger, init, parameter, diagnostic);
