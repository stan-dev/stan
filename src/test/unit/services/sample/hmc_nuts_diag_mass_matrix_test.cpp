...
TEST_F(ServicesSampleHmcNutsDiagEMassMatrix, use_mass_matrix_continue_300) {
  unsigned int random_seed = 12345;
  unsigned int chain = 1;
  double init_radius = 2;
  int num_warmup = 300;
  ...
  stan::test::unit::check_different(3, input_diag_vals, parameter, 0.001);

  // check mass matrix has not evolved to identy matrix
  stan::io::dump dmp = 
    stan::services::util::create_ident_diag_mass_matrix(3);
  stan::io::var_context& ident_mass_matrix = dmp;
  std::vector<double> ident_diag_vals
    = ident_mass_matrix.vals_r("mass_matrix");
  stan::test::unit::check_different(3, ident_diag_vals, parameter, 0.001);

}

// run model for 2000 iterations, starting w/ non-ident mass matrix
// resulting diag vals should be very close to 1
TEST_F(ServicesSampleHmcNutsDiagEMassMatrix, use_mass_matrix_continue_2000) {
  unsigned int random_seed = 12345;
  unsigned int chain = 1;
  double init_radius = 2;
  int num_warmup = 2000;
  ...
  // 2000 iterations should be close to ident
  stan::io::dump dmp = 
    stan::services::util::create_ident_diag_mass_matrix(3);
