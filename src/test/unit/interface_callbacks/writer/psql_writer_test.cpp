...
TEST_F(StanInterfaceCallbacksPQXXWriter, key_double_prefix) {
...
TEST_F(StanInterfaceCallbacksPQXXWriter, key_int_prefix) {
...
TEST_F(StanInterfaceCallbacksPQXXWriter, key_string_prefix) {
...
TEST_F(StanInterfaceCallbacksPQXXWriter, key_vector_prefix) {
  const int N = 5;
  double x[N];
  for (int n = 0; n < N; ++n) x[n] = n;
...
TEST_F(StanInterfaceCallbacksPQXXWriter, key_matrix_prefix) {
  const int n_cols = 2;
  const int n_rows = 3;
  double x[n_cols * n_rows];
  for (int i = 0; i < n_cols; ++i)
