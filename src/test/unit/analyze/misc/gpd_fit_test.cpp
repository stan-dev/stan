...
TEST(StanServices, khat_lx) {  
  ...
  x.resize(3, 1);
  x << .001, .002, .003;
  EXPECT_NO_THROW(stan::math::analyze::misc::lx(a, x));
...
TEST(StanServices, khat_adjust_wip) {
  ...
  EXPECT_NO_THROW(stan::math::analyze::misc::adjust_k_wip(k_est, n));
...
TEST(StanServices, khat_compute_khat) {
  ...
  EXPECT_NO_THROW(stan::math::analyze::misc::gpd_fit(x, 30, k));
