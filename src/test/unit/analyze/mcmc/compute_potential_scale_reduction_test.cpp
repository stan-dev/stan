...
TEST_F(ComputeRhat, compute_potential_scale_reduction_rank) {
  ...
  for (int index = 4; index < chains.num_params(); index++) {
    ...
    std::tie(computed_bulk_rhat, computed_tail_rhat) = stan::analyze::compute_potential_scale_reduction_rank(draws, sizes);
...
TEST_F(ComputeRhat, compute_potential_scale_reduction_rank_convenience) {
  ...
  for (int index = 4; index < chains.num_params(); index++) {
    ...
    double computed_bulk_rhat, computed_tail_rhat;
    std::tie(computed_bulk_rhat, computed_tail_rhat) = stan::analyze::compute_potential_scale_reduction_rank(draws, size);
    double expected_bulk_rhat = rhat_bulk(index - 4);
    double expected_tail_rhat = rhat_tail(index - 4);
...
TEST_F(ComputeRhat, compute_split_potential_scale_reduction_rank) {
  ...
  for (int index = 4; index < chains.num_params(); index++) {
    ...
    double computed_bulk_rhat, computed_tail_rhat;
    std::tie(computed_bulk_rhat, computed_tail_rhat) = stan::analyze::compute_split_potential_scale_reduction_rank(draws, sizes);
    double expected_bulk_rhat = rhat_bulk(index - 4);
    double expected_tail_rhat = rhat_tail(index - 4);
...
TEST_F(ComputeRhat, compute_split_potential_scale_reduction_convenience_rank) {
  ...
  for (int index = 4; index < chains.num_params(); index++) {
    ...
    double computed_bulk_rhat, computed_tail_rhat;
    std::tie(computed_bulk_rhat, computed_tail_rhat) = stan::analyze::compute_split_potential_scale_reduction_rank(draws, size);
    double expected_bulk_rhat = rhat_bulk(index - 4);
    double expected_tail_rhat = rhat_tail(index - 4);
...
TEST_F(ComputeRhat, compute_potential_scale_reduction_rank_constant) {
  ...
  double computed_bulk_rhat, computed_tail_rhat;
  std::tie(computed_bulk_rhat, computed_tail_rhat) = chains.split_potential_scale_reduction_rank(0);
