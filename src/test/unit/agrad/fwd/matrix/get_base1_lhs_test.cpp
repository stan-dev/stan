...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_vec2_fd) {
  ...
  for (size_t m = 1; m <= M; ++m) {
    for (size_t n = 1; n <= N; ++n) {
      ...
      fvar<double>  found = get_base1_lhs(get_base1_lhs(x, m, "x[m]",1),
                               n, "x[m][n]",2);
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_matrix_fd) {
  ...
  for (size_t i = 0; i < 4; ++i) {
    for (size_t j = 0; j < 3; ++j) {
      ...
      // fvar<double>  xij = get_base1_lhs<fvar<double> >(get_base1_lhs<fvar<double> >(x,i+1,"x",1),
      //                                j+1,"xi",2);
    }
  }
...
TEST(AgradFwdMatrixGetBase1LHS,failing_pre_20_fv) {
  using Eigen::Matrix;
  using Eigen::Dynamic;
  using stan::math::get_base1_lhs;
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_vec1_fv) {
  using stan::math::get_base1_lhs;
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_vec2_fv) {
  using stan::math::get_base1_lhs;
  using std::vector;
  size_t M = 3;
  size_t N = 4;
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_matrix_fv) {
  using Eigen::Matrix;
  using Eigen::Dynamic;
  using stan::math::get_base1_lhs;
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_vector_fv) {
  using Eigen::Matrix;
  using Eigen::Dynamic;
  using stan::math::get_base1_lhs;
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_row_vector_fv) {
  using Eigen::Matrix;
  using Eigen::Dynamic;
  using stan::math::get_base1_lhs;
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_8_fv) {
  using stan::math::get_base1_lhs;
  using std::vector;
  fvar<var>  x0(42.0);
  // ~ 4m entries ~ 32MB memory + sizes
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_vec2_ffd) {
  ...
  for (size_t m = 1; m <= M; ++m) {
    for (size_t n = 1; n <= N; ++n) {
      ...
      fvar<fvar<double> >  found = get_base1_lhs(get_base1_lhs(x, m, "x[m]",1),
                               n, "x[m][n]",2);
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_matrix_ffd) {
  ...
  for (size_t i = 0; i < 4; ++i) {
    for (size_t j = 0; j < 3; ++j) {
      ...
      EXPECT_FLOAT_EQ(x(i,j).val_.val_,get_base1_lhs(xi,j+1,"xi",2).val_.val_);
      // this is no good because can't get ref to inside val
      // could remedy by adding const versions, but don't need for Stan GM
      // fvar<fvar<double> >  xij = get_base1_lhs<fvar<fvar<double> > >(get_base1_lhs<fvar<fvar<double> > >(x,i+1,"x",1),
      //                                j+1,"xi",2);
    }
  }
  EXPECT_THROW(get_base1_lhs(x,10,"x",1), std::out_of_range);
...
TEST(AgradFwdMatrixGetBase1LHS,failing_pre_20_ffv) {
  using Eigen::Matrix;
  using Eigen::Dynamic;
  using stan::math::get_base1_lhs;
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_vec1_ffv) {
  using stan::math::get_base1_lhs;
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_vec2_ffv) {
  using stan::math::get_base1_lhs;
  using std::vector;
  size_t M = 3;
  size_t N = 4;
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_matrix_ffv) {
  using Eigen::Matrix;
  using Eigen::Dynamic;
  using stan::math::get_base1_lhs;
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_vector_ffv) {
  using Eigen::Matrix;
  using Eigen::Dynamic;
  using stan::math::get_base1_lhs;
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_row_vector_ffv) {
  using Eigen::Matrix;
  using Eigen::Dynamic;
  using stan::math::get_base1_lhs;
...
TEST(AgradFwdMatrixGetBase1LHS,get_base1_lhs_8_ffv) {
  using stan::math::get_base1_lhs;
  using std::vector;
