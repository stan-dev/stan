...
class Models_Transforms_CorrelationMatrix : public ::testing::Test {
protected:
  virtual void SetUp() {
    FILE *in;
    ...
    model.append("transforms").append(path_separator);
    model.append("correlation_matrix");
...
TEST_F(Models_Transforms_CorrelationMatrix,RunModel) {
  std::string command;
  command = model;
  command += " --samples=";
  command += output1;
