...
class Models_BugsExamples_Vol3_Hepatitis : public ::testing::Test {
protected:
  virtual void SetUp() {
    FILE *in;
    ...
    model.append("hepatitis").append(path_separator);
    model.append("hepatitis");
...
TEST_F(Models_BugsExamples_Vol3_Hepatitis,RunModel) {
  std::string command;
  command = model;
  command += " --samples=";
  command += output1;
