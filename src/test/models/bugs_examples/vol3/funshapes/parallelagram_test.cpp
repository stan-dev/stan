...
class Models_BugsExamples_Vol3_Parallelagram : public ::testing::Test {
protected:
  virtual void SetUp() {
    FILE *in;
    ...
    model.append("funshapes").append(path_separator);
    model.append("parallelagram");
...
TEST_F(Models_BugsExamples_Vol3_Parallelagram,RunModel) {
  std::string command;
  command = model;
  command += " --samples=";
  command += output1;
