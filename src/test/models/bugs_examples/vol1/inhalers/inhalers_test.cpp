...
class Models_BugsExamples_Vol1_Inhalers : public ::testing::Test {
protected:
  virtual void SetUp() {
    FILE *in;
    ...
    model.append("inhalers").append(path_separator);
    model.append("inhalers");
...
TEST_F(Models_BugsExamples_Vol1_Inhalers,RunModel) {
  std::string command;
  command = model;
  command += " --samples=";
  command += output1;
