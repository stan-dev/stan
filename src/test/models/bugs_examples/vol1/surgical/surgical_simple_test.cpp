...
class Models_BugsExamples_Vol1_SurgicalSimple : public ::testing::Test {
protected:
  virtual void SetUp() {
    FILE *in;
    ...
    model.append("surgical").append(path_separator);
    model.append("surgical_simple");
...
TEST_F(Models_BugsExamples_Vol1_SurgicalSimple,RunModel) {
  std::string command;
  command = model;
  command += " --samples=";
  command += output1;
