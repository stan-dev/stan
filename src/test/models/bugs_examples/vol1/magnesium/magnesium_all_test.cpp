...
class Models_BugsExamples_Vol1_MagnesiumAll : public ::testing::Test {
protected:
  virtual void SetUp() {
    FILE *in;
    ...
    model.append("magnesium").append(path_separator);
    model.append("magnesium_all");
...
TEST_F(Models_BugsExamples_Vol1_MagnesiumAll,RunModel) {
  std::string command;
  command = model;
  command += " --samples=";
  command += output1;
