...
class Models_BugsExamples_Vol1_RatsWaic : public ::testing::Test {
protected:
  virtual void SetUp() {
    FILE *in;
    ...
    model.append("rats").append(path_separator);
    model.append("rats_waic");
...
TEST_F(Models_BugsExamples_Vol1_RatsWaic,RunModel) {
  std::string command;
  command = model;
  command += " --samples=";
  command += output1;
