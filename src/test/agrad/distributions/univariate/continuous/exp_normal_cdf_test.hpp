...
public:
  void valid_values(vector<vector<double> >& parameters,
    ...
    cdf.push_back(0.7615783017);     // expected cdf
    ...
    param[3] = 1; //lambda
    parameters.push_back(param);
    cdf.push_back(0.7451154612); // expected cdf
    ...
    param[3] = 3; //lambda
    parameters.push_back(param);
    cdf.push_back(0.123236996); // expected cdf
    ...
    param[3] = 1.9; //lambda
    parameters.push_back(param);
    cdf.push_back(0.000940314301); // expected cdf
  }
  ...
  cdf_function(const T_y& y, const T_loc& mu, const T_scale& sigma,
    ...
    return (0.5 * (1 + erf((lambda) * (y - mu) / (lambda * sigma)))) * lambda - exp(-((lambda) * (y - mu)) + lambda * sigma * lambda * sigma / 2.0 + log((0.5 * (1 + erf((lambda) * (y - mu) - lambda * sigma))) - mu) * lambda);
  }
};
