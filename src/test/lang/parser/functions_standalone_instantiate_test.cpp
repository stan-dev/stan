...
void expect_no_errors(std::ostringstream& error_stream) {
  EXPECT_TRUE(error_stream.str().empty())
      << "Unexpected evaluation error: " << error_stream.str();
  error_stream.str("");
  error_stream.clear();
}
...
TEST(lang_parser, functions_standalone_instantiate_double_basic) {
  ...
  EXPECT_EQ(basic_functions::my_log1p_exp(5, &error_stream), log1p_exp(5))
      << "Problem instantiating a real -> real function from standalone "
         "compilation.";
  ...
  EXPECT_EQ(basic_functions::my_vector_mul_by_5(my_vec, &error_stream),
            multiply(my_vec, 5.0))
      << "Problem instantiating a vector -> vector function from standalone "
         "compilation.";
  expect_no_errors(error_stream);

  EXPECT_EQ(basic_functions::int_only_multiplication(3, 8, &error_stream),
            3 * 8)
      << "Problem instantiating an int-only function from standalone "
         "compilation.";
  expect_no_errors(error_stream);
}
