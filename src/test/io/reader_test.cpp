...
TEST(io_reader, cholesky_factor_exception) {
  ...
  theta[0] = -6.3;
  stan::io::reader<double> reader(theta,theta_i);
  EXPECT_THROW(reader.cholesky_factor(2), std::domain_error);
  EXPECT_THROW(reader.cholesky_factor(0), std::domain_error);
}
TEST(io_reader,cholesky_factor_constrain) {
  ...
  for (int i = 0; i < 8; ++i)
    theta.push_back(-static_cast<double>(i));
  stan::io::reader<double> reader(theta,theta_i);
  Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> L(reader.cholesky_factor_constrain(3U));
...
TEST(io_reader,cholesky_factor_constrain_jacobian) {
  ...
  stan::io::reader<double> reader(theta,theta_i);
  double lp = 1.9;
  Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic> L(reader.cholesky_factor_constrain(3U,lp));
  EXPECT_TRUE(stan::math::check_cholesky_factor("test_cholesky_factor_constrain",L));
  EXPECT_EQ(3,L.rows());
  EXPECT_EQ(3,L.cols());
  EXPECT_EQ(9,L.size());
