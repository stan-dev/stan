    ...
    for (size_t n = 0; n < parameters.size(); n++) {
      ...
      if (all_scalar<T0,T1,T2,T3,T4,T5>::value) {
        EXPECT_TRUE(stan::agrad::abs(expected_cdf_log[n] - cdf_log) < 1e-8)
          ...
          << "expected: " << expected_cdf_log[n] << std::endl
          << "actual:   " << cdf_log;
      }
    }
  }
 ...
 // works for <double>
  ...
  void test_gradient_function() {
    ...
    for (size_t n = 0; n < parameters.size(); n++) {
      ...
      T_return_type cdf_log_funct = TestClass.template cdf_log_function
        <Scalar0,Scalar1,Scalar2,Scalar3,Scalar4,Scalar5>
        (p0,p1,p2,p3,p4,p5);
  ...
  void test_repeat_as_vector() {
    ...
    for (size_t n = 0; n < parameters.size(); n++) {
      ...
      double single_cdf_log = calculate_gradients_1storder(single_gradients1,cdf_log,s1);
      ...
      T_return_type multiple_cdf_log = TestClass.template cdf_log
        <T0,T1,T2,T3,T4,T5>
        (p0,p1,p2,p3,p4,p5);
  ...
  void test_lower_bound() {
    ...
    for (size_t n = 0; n < parameters.size(); n++) {
      ...
      T_return_type cdf_log_at_lower_bound = TestClass.template cdf_log
        <T0,T1,T2,T3,T4,T5>
        (p0,p1,p2,p3,p4,p5);
      EXPECT_FLOAT_EQ(0.0,std::exp(value_of(value_of(value_of(cdf_log_at_lower_bound)))))
  ...
  void test_upper_bound() {
    ...
    for (size_t n = 0; n < parameters.size(); n++) {
      ...
      T_return_type cdf_log_at_upper_bound = TestClass.template cdf_log
        <T0,T1,T2,T3,T4,T5>
        (p0,p1,p2,p3,p4,p5);
      EXPECT_TRUE(cdf_log_at_upper_bound >= 0.0);
      EXPECT_FLOAT_EQ(0.0,value_of(value_of(value_of(cdf_log_at_upper_bound))))
