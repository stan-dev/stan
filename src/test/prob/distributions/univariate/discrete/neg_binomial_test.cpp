...
TEST(ProbDistributionsNegBinomial, doesFit) {
  boost::random::mt19937 rng;
  int N = 1000;
  int K = boost::math::round(2 * std::pow(N, 0.4));
  boost::math::negative_binomial_distribution<>dist (5,0.5);
  ...
  while (count < N) {
    int a = stan::prob::neg_binomial_rng(5,1,rng);
    int i = 0;
    while (i < K-1 && a > loc[i]) 
	++i;
    ++bin[i];
...
TEST(ProbDistributionsNegBinomial, doesFit2) {
  boost::random::mt19937 rng;
  int N = 1000;
  int K = boost::math::round(2 * std::pow(N, 0.4));
