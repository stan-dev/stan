...
TEST(ProbTransform,choleskyFactorLogJacobian) {
  ...
  x << 2.3;
  lp = 1.9;
  cholesky_factor_constrain(x,1,lp);
  ...
  x << 
    ...
    2, 3;
  lp = 7.2;
  cholesky_factor_constrain(x,2,lp);
  ...
  x << 
    ...
    4, 5, 6.1;
  lp = 1.2;
  cholesky_factor_constrain(x,3,lp);
...
TEST(ProbTransform,choleskyFactorConstrainError) {
  ...
  Matrix<double,Dynamic,1> x(3);
  x << 1, 2, 3;
  EXPECT_THROW(cholesky_factor_constrain(x,9),std::domain_error);
  double lp = 0;
  EXPECT_THROW(cholesky_factor_constrain(x,9,lp),std::domain_error);
}
