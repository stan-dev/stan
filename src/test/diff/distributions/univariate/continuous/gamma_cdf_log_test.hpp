...
class DiffCdfLogGamma : public DiffCdfLogTest {
public:
  void valid_values(vector<vector<double> >& parameters,
    ...
    cdf_log.push_back(std::log(0.59399414)); // expected cdf_log

    param[0] = 2.0;                 // y
    param[1] = 0.25;                // alpha
    param[2] = 0.75;                // beta
    parameters.push_back(param);
    cdf_log.push_back(std::log(0.96658355));  // expected cdf_log

    param[0] = 1.0;                 // y
    param[1] = 1.0;                 // alpha
    param[2] = 1.0;                 // beta
    parameters.push_back(param);
    cdf_log.push_back(std::log(0.63212055));       // expected cdf_log
  }
  ...
  template <typename T_y, typename T_shape, typename T_inv_scale,
      ...
      typename T9>
  typename stan::return_type<T_y, T_shape, T_inv_scale>::type 
  cdf_log_function(const T_y& y, const T_shape& alpha, const T_inv_scale& beta,
               const T3&, const T4&, const T5&, const T6&, const T7&, const T8&,
               const T9&) {
