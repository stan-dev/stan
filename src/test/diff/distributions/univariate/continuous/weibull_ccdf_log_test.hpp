...
class DiffCcdfLogWeibull : public DiffCcdfLogTest {
public:
  void valid_values(vector<vector<double> >& parameters,
    ...
    ccdf_log.push_back(std::log(1.0 - 0.86466472));       // expected ccdf_log

    param[0] = 0.25;                // y
    param[1] = 2.9;                 // alpha
    param[2] = 1.8;                 // sigma
    parameters.push_back(param);
    ccdf_log.push_back(std::log(1.0 - 0.0032585711));  // expected ccdf_log
  ...
  ccdf_log(const T_y& y, const T_shape& alpha, const T_scale& sigma,
           ...
           const T9&) {
    return stan::prob::weibull_ccdf_log(y, alpha, sigma);
  }
