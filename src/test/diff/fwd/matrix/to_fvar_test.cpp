...
TEST(DiffFwdMatrix,to_fvar_scalar) {
  ...
  fvar<double> v = 5.0;
   v.d_  = 1.0;
  fvar<double> fvar_x = stan::diff::to_fvar(d);
  EXPECT_FLOAT_EQ(5.0, fvar_x.val_);
  EXPECT_FLOAT_EQ(0.0, fvar_x.d_);

  fvar_x = stan::diff::to_fvar(v);
  EXPECT_FLOAT_EQ(5.0, fvar_x.val_);
...
TEST(DiffFwdMatrix,to_fvar_matrix) {
  ...
  matrix_d m_d(2,3);
  m_d << 0, 1, 2, 3, 4, 5;
  matrix_fv m_v = stan::diff::to_fvar(m_d);
...
TEST(DiffFwdMatrix,to_fvar_vector) {
  ...
  vector_fv out = stan::diff::to_fvar(d);
...
TEST(DiffFwdMatrix,to_fvar_rowvector) {
  ...
  row_vector_fv output = stan::diff::to_fvar(d);
...
TEST(DiffFwdMatrix,to_fvar_matrix_matrix) {
  ...
  matrix_fv output = stan::diff::to_fvar(val,d);
  ...
  matrix_d val1(4,4);
  EXPECT_THROW(stan::diff::to_fvar(val1, d), std::domain_error);
}
