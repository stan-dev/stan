...
typedef Matrix<AVAR, Dynamic, 1> vec_v;

TEST(AgradRevMatrix, rbind_matrix) {
  matrix_v a(2,2);
  matrix_v a_exp(2,2);
  MatrixXd b(2,2);
  ...
  AVAR rbind_ab = sum(rbind(a_exp, b));

  VEC g = cgradvec(rbind_ab, x);
...
TEST(AgradRevMatrix, rbind_row_vector) {
  vec_v a(3);
  vec_v a_exp(3);
  VectorXd b(2);
