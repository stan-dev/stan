...
generated quantities {
  ...
  v = student_t_rng(theta, mu, alpha);
  v = cauchy_rng(theta, alpha);
  v = double_exponential_rng(mu, alpha);
  v = logistic_rng(mu, theta);
  v = gumbel_rng(mu, alpha);
  v = lognormal_rng(mu, theta);
  v = chi_square_rng(mu);
  v = inv_chi_square_rng(mu);
  v = scaled_inv_chi_square_rng(mu, theta);
  v = exponential_rng(mu);
  v = gamma_rng(alpha, theta);
  v = inv_gamma_rng(alpha, theta);
  v = weibull_rng(alpha, theta);
  v = frechet_rng(mu, theta);
  v = rayleigh_rng(theta);
  v = pareto_rng(alpha, theta);
  v = pareto_type_2_rng(mu, alpha, theta);
  v = beta_rng(theta, alpha);
