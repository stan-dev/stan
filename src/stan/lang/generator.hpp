...
namespace stan {
  ...
  namespace lang {
    ...
    struct statement_visgen : public visgen {
      ...
      void generate_truncation(sample const& x, bool is_user_defined,
    ...
    void generate_function_using(const std::string& name_space,
        const function_decl_def& fun, std::ostream& out) {
      out << "using " << name_space << "::";
      generate_function_name(fun, out);
      out << ";" << EOL2;
    }
    ...
    void generate_function_usings(const std::vector<function_decl_def>& funs,
      const std::string name_space, std::ostream& out) {
      for (size_t i = 0; i < funs.size(); ++i) {
        generate_function_using(name_space, funs[i], out);
      }
    }
    ...
    void generate_functions_cpp(const program& prog,
      ...
      generate_end_namespace(out);
      generate_function_usings(prog.function_decl_defs_, "stan_namespace", out);
    }
