  ...
  namespace lang {
    ...
    struct get_var_decls_visgen : public visgen {
      ...
      void operator()(const unit_vector_var_decl& x) const {
        push_back_var_decl("unit_vector", x.name_, x.dims_.size());
      }

      void operator()(const simplex_var_decl& x) const {
        push_back_var_decl("simplex", x.name_, x.dims_.size());
      }

      void operator()(const ordered_var_decl& x) const {
        push_back_var_decl("ordered", x.name_, x.dims_.size());
      }

      void operator()(const positive_ordered_var_decl& x) const {
        push_back_var_decl("positive_ordered", x.name_, x.dims_.size());
      }
      ...
      void operator()(const cov_matrix_var_decl& x) const {
        push_back_var_decl("cov_matrix", x.name_, x.dims_.size());
      }

      void operator()(const corr_matrix_var_decl& x) const {
        push_back_var_decl("corr_matrix", x.name_, x.dims_.size());
      }
    };
