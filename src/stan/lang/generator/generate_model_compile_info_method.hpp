...
void generate_model_compile_info_method(std::ostream& o) {
  o << INDENT << "std::vector<std::string> model_compile_info() {" << EOL << INDENT2
    "std::vector<std::string> stanc_info;" << EOL << INDENT2
    "stanc_info.push_back(\2"stanc_version = stanc2\");" << EOL << INDENT2
