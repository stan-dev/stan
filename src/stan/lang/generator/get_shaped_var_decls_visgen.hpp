#ifndef STAN_LANG_GENERATOR_GET_SHAPED_VAR_DECLS_VISGEN_HPP
#define STAN_LANG_GENERATOR_GET_SHAPED_VAR_DECLS_VISGEN_HPP
...
namespace stan {
  namespace lang {
     ...
     */
    struct get_shaped_var_decls_visgen : public visgen {
      /**
       ...
       */
      get_shaped_var_decls_visgen(std::ostream& o)
        : visgen(3, o) { }


      void generate_bound_expression(const expression& x) const {
        if (is_nil(x))
          o_ << "0";  // ok to generate anything---won't be output
        else
          generate_expression(x.expr_, false, false, o_);
      }

      void push_back_var_decl(const std::string& type_name,
                              const std::string& name,
                              const std::vector<expression>& dims,
                              bool has_lb, bool has_ub,
                              const expression& lb = expression(),
                              const expression& ub = expression()) const {
        o_ << INDENT3 << "decls__.push_back(shaped_var_decl("
           << "\"" << name << "\", "
           << "\"" << type_name << "\", "
           << dims.size()
           << ", " << (has_lb ? "true" : "false")
        ...
        o_ << ".array(), "
           << EOL << INDENT3 << INDENT;
        generate_bound_expression(lb);
        o_ << ", ";
        generate_bound_expression(ub);
        o_ << "));" << EOL;
      }
      ...
      void push_back_var_decl(const std::string& type_name,
        ...
        push_back_var_decl(type_name, name, dims_plus, has_lb, has_ub, lb, ub);
      }
