#ifndef STAN_LANG_GENERATOR_GENERATE_GET_SIZED_VAR_DECLS_HPP
#define STAN_LANG_GENERATOR_GENERATE_GET_SIZED_VAR_DECLS_HPP

#include <stan/lang/ast.hpp>
#include <stan/lang/generator/get_sized_var_decls_visgen.hpp>
...
namespace stan {
  namespace lang {
     ...
     */
    void generate_get_sized_var_decls(const std::string& flag,
                      const std::vector<stan::lang::var_decl>& vs,
                      std::ostream& o) {
      o << EOL << INDENT2 << "if (" << flag << ") {" << EOL;

      get_sized_var_decls_visgen vis(o);
     ...
     * @param[in, out] o stream to which code is generated
     */
    void generate_get_sized_var_decls(const stan::lang::program& prog,
                                      std::ostream& o) {
      o << EOL
        << EOL << INDENT << "std::vector<stan::model::sized_var_decl>"
        << EOL << INDENT << "get_sized_var_decls(bool data__,"
        ...
        << EOL << INDENT2 << "using stan::model::sized_var_decl;"
      ...
      generate_get_sized_var_decls("generated_quantities__",
