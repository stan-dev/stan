...
for (size_t i=1; i < 10; i++) {
  add("flatten", expr_type(DOUBLE_T,1), expr_type(DOUBLE_T,i));
  add("flatten", expr_type(INT_T,1), expr_type(INT_T,i));
}
add_unary("floor");
add_ternary("fma");
add_binary("fmax");
...
add("to_row_vector", ROW_VECTOR_T, expr_type(INT_T,2));
add("to_vector", VECTOR_T, ROW_VECTOR_T);
add("to_vector", VECTOR_T, MATRIX_T);
add("to_vector", VECTOR_T, expr_type(DOUBLE_T,1));
add("to_vector", VECTOR_T, expr_type(INT_T,2));
add("to_array", expr_type(DOUBLE_T,2), MATRIX_T);
add("to_array", expr_type(DOUBLE_T,1), VECTOR_T);
add("to_array", expr_type(DOUBLE_T,1), ROW_VECTOR_T);
