#ifndef __STAN__AGRAD__FWD__OWENS__T_HPP__
...
namespace stan {
  namespace agrad {
    ...
    owens_t(const fvar<T>& x1, const fvar<T>& x2) {
      ...
      return fvar<T>(owens_t(x1.val_, x2.val_), -x1.d_ 
    ...
    owens_t(double x1, const fvar<T>& x2) {
      using stan::math::owens_t;
      using boost::math::erf;
      using stan::math::pi;
      using std::exp;
      ...
      return fvar<typename stan::return_type<T,double>::type>
        (owens_t(x1, x2.val_), x2.d_ * exp(neg_x1_sq_div_2 * one_p_x2_sq) 
         / (one_p_x2_sq * 2.0 * pi()));
    }
    ...
    inline
    fvar<typename stan::return_type<T,double>::type>    
    owens_t(const fvar<T>& x1, double x2) {
      using stan::math::owens_t;
      using boost::math::erf;
      using stan::math::pi;
      using std::exp;
      ...
      return fvar<typename stan::return_type<T,double>::type>
        (owens_t(x1.val_, x2), -x1.d_  * (erf(x2 * x1.val_ / std::sqrt(2.0)) 
                                          * exp(neg_x1_sq_div_2) 
                                          / std::sqrt(8.0 * pi())));
    }
