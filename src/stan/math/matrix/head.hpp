...
namespace stan {
  namespace math {
    ...
    head(const Eigen::Matrix<T,Eigen::Dynamic,1>& v,
         size_t n) {
      if (n != 0)
        stan::error_handling::check_row_index("head(%1%)",n,v,"n",(double*)0);
      return v.head(n);
    }
    ...
    head(const Eigen::Matrix<T,1,Eigen::Dynamic>& rv,
         size_t n) {
      if (n != 0)
        stan::error_handling::check_column_index("head(%1%)",n,rv,"n",(double*)0);
      return rv.head(n);
    }
    ...
    std::vector<T> head(const std::vector<T>& sv,
                        size_t n) {
      if (n != 0)
        stan::error_handling::check_std_vector_index("head(%1%)",n,sv,"n",(double*)0);
