...
namespace stan {
  namespace math {
    ...
    segment(const Eigen::Matrix<T,Eigen::Dynamic,1>& v,
      ...
      stan::math::check_less_or_equal("segment", "n", i, static_cast<size_t>(v.rows()));
      if (n != 0) {
    ...
    segment(const Eigen::Matrix<T,1,Eigen::Dynamic>& v,
            size_t i, size_t n) {
      stan::math::check_greater("segment", "n", i, 0.0);
      stan::math::check_less_or_equal("segment", "n", i, static_cast<size_t>(v.cols()));
      if (n != 0) {
        stan::math::check_greater("segment", "n", i+n-1, 0.0);
    ...
    segment(const std::vector<T>& sv,
      ...
      if (n != 0) {
        ...
        stan::math::check_less_or_equal("segment", "i+n-1", i+n-1, static_cast<size_t>(sv.size()));
      }
      std::vector<T> s;
