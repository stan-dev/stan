#ifndef STAN__MATH__PRIM__SCAL__PROB__NEG_BINOMIAL_2_CDF_LOG_HPP
#define STAN__MATH__PRIM__SCAL__PROB__NEG_BINOMIAL_2_CDF_LOG_HPP
...
namespace stan {
  ...
  namespace prob {
    ...
    neg_binomial_2_cdf_log(const T_n& n,
      ...
      if (size_n == 1) {
        if (size_phi_mu == 1)
          return beta_cdf_log(phi_mu[0], phi, np1[0]);                   else
          return beta_cdf_log(phi_mu, phi, np1[0]);                           }
      else {
        if (size_phi_mu == 1)
          return beta_cdf_log(phi_mu[0], phi, np1);                   else
