...
namespace stan {
  ...
  namespace math {
    ...
    template <typename T> 
    const std::vector<int> extract_v(Eigen::SparseMatrix<T, Eigen::RowMajor> A) {
      std::vector<int> v(A.nonZeros());
      for (int j = 0; j < A.nonZeros(); ++j)
        v[j] = *(A.innerIndexPtr()+j) + 1;  // make 1-indexed
      return v;
    ...
    template <typename T> 
    const std::vector<int> extract_u(Eigen::SparseMatrix<T, Eigen::RowMajor> A) {
      std::vector<int> u(A.outerSize()+1);
      for (int j = 0; j <= A.outerSize(); ++j)
        u[j] = *(A.outerIndexPtr()+j) + 1;  // make 1-indexed
      return u;
    ...
    template <typename T> 
    const std::vector<int> extract_z(Eigen::SparseMatrix<T, Eigen::RowMajor> A) {
      std::vector<int> u(A.outerSize()+1);
      std::vector<int> z(A.outerSize()+1);
      u = extract_u(A);
      std::adjacent_difference(u.begin(), u.end(), z.begin());
