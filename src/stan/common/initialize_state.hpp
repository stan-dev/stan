...
namespace stan {
  namespace common {    
    ...
    bool initialize_state_source_and_random(const std::string source,
      ...
      try {
        ...
        for (num_init_tries = 1; num_init_tries <= MAX_INIT_TRIES; ++num_init_tries) {
          ...
          model.write_array(base_rng, cont_vecs, int_vecs, cont_vecs_constrained, false, false, output);
     ...
     *                            one method that takes a string.
     * @param[in]     enable_random_init if true, it allows partially specifying
                                  inits from source, otherwise not
     */
    template <class ContextFactory, class Model, class RNG>
    ...
    bool initialize_state(const std::string init,
                          ...
                          bool enable_random_init = true) {
      double init_d;
      if (!get_double_from_string(init, init_d)) {
        return initialize_state_source(init, R, cont_params, model,
                                       base_rng, output,
                                       context_factory, enable_random_init);
      }
      if (0 == init_d)
          return initialize_state_zero(cont_params, model, output);
      return initialize_state_random(init_d, cont_params, model,
                                     base_rng, output);
    }
  } // common
} // stan
