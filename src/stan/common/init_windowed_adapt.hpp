  ...
  namespace common {
    ...
    bool init_windowed_adapt(stan::mcmc::base_mcmc* sampler,
      ...
      unsigned int init_buffer
        = dynamic_cast<stan::gm::u_int_argument*>(adapt->arg("init_buffer"))
        ->value();
      unsigned int term_buffer
        = dynamic_cast<stan::gm::u_int_argument*>(adapt->arg("term_buffer"))
        ->value();
      unsigned int window
        = dynamic_cast<stan::gm::u_int_argument*>(adapt->arg("window"))
        ->value();

      dynamic_cast<Sampler*>(sampler)
        ->set_window_params(num_warmup, init_buffer,
