...
namespace stan {
  namespace common {
    ...
    bool init_adapt(stan::mcmc::base_mcmc* sampler,
                    ...
                    const Eigen::VectorXd& cont_params) {
      double delta
        = dynamic_cast<stan::gm::real_argument*>(adapt->arg("delta"))->value();
      double gamma
        = dynamic_cast<stan::gm::real_argument*>(adapt->arg("gamma"))->value();
      double kappa
        = dynamic_cast<stan::gm::real_argument*>(adapt->arg("kappa"))->value();
      double t0
        = dynamic_cast<stan::gm::real_argument*>(adapt->arg("t0"))->value();
