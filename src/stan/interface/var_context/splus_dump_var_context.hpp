#ifndef STAN_INTERFACE_VAR_CONTEXT_SPLUS_DUMP_VAR_CONTEXT_HPP
#define STAN_INTERFACE_VAR_CONTEXT_SPLUS_DUMP_VAR_CONTEXT_HPP
...
namespace stan {
  namespace interface {
    namespace var_context {
      ...
      // Extends a map_var_context to be able to construct
      // an object from an ASCII file formatted in the S-plus
      // dump format
      class splus_dump_var_context : public map_var_context {
      public:
        /**
         ...
         */
        explicit splus_dump_var_context(std::istream& in) {
          dump_reader reader(in);
          while (reader.next()) {
            if (reader.is_int())
              vars_i_[reader.name()]
                = vals_i_t(reader.int_values(), reader.dims());
            else
              vars_r_[reader.name()]
                = vals_r_t(reader.double_values(), reader.dims());
          }
        }
