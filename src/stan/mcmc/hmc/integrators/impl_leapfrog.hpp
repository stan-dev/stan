...
class impl_leapfrog
    : public base_leapfrog<impl_leapfrog<Hamiltonian>, Hamiltonian> {
 public:
  impl_leapfrog()
      : base_leapfrog<impl_leapfrog<Hamiltonian>, Hamiltonian>(),
  ...
  inline void begin_update_p(point_type& z, hamiltonian_type& hamiltonian,
    ...
    this->hat_phi(z, hamiltonian, epsilon, logger);
    this->hat_tau(z, hamiltonian, epsilon, this->max_num_fixed_point_, logger);
  }
  ...
  inline void hat_phi(point_type& z, hamiltonian_type& hamiltonian,
                      double epsilon, callbacks::logger& logger) {
    z.p -= epsilon * hamiltonian.dphi_dq(z, logger);
  }

  // hat{tau} = dtau/dq * d/dp
  inline void hat_tau(point_type& z, hamiltonian_type& hamiltonian,
                      double epsilon, int num_fixed_point,
  ...
  inline void set_max_num_fixed_point(int n) {
    if (n > 0)
  ...
  inline void set_fixed_point_threshold(double t) {
    if (t > 0)
