...
class diag_e_nuts
    : public base_nuts_ct<ParallelBase, Model, diag_e_metric, expl_leapfrog, BaseRNG> {
 using base_nuts_t = base_nuts_ct<ParallelBase, Model, diag_e_metric, expl_leapfrog, BaseRNG>;
 public:
  template <bool ParallelBase_ = ParallelBase, std::enable_if_t<!ParallelBase_>* = nullptr>
  diag_e_nuts(const Model& model, BaseRNG& rng)
      : base_nuts_t(model, rng) {}
  template <bool ParallelBase_ = ParallelBase, std::enable_if_t<ParallelBase_>* = nullptr>
