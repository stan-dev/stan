...
class base_parallel_nuts
 ...
 public:
  ...
  base_parallel_nuts(const Model& model, std::vector<BaseRNG>& thread_rngs)
      : base_hmc<Model, Hamiltonian, Integrator, BaseRNG>(
          model, thread_rngs[tbb::this_task_arena::current_thread_index()]),
        rand_uniform_vec_(make_uniform_vec(thread_rngs)) {}

  base_parallel_nuts(const Model& model, BaseRNG& rng,
                     std::vector<BaseRNG>& thread_rngs)
  ...
  inline void set_max_delta(double d) noexcept { max_deltaH_ = d; }
  ...
  struct subtree {
    subtree(const double sign, const ps_point& z_end,
            const Eigen::VectorXd& p_sharp_end, double H0)
        : z_end_(z_end),
          z_propose_(z_end),
  ...
  // subtree
  using extend_tree_t = std::tuple<bool, double, Eigen::VectorXd,
                                   Eigen::VectorXd, ps_point, int, double>;
  ...
  inline extend_tree_t extend_tree(int depth, subtree& tree, state_t& z,
    ...
    Eigen::VectorXd p_sharp_dummy
        = Eigen::VectorXd::Zero(tree.p_sharp_end_.size());
    ...
    bool valid_subtree = build_tree(
        depth, z, tree.z_propose_, p_sharp_dummy, tree.p_sharp_end_,
        rho_subtree, tree.H0_, tree.sign_, tree.n_leapfrog_,
        log_sum_weight_subtree, tree.sum_metro_prob_, logger);

    tree.z_end_.ps_point::operator=(z);

    return std::make_tuple(valid_subtree, log_sum_weight_subtree, rho_subtree,
                           tree.p_sharp_end_, tree.z_propose_, tree.n_leapfrog_,
                           tree.sum_metro_prob_);
  }
  ...
  // with a check node.
  sample transition_parallel(sample& init_sample, callbacks::logger& logger) {
    ...
    const std::size_t num_fwd
        = std::accumulate(fwd_direction.begin(), fwd_direction.end(), 0);
    ...
    */
    ...
    tbb::concurrent_vector<extend_tree_t> ends(
        this->max_depth_, std::make_tuple(true, 0, Eigen::VectorXd(),
                                          Eigen::VectorXd(), z_sample, 0, 0.0));
    ...
    */
    ...
    // const double sum_metro_prob = tree_fwd.sum_metro_prob_ +
    // tree_bck.sum_metro_prob_;
  ...
  sample transition_refactored(sample& init_sample, callbacks::logger& logger) {
    // Initialize the algorithm
    this->sample_stepsize();
    ...
    const double sum_metro_prob
        = tree_fwd.sum_metro_prob_ + tree_bck.sum_metro_prob_;
  ...
  sample transition_old(sample& init_sample, callbacks::logger& logger) {
    // Initialize the algorithm
    this->sample_stepsize();
    ...
    while (this->depth_ < this->max_depth_) {
      ...
      if (this->rand_uniform_() > 0.5) {
        ...
        valid_subtree
            = build_tree(this->depth_, this->z_, z_propose, p_sharp_dummy,
                         p_sharp_plus, rho_subtree, H0, 1, n_leapfrog,
                         log_sum_weight_subtree, sum_metro_prob, logger);
        z_plus.ps_point::operator=(this->z_);
      } else {
        this->z_.ps_point::operator=(z_minus);
        valid_subtree
            = build_tree(this->depth_, this->z_, z_propose, p_sharp_dummy,
                         p_sharp_minus, rho_subtree, H0, -1, n_leapfrog,
                         log_sum_weight_subtree, sum_metro_prob, logger);
   ...
   */
  bool build_tree(int depth, state_t& z_beg, ps_point& z_propose,
                  Eigen::VectorXd& p_sharp_left, Eigen::VectorXd& p_sharp_right,
                  Eigen::VectorXd& rho, double H0, double sign, int& n_leapfrog,
    ...
    if (depth == 0) {
      ...
      this->integrator_.evolve(z_beg, this->hamiltonian_, sign * this->epsilon_,
                               logger);
    ...
    bool valid_left = build_tree(depth - 1, z_beg, z_propose, p_sharp_left,
                                 p_sharp_dummy, rho_left, H0, sign, n_leapfrog,
                                 log_sum_weight_left, sum_metro_prob, logger);
    ...
    bool valid_right
        = build_tree(depth - 1, z_beg, z_propose_right, p_sharp_dummy,
                     p_sharp_right, rho_right, H0, sign, n_leapfrog,
                     log_sum_weight_right, sum_metro_prob, logger);
  ...
  inline double get_rand_uniform() {
    return this
        ->rand_uniform_vec_[tbb::this_task_arena::current_thread_index()]();
  }
