    ...
    public:
	  ...
	  try {
		...
		Ytrain = Y;
	      ...
	      Eigen::MatrixXd cov_train = (Ytrain.cols() > 0) ? internal::covariance(Ytrain.transpose()) : Eigen::MatrixXd::Zero(M, M);
	      Eigen::MatrixXd cov_test = (Ytest.cols() > 0) ? internal::covariance(Ytest.transpose()) : Eigen::MatrixXd::Zero(M, M);
		...
		double c_diag = 0.0;
		for(int i = 0; i < 5; i++) {
		  double high_eigenvalue_dense = internal::eigenvalue_scaled_hessian(model, L_dense, Ytest.block(0, i, M, 1));
		  double high_eigenvalue_diag = internal::eigenvalue_scaled_hessian(model, L_diag, Ytest.block(0, i, M, 1));
	    ...
	    covar = ((N / (N + 5.0)) * cov.diagonal()
		     + 1e-3 * (5.0 / (N + 5.0)) * Eigen::VectorXd::Ones(cov.cols())).asDiagonal();
	    covar_is_diagonal = true;
	  }
