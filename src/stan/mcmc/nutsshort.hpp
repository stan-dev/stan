#ifndef __STAN__MCMC__NUTSSHORT_H__
#define __STAN__MCMC__NUTSSHORT_H__
...
namespace stan {
  ...
  namespace mcmc {
     ...
     */
    class nutsshort : public sampler {
    private:
    ...
    public:
       ...
       */
      nutsshort(mcmc::prob_grad& model,
	...
	: _model(model),
	  ...
	  _rand_uniform_01(_rand_int), shortcrit(log(4)) {
...
//                 _meanacceptprob, H, _H0);
        ...
        _ninslice = 1 + (u < H);
        int ninsliceclose = 0;
        int criterion = computeCriterion(xplus, xminus, mplus, mminus) * (u < H + shortcrit);
//         criterion &= nvalid == 2;
...
//           fprintf(stderr, "doubling. depth = %d\n", depth);
          ...
          int nvalid1 = nvalid;
          nvalid += nvalid2;
          criterion = computeCriterion(xplus, xminus, mplus, mminus) * stopall;
//           criterion &= nvalid == pow(2, depth+1);
//           if (_rand_uniform_01() < float(nvalid2) / float(nvalid1 + nvalid2)) {
          if (!stopall && 
              (_rand_uniform_01() < float(nvalid2) / (1e-100+float(nvalid1)))){
            _x = newsample;
            _g = newgrad;
            _E = -newlogp;
          }
...
//           double g = _meanacceptprob - _delta;
	...
	return s;
      ...
      void recurse(int direction, std::vector<double>& x,
                   ...
                   int& criterion, int& nvalid, int& stopall) {
        stopall = 0;
        if (depth == 1) { // base case
          ...
          _ninslice += nvalid;
          if ((u >= Hplus + shortcrit) || (u >= Hminus + shortcrit))
            stopall = 1;
        } else { // depth > 1
