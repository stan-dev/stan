...
namespace stan {
  namespace prob {
    ...
    template <typename T_y, typename T_shape, typename T_scale, class Policy>
    inline void
    inv_gamma_propto_log(stan::agrad::var& lp, const T_y& y, const T_shape& alpha, const T_scale& beta, const Policy& /* pol */) {
      lp += inv_gamma_propto_log (y, alpha, beta, Policy());
    }
    /**
     * The log of a distribution proportional to an inverse gamma density for y with the specified
     * shape and scale parameters.
    ...
    inv_gamma_propto_log(stan::agrad::var& lp, const T_y& y, const T_shape& alpha, const T_scale& beta) {
      lp += inv_gamma_propto_log (y, alpha, beta, boost::math::policies::policy<>());
    }
