...
namespace stan {
  ...
  const double deriv_logerfc = -2.0 / std::sqrt(pi_dbl) * exp(-(mu_dbl + lambda_dbl * sigma_dbl * sigma_dbl - y_dbl) / (std::sqrt(2.0) * sigma_dbl) * (mu_dbl + lambda_dbl * sigma_dbl * sigma_dbl - y_dbl) / (sigma_dbl * std::sqrt(2.0))) / boost::math::erfc((mu_dbl + lambda_dbl * sigma_dbl * sigma_dbl - y_dbl) / (sigma_dbl * std::sqrt(2.0)));
    ...
    exp_mod_normal_log(const T_y& y, const T_loc& mu, const T_scale& sigma, const T_inv_scale& lambda) {
      return exp_mod_normal_log<propto>(y,mu,sigma,lambda,stan::math::default_policy());
    }
    ...
    inline
    typename return_type<T_y,T_loc,T_scale, T_inv_scale>::type
    exp_mod_normal_log(const T_y& y, const T_loc& mu, const T_scale& sigma, const T_inv_scale& lambda, const Policy&) {
      return exp_mod_normal_log<false>(y,mu,sigma,lambda,Policy());
    }
    ...
    typename return_type<T_y,T_loc,T_scale, T_inv_scale>::type
    exp_mod_normal_log(const T_y& y, const T_loc& mu, const T_scale& sigma, const T_inv_scale& lambda) {
      return exp_mod_normal_log<false>(y,mu,sigma,lambda,stan::math::default_policy());
    }
    ...
    exp_mod_normal_cdf(const T_y& y, const T_loc& mu, const T_scale& sigma, const T_inv_scale& lambda,
      ...
      if (!check_not_nan(function, sigma, "Scale parameter", 
                         &cdf, Policy()))
        return cdf;      
      if (!check_finite(function, sigma, "Scale parameter", &cdf, Policy()))
        return cdf;
      if (!check_positive(function, sigma, "Scale parameter", 
                          &cdf, Policy()))
        return cdf;
  ...
  if(boost::math::isinf(y_vec[n]))
    {
      if (y_vec[n] < 0.0)
        ...
        cdf *= 0.5 * (1 + erf((y_vec[n] - mu_vec[n]) / (sqrt(2.0) * sigma_vec[n]))) - exp(-lambda_vec[n] * (y_vec[n] - mu_vec[n]) + lambda_vec[n] * sigma_vec[n] * lambda_vec[n] * sigma_vec[n] / 2.0) * (0.5 * (1 + erf((y_vec[n] - mu_vec[n] - sigma_vec[n] * lambda_vec[n] * sigma_vec[n]) / (sqrt(2.0) * sigma_vec[n]))));
      }
    ...
    typename return_type<T_y, T_loc, T_scale, T_inv_scale>::type
    exp_mod_normal_cdf(const T_y& y, const T_loc& mu, const T_scale& sigma, const T_inv_scale& lambda) {
      return exp_mod_normal_cdf(y,mu,sigma,lambda,stan::math::default_policy());
    }
