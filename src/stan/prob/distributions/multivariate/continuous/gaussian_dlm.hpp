...
namespace stan {
  namespace prob {
    ...
    gaussian_dlm_log(const Eigen::Matrix<T_y,Eigen::Dynamic,Eigen::Dynamic>& y,
      ...
      if (include_summand<propto,T_y,T_F,T_G,T_V,T_W>::value) {
        ...
        for (int i = 0; i < T; ++i) {
          ...
          Q_inv = inverse(Q);
          // LDLT_factor<typename 
          //             boost::math::tools::promote_args<T_y,T_F,T_G,T_V,T_W>::type,
          //             Eigen::Dynamic,Eigen::Dynamic> ldlt_Q(Q);
          // if (!ldlt_Q.success()) {
     ...
     * @param F A n x r matrix. The design matrix.
     * @param G A n x n matrix. The transition matrix.
     * @param V A size r vector. The diagonal of the observation covariance matrix.
     * @param W A n x n matrix. The state covariance matrix.
     * @return The log of the joint density of the GDLM.
     * @throw std::domain_error if a matrix in the Kalman filter is
     * not semi-positive definite.
    ...
    gaussian_dlm_log(const Eigen::Matrix<T_y,Eigen::Dynamic,Eigen::Dynamic>& y,
      ...
      if (include_summand<propto,T_y,T_F,T_G,T_V,T_W>::value) {
        ...
        Eigen::Matrix<typename
                      boost::math::tools::promote_args<T_y,T_F,T_G,T_V,T_W>::type,
                      Eigen::Dynamic, Eigen::Dynamic> R(n, n);
        <typename boost::math::tools::promote_args<T_y,T_F,T_G,T_V,T_W>::type f(r);
        <typename boost::math::tools::promote_args<T_y,T_F,T_G,T_V,T_W>::type Q;
        <typename boost::math::tools::promote_args<T_y,T_F,T_G,T_V,T_W>::type Q_inv;
        <typename boost::math::tools::promote_args<T_y,T_F,T_G,T_V,T_W>::type e;
        Eigen::Matrix<typename
                      boost::math::tools::promote_args<T_y,T_F,T_G,T_V,T_W>::type,
                      Eigen::Dynamic, 1> A(n);
        ...
        for (int i = 0; i < T; ++i) {
          ...
          // Filter
          for (int j = 0; i < r; ++j) {
            <typename boost::math::tools::promote_args<T_y>::type yij(y(i, j));
            // select row of F
            f = multiply(transpose(f.col(j)), a);
            Q = quad_form_sym(R, F) + V;
            e = yi - f;
            A = multiply(multiply(R, F), Q_inv);            
            m = add(a, multiply(A, e));
