...
inline auto gpdfit(const EigArray& x, bool wip = true,
  ...
  double xstar = x[std::floor(static_cast<double>(N) / 4 + 0.5)
                   - 1];  // first quartile of sample
  auto theta
      = (1.0 / x[N - 1] + (1.0 - (M / (jj - 0.5)).sqrt()) / prior / xstar)
            .eval();
  auto l_theta
      = (static_cast<double>(N) * lx(theta, x)).eval();   // profile log-lik
...
inline auto psis_smooth_tail(const EigArray& x, double cutoff) {
  ...
  // FIXME: Always have k in the first or last spot
...
template <typename EigArray>
inline auto get_psis_weights(const EigArray& log_ratios_i, size_t tail_len_i) {
