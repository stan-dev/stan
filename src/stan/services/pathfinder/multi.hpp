...
inline int pathfinder_lbfgs_multi(
  ...
  const double pathfinders_delta_time = std::chrono::duration_cast<std::chrono::milliseconds>(
  ...
  double psis_delta_time = std::chrono::duration_cast<std::chrono::milliseconds>(
                              end_psis_time - start_psis_time)
                              .count()
                          / 1000.0;
  parameter_writer();
  const auto time_header = std::string("Elapsed Time: ");
  std::string optim_time_str =  time_header + std::to_string(pathfinders_delta_time) + " seconds (Pathfinders)";
  parameter_writer(optim_time_str);

  std::string psis_time_str = std::string(time_header.size(), ' ') + std::to_string(psis_delta_time)
      + " seconds (PSIS)";
  parameter_writer(psis_time_str);

  std::string total_time_str = std::string(time_header.size(), ' ') + std::to_string(pathfinders_delta_time + psis_delta_time)
      + " seconds (Total)";
