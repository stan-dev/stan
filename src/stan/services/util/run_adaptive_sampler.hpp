...
/**
 * Runs the sampler with adaptation using the dispatcher for output.
 *
...
void run_adaptive_sampler(Sampler& sampler, Model& model,
  ...
  util::generate_transitions(sampler, num_warmup, 0, num_warmup + num_samples,
                             num_thin, refresh, save_warmup, true, dispatcher,
                             logger, num_model_values, s, model, rng, interrupt,
                             chain_id, num_chains);
  ...
  // Use the generate_transitions function that takes a dispatcher
  util::generate_transitions(sampler, num_samples, num_warmup,
                             num_warmup + num_samples, num_thin, refresh, true,
                             false, dispatcher, logger, num_model_values, s,
                             model, rng, interrupt, chain_id, num_chains);
  ...
  util::write_timing(warm_delta_t, sample_delta_t, dispatcher, logger);
}
