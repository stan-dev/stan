...
template <class Model, typename InitContext, typename InitInvContext, typename SampWriter, typename DiagnoseWriter>
int hmc_nuts_diag_e_adapt(
    ...
    unsigned int window, callbacks::interrupt& interrupt,
    callbacks::logger& logger, callbacks::writer& init_writer,
    std::vector<SampWriter>& sample_writer, std::vector<DiagnoseWriter>& diagnostic_writer, size_t n_chain) {
...
template <class Model, typename InitContext, typename SampWriter, typename DiagnoseWriter>
int hmc_nuts_diag_e_adapt(
