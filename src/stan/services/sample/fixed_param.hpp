...
template <typename Model, typename InitContextPtr,
          typename InitWriter, typename SampleWriter, typename DiagnosticWriter>
int fixed_param(Model& model, const std::size_t num_chains, const std::vector<InitContextPtr>& init,
                unsigned int random_seed, unsigned int chain,
                double init_radius, int num_samples, int num_thin, int refresh,
                callbacks::interrupt& interrupt, callbacks::logger& logger,
  ...
  for (int i = 0; i < num_chains; ++i) {
    ...
    auto cont_vector = util::initialize(
        model, *init[i], rngs[i], init_radius, false, logger, init_writer[i]);
    cont_vectors.push_back(Eigen::Map<Eigen::VectorXd>(cont_vector.data(), cont_vector.size()));
  ...
  tbb::parallel_for(
    ...
    [&samplers, &writers, &samples, &model, &rngs, &interrupt, &logger,
     num_samples, num_thin, refresh, chain, num_chains](const tbb::blocked_range<size_t>& r) {
        for (size_t i = r.begin(); i != r.end(); ++i) {
          auto start = std::chrono::steady_clock::now();
          util::generate_transitions(samplers[i], num_samples, 0, num_samples, num_thin,
                                    refresh, true, false, writers[i], samples[i], model, rngs[i],
                                    interrupt, logger, chain + i, num_chains);
          auto end = std::chrono::steady_clock::now();
          double sample_delta_t
              = std::chrono::duration_cast<std::chrono::milliseconds>(end - start)
                    .count()
