 ...
 * of <code>writer</code> that writes to a stream.
 */
class stream_file_writer : public writer {
 public:
  /**
   * Constructs a stream writer with an output stream
   * and an optional prefix for comments.
   ...
   */
   ...
   explicit stream_file_writer(T&& output,
                         const std::string& comment_prefix = "")
      : output_(std::move(output)), comment_prefix_(comment_prefix) {}

  stream_file_writer();
  stream_file_writer(stream_file_writer& other) :
   output_(std::move(other.output_)) , comment_prefix_(other.comment_prefix_) {}
  stream_file_writer(stream_file_writer&& other) :
