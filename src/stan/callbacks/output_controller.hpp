...
class output_controller {
  ...
  std::shared_ptr<writer> create_writer(const OutputConfig& config) {
    switch (config.format) {
      ...
      case OutputFormat::JSON: {
        ...
        auto jwriter = std::make_shared<json_writer<std::ofstream, std::default_delete<std::ofstream>>>(
            std::move(file_ptr));
        return std::static_pointer_cast<writer>(jwriter);
      }
      default:
 ...
 public:
  ...
  json_writer<Stream, Deleter>* get_json_writer(const std::string& info_type) {
    ...
    auto* jwriter = dynamic_cast<json_writer<Stream, Deleter>*>(writer.get());
    if (!jwriter) {
  ...
  void write(const std::string& info_type, const std::vector<double>& data) {
    auto swriter = get_writer(info_type);
    swriter->operator()(data);
  }
