...
namespace stan {
  ...
  namespace io {
	...
	// no op
      ...
      template <typename T> 
      void dims(Matrix<T,Dynamic,Dynamic> m, 
	...
	ds.push_back(m.cols());
      ...
      template <typename T> 
      void dims(Matrix<T,Dynamic,1> v, 
		std::vector<unsigned int> ds) {
	ds.push_back(v.size());
      }

      template <typename T> 
      void dims(Matrix<T,1,Dynamic> rv, 
		std::vector<unsigned int> ds) {
	...
	write_val(x[idx[pos]]);
      ...
      void write_stan_val(const Matrix<double,Dynamic,Dynamic>& x,
			  const std::vector<unsigned int>& idx,
			  const unsigned int pos) {
	unsigned int next_pos = pos + 1;
	write_val(x(idx[pos],idx[next_pos]));
      }
      void write_stan_val(const Matrix<double,1,Dynamic>& x,
			  const std::vector<unsigned int>& idx,
			  const unsigned int pos) {
	write_val(x[idx[pos]]);
      }
      void write_stan_val(const Matrix<double,Dynamic,1>& x,
			  const std::vector<unsigned int>& idx,
			  const unsigned int pos) {
	write_val(x[idx[pos]]);
      }
	...
	write_val(x);
      ...
      void write_stan(const Matrix<double,1,Dynamic>& x) {
	write_list(x);
      }
      void write_stan(const Matrix<double,Dynamic,1>& x) {
	write_list(x);
      }
      void write_stan(const Matrix<double,Dynamic,Dynamic>& x) {
	out_ << "structure(c(";
	...
	while (reader.next()) {
	  if (reader.is_int()) {
	    vars_i_[reader.name()] = std::pair<std::vector<int>, 
					       std::vector<unsigned int> >(reader.int_values(), 
									   reader.dims());
	    
	  } else {
	    vars_r_[reader.name()] = std::pair<std::vector<double>, 
