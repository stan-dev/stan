 ...
 */
template <typename T>
class deserializer<T, require_not_var_matrix_t<T>> {
 private:
 ...
 */
template <typename T>
class deserializer<stan::math::var_value<T>, require_matrix_t<T>> {
 private:
  stan::math::var_value<T> map_r_;    // map of reals.
  Eigen::Map<Eigen::Matrix<int, -1, 1>> map_i_;  // map of integers.
  size_t r_size_{0};                             // size of reals available.
  size_t i_size_{0};                             // size of integers available.
  size_t pos_r_{0};  // current position in map of reals.
   ...
   * @param m amount to move `pos_r_` up.
   */
  inline stan::math::var scalar_ptr_increment(size_t m) {
    pos_r_ += m;
    return map_r_.coeff(pos_r_ - m);
  }
  ...
  template <typename S, typename K>
  using conditional_var_val_t
      = std::conditional_t<is_var_matrix<S>::value,
                           return_var_matrix_t<K, S, K>, K>;
 ...
 public:
  using matrix_t = Eigen::Matrix<stan::math::var, Eigen::Dynamic, Eigen::Dynamic>;
  using vector_t = Eigen::Matrix<stan::math::var, Eigen::Dynamic, 1>;
  using row_vector_t = Eigen::Matrix<stan::math::var, 1, Eigen::Dynamic>;
   ...
   * @param data_i Sequence of integer values.
   */
  template <typename RVec, typename IntVec, require_var_matrix_t<RVec>* = nullptr,
            require_vector_like_t<IntVec>* = nullptr>
  deserializer(RVec& data_r, IntVec& data_i)
      : map_r_(data_r),
        map_i_(data_i.data(), data_i.size()),
        r_size_(data_r.size()),
  ...
  inline auto read() {
    ...
    auto real = scalar_ptr_increment(1);
    auto imag = scalar_ptr_increment(1);
    return std::complex<stan::math::var>{real, imag};
  }
  ...
  inline auto read(Eigen::Index m) {
    if (unlikely(m == 0)) {
      ...
      check_r_capacity(m);
      stan::math::arena_matrix<vector_t> ret = map_r_.val().segment(pos_r_, m);
      stan::math::reverse_pass_callback(
          [ret, map_r_ = this->map_r_, write_start = pos_r_, write_size = m]() mutable {
             map_r_.adj().segment(write_start, write_size) += ret.adj(); });
      pos_r_ += m;
      return vector_t(std::move(ret));
//      return map_vector_t(&scalar_ptr_increment(m), m);
    }
  }
  ...
  inline auto read(Eigen::Index m) {
    if (unlikely(m == 0)) {
      ...
      for (Eigen::Index i = 0; i < m; ++i) {
        auto real = scalar_ptr_increment(1);
        auto imag = scalar_ptr_increment(1);
        ret.coeffRef(i) = std::complex<stan::math::var>{real, imag};
      }
      return ret;
    }
  }
  ...
  inline auto read(Eigen::Index m) {
    if (unlikely(m == 0)) {
      return row_vector_t(m);
    } else {
      check_r_capacity(m);
      stan::math::arena_matrix<row_vector_t> ret = map_r_.val().segment(pos_r_, m);
      stan::math::reverse_pass_callback(
          [ret, map_r_ = this->map_r_, write_start = pos_r_, write_size = m]() mutable {
             map_r_.adj().segment(write_start, write_size) += ret.adj(); });
      pos_r_ += m;
      return row_vector_t(std::move(ret));
      //return map_row_vector_t(&scalar_ptr_increment(m), m);
    }
  }
  ...
  inline auto read(Eigen::Index rows, Eigen::Index cols) {
    if (rows == 0 || cols == 0) {
      ...
      check_r_capacity(total_size);
      stan::math::arena_matrix<matrix_t> ret = Eigen::Map<Eigen::MatrixXd>(map_r_.vi_->val_.data() + pos_r_, rows, cols);
      stan::math::reverse_pass_callback(
          [ret, map_r_ = this->map_r_, write_start = pos_r_, rows, cols]() mutable {
             Eigen::Map<Eigen::MatrixXd>(map_r_.adj().data() + write_start, rows, cols) += ret.adj(); });
      pos_r_ += rows * cols;
      return matrix_t(std::move(ret));
      //return map_matrix_t(&scalar_ptr_increment(rows * cols), rows, cols);
    }
  }
   ...
   * @param m Size of column vector.
   */
  template <typename Ret, require_var_col_vector_t<Ret>* = nullptr>
  inline auto read(Eigen::Index m) {
    if (unlikely(m == 0)) {
      return Ret(map_r_.segment(pos_r_, 0));
    } else {
      check_r_capacity(m);
      pos_r_ += m;
      return Ret(map_r_.segment(pos_r_ - m, m));
    }
  }

  template <typename Ret, require_var_row_vector_t<Ret>* = nullptr>
  inline auto read(Eigen::Index m) {
    if (unlikely(m == 0)) {
      return Ret(map_r_.segment(pos_r_, 0).transpose());
    } else {
      check_r_capacity(m);
      pos_r_ += m;
      return Ret(map_r_.segment(pos_r_ - m, m).transpose());
    }
  }

  template <typename Ret, require_eigen_matrix_dynamic_t<typename std::decay_t<Ret>::value_type>* = nullptr,
      require_var_matrix_t<Ret>* = nullptr>
  inline auto read(Eigen::Index rows, Eigen::Index cols) {
    if (rows == 0 || cols == 0) {
      return Ret(matrix_t(rows, cols));
    } else {
      const auto total_size = rows * cols;
      check_r_capacity(total_size);
      using vari_type = typename Ret::vari_type;
      using value_type = typename Ret::value_type;
      using map_value_type = Eigen::Map<value_type>;
      Ret ret = new vari_type(
        map_value_type(map_r_.vi_->val_.data() + pos_r_, rows, cols),
        map_value_type(map_r_.adj().data() + pos_r_, rows, cols));
      pos_r_ += rows * cols;
      return ret;
      //return map_matrix_t(&scalar_ptr_increment(rows * cols), rows, cols);
    }
