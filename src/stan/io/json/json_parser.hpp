...
class parser {
 public:
  parser(Handler& h, std::istream& in)
      : h_(h), in_(in), next_char_(0), line_(0), column_(0) {}
 ...
 private:
  ...
  void get_escaped_unicode(std::stringstream& s) {
    ...
    if (!(is_high_surrogate(codepoint) || is_low_surrogate(codepoint))) {
      ...
      unsigned int supplemental = ((codepoint - MIN_HIGH_SURROGATE) << 10)
  ...
  void get_chars(const std::string& s) {
    for (size_t i = 0; i < s.size(); ++i) {
      char c = get_char();
      if (c != s[i])
        throw json_exception("expecting rest of literal: " + s.substr(i));
    }
  }
