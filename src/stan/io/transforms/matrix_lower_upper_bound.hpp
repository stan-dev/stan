#ifndef STAN__IO__TRANSFORMS__MATRIX_LOWER_UPPER_BOUND_HPP
#define STAN__IO__TRANSFORMS__MATRIX_LOWER_UPPER_BOUND_HPP
...
namespace stan {
  namespace io {
    ...
    template <typename T>
    class matrix_lower_upper_bound: public matrix_transform<T> {
    private:
      T lower_bound_;
      T upper_bound_;

    public:
      matrix_upper_bound(T lower_bound_, T upper_bound,
                         idx_t N_rows, idx_t N_cols):
      ...
      void unconstrain(const matrix_t& input, std::vector<T>& output) {
        for (idx_t col = 0; col < N_cols_; ++col)
          for (idx_t row = 0; row < N_rows_; ++row)
            output.push_back(stan::prob::lub_free(input(row, col),
      ...
      void constrain(const std::vector<T>& input, matrix_t& output) {
        for (idx_t col = 0; col < N_cols_; ++col)
          for (idx_t row = 0; row < N_rows_; ++row)
            output(row, col) =
              stan::prob::lub_constrain(input[row * N_cols_ + col],
                                       lower_bound_, upper_bound_);
      }
