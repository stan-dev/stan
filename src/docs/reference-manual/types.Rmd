...
affine transformation $x\mapsto \mu + \sigma * x$ with location $\mu$ and
(positive) scale $\sigma$, using a syntax similar to
that for bounds.
...
The variable `x` may be declared to have location $1$ as follows.

```stan
real<location=1> x;
```

Similarly, it can be declared to have scale $2$ as follows.

```stan
real<scale=2> x;
```

Finally, we can combine both declarations to declare a variable with location
$1$ and scale $2$.

```stan
real<location=1,scale=2> x;
```
...
### Expressions as Bounds and Location/Scale{-}

Bounds (and location and scale)
...
Similarly, they may be declared with a location and/or scale, as in the
following example
```stan
vector<location=42,scale=3>[3] u;
```
...
```
Location and scale are also similar as for the following 3-row-vector with
location -42 and scale 3.
```stan
row_vector<location=-42,scale=3>[3] u;
```
...
Similarly, matrices can be declared to have a set location and/or scale, as in
this matrix with scale 5.
```stan
matrix<scale=5>[3, 4] B;
```
