...
\Abstract{ \proglang{Stan} is a probabilistic programming language for
  specifying statistical models. A \proglang{Stan} program defines a
  log probability function over parameters.  Other variable types
  include (transformed) data, transformed parameters, and generated
  quantities. Variables in \proglang{Stan} programs are strongly typed
  and may be declared with constraints, such as positivity for a
  scalar or positive definiteness for a matrix. Statements, such 
  as assignment, sampling, conditionals, and loops, are executed
  imperatively in the order they are written.
  %
  % Constrained variables
  % are automatically transformed and the log Jacobian adjustment made
  ...
  % 
  % \proglang{Stan} programs are translated to \proglang{C++}
  % and compiled.  The target \proglang{C++} code includes an
  % extensible
  % custom library of special functions, matrix and linear algebra
  % operations, and probability mass, density and cumulative
  % distribution functions.  Functions are vectorized and support
  % higher-order algorithmic
  % differentiation to calculate gradients, Hessians, and
  % Hessian-vector products efficiently up to machine precision.
  % 
  % There is an extensive I/O library to deal with constrained
  % variable input for data, initialization for parameters, and output
  % for samples or point estimates.  
  %
  \proglang{Stan} can be called from the command line or through \proglang{R} using
  the \pkg{RStan} package.  Both interfaces support running samplers
  or optimizers and analyzing the resulting inferences.
}
...
%  BFGS, 
%  \proglang{C++}
  \proglang{Stan}} %% at least one keyword must be supplied
%
\Plainkeywords{
  probabilistic programming,
%  Bayesian inference,
...
i.i.d.\ with Bernoulli chance-of-success \code{theta}.  The prior on
\code{theta} is \code{beta(1,1)} (i.e., uniform).

\subsubsection{Semicolons, whitespace and comments}

Unlike in \proglang{R}, \proglang{BUGS}, and \proglang{JAGS}, the form
of whitespace does not matter in \proglang{Stan}.  This allows an
arithmetic expression such as \code{a + b} to be broken so that the
...
Longer comments may be included between the \proglang{C++} style
...
\code{theta ~ beta(1,1)} could be removed becuase \code{beta(1,1)} is
...
statement \code{y ~ bernoulli(theta)}.  Performance gains from
...
After a \proglang{C++} toolchain is installed,%
%
\footnote{A \proglang{C++} toolchain is built into Linux.  The 
  \pkg{RTools} package suffices for Windows and \pkg{Xcode} for the
  Mac.}
%
...
\footnote{The first time it is run it needs to build the entire
  \proglang{Stan} translator and library with high levels of
  optimization, so please be patient (and consider \code{make} option
  \code{-j2} or \code{-j4} if two or four cores are available).}
%
\begin{CodeChunk}
\begin{CodeInput}
...
(\code{bernoulli.exe} on Windows) on the specified path (yes, forward
