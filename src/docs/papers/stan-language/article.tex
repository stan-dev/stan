...
The models we wanted to explore turned out to be a challenge for
existing general-purpose software to fit.  A direct encoding in
...
We added custom vectorized logistic regressions to \proglang{JAGS}
using \proglang{C++} to overcome the cost of interpretation.  Although
this is much faster than looping in \proglang{JAGS}, the root of the
problem was that conditional sampling took too long to converge when
...
toolkit (see \citep{BellBurke:2008}).  But neither package supported
very many special functions (e.g., probability functions, log gamma,
inverse logit) or linear algebra operations (e.g., Cholesky
decompositions, matrix division) and were not easily and modularly
extensible.
...
complete description of template metaprogramming techniques.  But we
...
we initially began by promoting floating-point vectors to alorithmic
...
models was variables with constrained support, such as simplexes and
covariance matrices.  Although it is possible to implement HMC with
reflection (i.e., bouncing) for simple boundary constraints (e.g.,
positive scale or precision parameters), it is not so easy with more
complex multivariate constraints.  To get around this problem, we
introduced typed variables and automatically transformed them to
...
rejection rates, but the number of steps was not so easy to tune while
maintaining detailed balance in the sampler.  This led to the
...
not rotation invariant.  The last tweak to \proglang{Stan} before
version 1.0 was to estimate a diagonal mass matrix during warmup; this
has since been upgraded to a full mass matrix in \proglang{Stan}
version 1.2.  Using a mass matrix sped up the non-standardized data
models by an order of magnitude, though it breaks the nice theoretical
property of rotation invariance.  The full mass matrix estimation has
rotational invariance as well, but scales less well because of the
need to invert the mass matrix once and then do matrix multiplications
every leapfrog step.  

Even full mass matrix estimation only estimates a global mass matrix;
some models, such as hierarchical models, have a scale that varies by
location in the posterior.  RM-HMC (Riemann-manifold HMC) solves the
varying scale problem by estimating a mass matrix based on its
position and adjusting the integrator to match
\citep{GirolamiCalderhead:2011}.
...
\code{theta} is \code{beta(1,1)} (i.e., uniform), though there is no
special behavior for conjugate priors in \proglang{Stan}.  This model
is available in the \proglang{Stan} source distribution in file
...
\footnote{Acceptance is the usual notion for Metropolis-Hastings
  sampling.  For NUTS, the acceptance statistic is the average of the
  acceptance probabilities along the path defined by the leapfrog
  integrator.}
%
The column \code{stepsize\_\_} indicates the step size
(i.e., time interval) for the discretization of the Hamiltonian in the
leapfrog integrator.  The column \code{treedepth\_\_} is the tree
depth for NUTS, which is the log base 2 of number of leapfrog steps.
...
%
There is one multiplier per parameter, and they are adjusted to
identify the product of step size and parameter size used for
sampling.  Because of Metropolis rejection, parameter values may be
repeated.  The file ends with comments reporting the elapsed time.
...
The cahins can be safely run in parallel under different processes.
How to do this depends on the operating system and the shell or
terminal program, and will likely cause the terminal output to be
interleaved if each command directs output to the same shell.
%
\begin{CodeChunk}
{\small
\begin{CodeInput}
...
is used to skip the pseudorandom number generator ahead so that the
values generated for each chain are independent; see
...
containing output from sampling, it is invoked as follows.%
%
...
\code{lognormal} density function  using the normal density as follows.
%
\begin{Code}
parameters {
  real<lower=0> u;
...
message is printed.  If initial values are supplied randomly, the
...
\end{tabular}
\end{center}
\caption{Variables of the kind indicated in the left column must be
  declared in one of the blocks declared in the right column.  Data
...
amplified by the lower-level parameters and ends up introducing a huge
change in density.  Because EM-HMC transitions are, on average,
limited in their density variations, large changes in \code{sigma}
induce huge autocorrelations.  
...
\end{Code}
%
The ``Matt trick'' removes the particular correlations amongst the
...
or full mass matrices.  NUTS automatically adapts the number of
leapfrog steps, eliminating the need for user-specified tuning
parameters.  NUTS is the default sampler for \proglang{Stan}.  HMC and
NUTS require gradients of the log probability function; randomization
plus gradient following allows HMC to explore the typical set of the
...
does not require any derivatives of the log probability function; lack
of gradient information related to the current position leads to
random walk behavior.


\subsubsection{Riemann-Manifold Hamiltonian Monte Carlo}

\proglang{Stan} will soon have an implementation of Riemann-Manifold
...
into the Hamiltonian simulation \cite{Betancourt:2012}.  This
...
warmup.  \proglang{Stan}'s MCMC and HMC framework make it
straightforward to configure with new metrics.  RM-HMC requires
gradients, Hessians, and a specialized form of third-order derivative
information.  Use of curvature information leads to very efficient
adaptation to varying scales and curvature in the posterior.
...
\end{tabular}
\end{center}
\caption{\proglang{Stan}'s unary and binary operators, with their
