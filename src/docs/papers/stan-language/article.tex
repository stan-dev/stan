...
The column \code{stepsize\_\_} indicates the step size (i.e., time
interval) for the discretization of the Hamiltonian in the leapfrog
integrator.  The column \code{treedepth\_\_} is the tree depth for
...
\code{best[j]} to 1 if hospital \code{j} has the lowest estimated
...
\code{log1m(x)}, which is more stable arithmetically for values of x
near 0 than \code{log(1 - x)}.
...
a real.  A postfix apostrophe (\code{'}) is used for matrix
...
much more arithmetically stable than inversion, e.g., \code{Sigma \ (y
  - mu)} computes the same function as \code{inverse(Sigma) * y}.
\proglang{Stan} also supports elementwise multiplication (\code{.*})
and division (\code{./}).
...
probability functions built in (compared to base \proglang{R} or
\proglang{BUGS}).  The probability functions all share various
...
example, \code{normal\_log(y,mu,sigma)} requires the scale parameter
...
constant such as \code{pi()}, a variable declared as data or
...
%
probability functions in \proglang{Stan} share vectorized so that they
accept arrays (or vectors) of arguments.  For example, although the
basic type of \code{normal\_log(y,mu,sigma)} is that \code{y},
\code{mu} and \code{sigma} are all real, it supports calls in which
any any or all of \code{y}, \code{mu} and \code{sigma} contain more
than one element. A typical use case would be for linear regression,
where \code{normal\_log(y,X * beta,sigma)}, where \code{y} is a vector
of observed data, \code{X} is a predictor matrix, \code{beta} is a
coefficient vector, and \code{sigma} is a real value for the noise
scale.
...
of virtual calls and required in the compiled \proglang{C++}
executable for calculating gradients and higher-order derivatives.
For example, in the example above, \code{normal\_log(y,X *
  beta,sigma)}, the logarithm of \code{sigma} need only be computed
once, and if \code{y} is an $N$-vector, it reduces the number of
...
optimization strategies can be implemented within \proglang{Stan}.
...
within \proglang{Stan}'s \proglang{C++} framework for sampling and
...
the default sampler for \proglang{Stan}.
...
\proglang{Stan} will soon have two forms of Riemann-Manifold
...
support adapting step sizes during warmup.  \proglang{Stan}'s MCMC and
...
using the \pkg{Boost}'s \pkg{Spirit} parser, which itself depends on
...
the error checking facilities in the \pkg{Math} and the
\pkg{Exception} package.  Output formatting and ad-hoc input parsing
...
documentation, including both the \proglang{C++} application
programmer interface (API) documentation using the \pkg{doxygen}
