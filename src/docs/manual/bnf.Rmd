...
```
program ::=
 ?functions ?data ?tdata ?params ?tparams ?model ?gqs
...
gqs ::= 'generated quantities' var_decls_statements
...
```
function_decl ::=
  unsized_return_type identifier '(' unsized_types ')' statement
...
real_literal ::= integer_literal ?('.' [0-9]*) ?exponent_literal
...
In the Stan grammar provided in this chapter, the expression `1
