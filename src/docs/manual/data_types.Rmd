...
* Strong typing forces the programmer's intent to be declared with
  the variable, making programs easier to comprehend and hence easier
  to debug and maintain.
* Strong typing allows programming errors relative to the declared
...
* Constrained types will catch runtime data, initialization, and
...
raised;  the number of rows of `a` is `N`, which is also the
size of `b`.
...
declares `L` to be a Cholesky factor for a `K` by `K`
correlation matrix.
...
`real[]` is the type of one-dimensional array of reals and
`matrix` is the type of matrices.  The three-dimensional integer
array type is written as `int[ ,  ,]`, indicating the number slots
available for indexing.  Similarly, `vector[ , ]` is the type of a
...
even though `0` is of type `int` and `sum` is of type
`real`, because integer-typed scalar expressions can be assigned
...
declares a matrix variable `a` and assigns it to the value of
`b`, which must be of type `matrix` for the compound
...
assigns the matrix variable `a` to half of the sum of `b`
and `c`.  The only requirement on `b` and `c` is that
the expression `b~+~c` be of type `matrix`.  For example,
`b` could be of type `matrix` and `c` of type
`real`, because adding a matrix to a scalar produces a matrix,
