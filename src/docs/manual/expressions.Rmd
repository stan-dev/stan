...
suffixed with `_lpdf`, `_lpmf`, `_lcdf`, and
`_lccdf`, `_cdf`, and `_ccdf`, such as
`normal_lcdf_log`; this also holds for the deprecated forms
`_log`, `_cdf_log`, and `_ccdf_log`,
...
> `3.0 + 0.14`, `-15`, `2 * 3 + 1`, `(x - y) / 2.0`, `(n * (n + 1)) / 2`, `x / n`, `m % n`
...
[chapter](#matrix-operations.chapter)

For example, if `y` and `mu` are variables of type
`vector` and `Sigma` is a variable of type `matrix`,
then

> `(y - mu)' * Sigma * (y - mu)`

is a well-formed expression of type `real`.  The type of the
complete expression is inferred working outward from the
subexpressions.  The subexpression(s) `y - mu` are of type
`vector` because the variables `y` and `mu` are of type
`vector`.  The transpose of this expression, the subexpression
`(y - mu)'` is of type `row_vector`.  Multiplication is
...
operations, `a~.*~b` and `a~./b`.  These provide a shorthand
...
> `3 ^ 2`, `3.0 ^ -2`, `3.0 ^ 0.14`, `x ^ n`, `n ^ x`, `n ^ m`, `x ^ y` 
...
tabular form in [figure](#operator-precedence).
...
Table: Stan's unary, binary, and ternary operators, with their
...
result types are provided in the function documentation for the 
operators (i.e., `operator*(int,int):int` indicates the application of the
...
relying on precedence and associativity.{#operator-precedence.figure}
...
[the index types table](#index-types.figure).
...
Table: Types of indexes and examples with one-dimensional
  containers of size `N` and an integer array `ii` of type 
  `int[]` size `K`.{#index-types.figure}
...
[matrix indexing table](#matrix-indexing,figure).
...
Table: Special rules for reducing matrices based on
  whether the argument is a single or multiple index.  Examples are for
a matrix `a`, with integer single indexes `i` and `j`
and integer array multiple indexes `is` and `js`.  The same
typing rules apply for all multiple indexes.{#matrix-indexing.figure}
...
[primitive type table](#primitive-type.figure) for the mapping from types to their
primitive types.
...
Table: The table shows the variable declaration types of Stan
...
If `x` is an expression of total dimensionality greater than or
equal to $N$, then the type of expression `e[i1, ..., iN]` is the
same as that of `e[i1]...[iN]`, so it suffices to define the type
of a singly-indexed function.  Suppose `e` is an expression and
`i` is an expression of primitive type `int`.  Then


* if `e` is an expression of array dimensionality $K > 0$,
  then `e[i]` has array dimensionality $K-1$ and the same
  primitive implementation type as `e`,

* if `e` has implementation type `vector` or
  `row_vector` of array dimensionality 0, then `e[i]` has
  implementation type `real`, and

* if `e` has implementation type `matrix`, then
  `e[i]` has type `row_vector`.
...
functions are listed in the [higher-order functions table](#higher-order-functions).
...
Table: Higher-order functions in Stan with their argument
  ...
  literals.{#higher-order-functions.figure}
...
`sqrt-x-minus-x`, the test can be run as
