...
**Variable Declaration Table.** <a id="where-to-declare-table"></a>
...
the [variable declaration table](#where-to-declare-table).  The last line has no
corresponding location, as there is no need to print a variable
every iteration that does not depend on parameters.^[It is possible to print a variable every iteration that does not depend on parameters---just define it (or redefine it if it is transformed data) in the `generated quantities` block.]
...
**Statistical Variable Taxonomy Table** <a id="variable-kinds-table"></a>
