...
\citep{Geyer:1992, Geyer:2011}.  Stan uses Geyer's initial monotone
...
\subsection{Estimation of MCMC Standard Error}
...
\[
\hat{\theta}_n = \mathrm{mean}(\theta^{(1)}_n, \ldots,
\theta^{(m)}_n)
\]
...
the true value.  This is just the MCMC central limit theorem applied
to an estimator; see \citep{Geyer:2011} for more details of the MCMC
