...
autocorrelations on odd lags. NUTS algorithm used in Stan can
...
\[
\hat{\rho}_t
= 1 - \frac{\displaystyle W - \textstyle \frac{1}{M}\sum_{m=1}^M \hat{\rho}_{t,m}}{\widehat{\mbox{var}}^{+}}.
\]
...
$t$ increases, only the initial estimates of $\hat{\rho}_t$ until lag
$T$ are typically used. Negative autocorrelations can happen only on
odd lags and by summing over pairs starting from lag 0, the paired
autocorrelation is guaranteed to be positive modulo estimator noise
\citep{Geyer:2011}.  Stan uses Geyer's initial positive sequence
criterion and the effective sample size estimator is defined as
\[
...
\]
%
where $T$ is the first odd positive integer for which
$\hat{\rho}_{T+1}+\hat{\rho}_{T+2}$ is negative.
