...
computation of the covariance matrix \code{K} is now in the model
...
covariance matrix \code{K} is an unknown dependent on the
...
functions. In this case, an inverse gamma will work well as it has a sharp left
tail that puts negligible mass on length-scales, but a generous right tail, allowing
...
of zero crossings, is $T / \pi \rho$. You can see that as $\rho$ decreases,
...
covariates. However, this statistic is only valid for one-dimensional inputs.
...
conditional mean of the total output. If we want to allow for the possibility
that the GP won't contribute to the conditional mean, we can encode this in the
marginal standard-deviation by allowing the prior for $\alpha$ to put mass at
zero.
...
would be
\[
  K(x | \alpha, \rho, \sigma)_{i, j} = \eta^2 \exp(-\dfrac{1}{2 \rho^2} \sum_{d=1}^D (x_{i,d} -
\tilde{x}_{j,d})^2).
\]
...
Suppose we have $N$ observations of $y_{[i,]} \in \reals^M$ each observed at
$x_i \in \reals^K$. One can model each of the $M$ dimensions of $y$, $y_m$:
\begin{align*}
  y_m \, |  \, x & \sim \distro{Normal}(f_m(x), \sigma) \\
  f_m(x) & \sim \distro{GP}(m(x), K(x | \alpha_m, \rho))
\end{align*}
We model each dimension of $y_i$ being generated from an $M$-dimensional vector 
$f_i$ with a full-rank covariance matrix defining the covariance
between dimensions of $f_i$ and $f_{i,j}$ .
We could fit the following model to the data:
\begin{align*}
  f & \sim \distro{MatrixNormal}(\mu(x), K(x | \alpha, \rho), C) \\
  y_{i, m} & \sim \distro{Normal}(f_{i,m}, \sigma) \\
  f & \in \reals^{N \times M}
\end{align*}
The \distro{MatrixNormal} distribution has two covariance matrices: $K(x |
\alpha, \rho)$ to encode among-row covariance, and $C$ to define among-column
covariance. This means that $\E\left[f^T f\right]$ is equal to
...
\begin{align*}
	...
	f & = L_{K(x | 1.0, \rho)} \, \eta \, L_C^T \\
	f & \sim \distro{MatrixNormal}(0, K(x | 1.0, \rho), C) \\
