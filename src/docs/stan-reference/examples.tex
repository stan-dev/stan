...
\begin{align}
  f & \sim \distro{MatrixNormal}(\mu(x), k(x), C) \\
  y_{i, m} & \sim \distro{Normal}(f_{i,m}, \sigma) \\
  f & \in \reals^{N \times M}
\end{align}

The \distro{MatrixNormal} distribution has two covariance matrices: $k(x)$ to deal with
among-row covariance, and $C$ to deal with among-column covariance.
