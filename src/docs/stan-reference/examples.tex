...
problem, that is, given a function $f(x)$, we wish to find $x$ such that
$f(x) = 0$.
...
using an inverse matrix. The algebraic solver becomes handy when we want
...
$x_1$ and $x_2$, such that the vector function $z$ goes to 0. 
%
...
\end{stancode}
%
The function takes the unknowns we wish to solve for in \code{x} (a real array),
the system parameters in \code{theta} (a real array), the real data in
\code{dat\_r} (a real array) and the integer data in \code{dat\_i} (an integer array). The
...
return type. This may require passing in zero-length arrays for data or parameter
if the system does not involve data or parameters.

\section{Calling the Algebraic Solver}
%
Let's suppose $\theta = \{3, 6\}$. To call the algebraic solver, we need to
provide an initial guess. A good guess can significantly speed up the solver and
even determine whether the solver converges or not.
%
...
transformed parameters {
  ...
  x = algebra_solver(guess, theta, dat_r, dat_i);
}
\end{stancode}
...
The arguments for the initial guess \code{guess}, the real data \code{dat\_r}, and
the integer data \code{dat\_i} must be expressions that only involve data or
...
using the implicit function theorem, which places certain restrictions. In particular,
the Jacobian of the algebraic function $f$ with respect to the unknowns $x$ must
be invertible. This requires the Jacobian to be square, meaning \textit{$f(x)$ and
$x$ have the same length} or, in other words \textit{the number of equations in
...
\section{Control Parameters for the Algebraic Solver}
%
...
number of steps. Their default values are respectively \code{1e-10} ($10^{-10}$), \code{1e-6}
($10^{-6}$), and \code{1e3} ($10^3$).
...
deviations from this ideal. Users should remember the norm scales with the square root of
$f$'s length.
...
MCMC when a bad jump is taken, particularly during warmup.
