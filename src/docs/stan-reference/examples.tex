...
\[
y \sim \distro{MultiNormal}(m(x), k(x)),
\] 
...
prior at a single input $x$ conditional on one value of $\alpha$ one would
recover $\alpha$.
...
target distribution,
\[
  \mu + Lz \sim \distro{MultiNormal}(\mu(x), k(x)).
\]
...
calculate the covariance matrix \code{Sigma} in the model block, rather than
...
Cholesky-parameterized normal likelihood, only now the value \code{y} is known and the
...
covariance matrix parameterized by a Matern kernel \citep{zhang-gp:2004}. The ratio of the two
...
a model in Stan using Euclidean Hamiltonian Monte Carlo. When the length-scale
falls far below the minimum spacing of the input covariates the functions
supported by the GP will interpolate almost exactly between the input data.
These length-scales yield a likelihood that is nearly flat with respect to
length-scale. If our prior does not sharply regularize small length-scales,
there can be non-negligible mass in these regions of the posterior, which would
require our sampler to decrease its step-size in order to explore these
regions. For example, if our covariate $x$ were equally spaced one unit apart,
a change from a length-scale of $0.1$ to $0.01$ would not yield a change in the
likelihood.  Thus the prior must identify the model below these length-scales.
In order to avoid these small length scales we need a prior that favors length
scales more appropriate to the input data.
...
we likely don't need further constraints.  We'd like to allow the GP prior to
represent both high-frequency and low-frequency functions, so our prior should
put non-negligible mass on both sets of functions. In this case, an inverse
gamma will work well as it has a left tail that puts negligible mass on
length-scales that are small with respect to the input data. 
...
\begin{align}
  f & \sim \distro{MultiNormal}\left(0, K_{\rho,\alpha}(x)\right) \\
  y_i & \sim \distro{Normal}(\beta_0 + x_i \beta_{[1:D]} + f_i, \sigma) \, \forall i \in \{1, \dots, N\} \\
  & x_i^T, \beta_{[1:D]} \in \reals^D,\, x \in \reals^{N \times D}, f \in \
\end{align} 
...
to limit the GP to higher-frequency functions. We can use a generalized inverse Gaussian 
distribution:

\begin{align}
	f(x | a, b, p) = \dfrac{(a/b)^{p/2}}{2K_p(\sqrt{ab})} x^{p - 1}\exp(-(ax + b / x)/2)
\end{align} 
...
covariates. However, this statistic only valid for one-dimensional inputs.
...
would be
\[
  K_{i, j} = \eta^2 \exp(-\dfrac{1}{2 \rho^2} \sum_{d=1}^D (x_{i,d} -
\tilde{x}_{j,d})^2).
\]
...
Suppose we have observations $y_{i} \in \reals^M$ each observed at $x_i \in
\reals$. We could fit the following model to the data:

\begin{align}
  f & \sim \distro{MatrixNormal}(\mu(x), K_{\rho,\alpha}(x), C) \\
  y_{i, m} & \sim \distro{Normal}(f_{i,m}, \sigma) \\
  f & \in \reals^{N \times M}
\end{align}

The \distro{MatrixNormal} distribution has two covariance matrices: $k(x)$ to encode 
among-row covariance, and $C$ to define among-column covariance. This means that the 
expected value of $f^T f$ is equal to $\text{trace}(K_{\rho,\alpha}(x))C$, whereas the expected value of $f f^T$ is 
$\text{trace}(C) \times k(x)$. We can derive this using properties of expectation
and the \distro{MatrixNormal} density:

\begin{align}
	\E[f f^T]_{i,j} & = \E\left[f_i f_j\right] \text{where} f_i, \, f_j \in \reals^M \\
									& = \E\left[\sum_{m = 0}^M f_{i,m} f_{j,m}\right] \\
									& = \sum_{m = 0}^M \E\left[f_{i,m} f_{j,m}\right] \\
									& = \sum_{m = 0}^M \E\left[f_m(x_i) f_m(x_j)\right] \\
									& = \sum_{m = 0}^M C_{m,m} \times k(x_i, x_j | \rho, \alpha) \\
									& = \text{trace}(C) \times k(x_i, x_j)
\end{align}

\begin{align}
	\E[f^T f]_{i,j} & = \E\left[f_i f_j\right] \text{where} f_i, \, f_j \in \reals^N \\
									& = \E\left[\sum_{n = 0}^N f_{n,i} f_{n,j}\right] \\
									& = \sum_{n = 0}^N \E\left[f_{n,i} f_{n,j}\right] \\
									& = \sum_{n = 0}^N \E\left[f_i(x_n) f_j(x_n)\right] \\
									& = \sum_{n = 0}^N C_{i,j} \times k(x_n, x_n | \rho, \alpha) \\
									& = N \alpha^2 \times C_{i, j} \\
									& = \text{trace}(K_{\rho, \alpha}) \times C_{i,j}
\end{align}

We can see that we should set $\alpha$ to $1.0$ because the parameter is not identified unless
...
\begin{align}
	...
	f & = L_{K_{\rho, 1.0}} \, \eta \, L_C^T \\
	f & \sim \distro{MatrixNormal}(0, K_{\rho, 1.0}, C) \\
	\eta & \in \reals^{N \times M} \\
	L_C & = \text{cholesky\_decompose}(C) \\
	L_{K_{\rho, 1.0}} & = \text{cholesky\_decompose}(K_{\rho, 1.0})
\end{align}

This can be implemented in Stan using a latent-variable GP formulation:
...
model {
  matrix[N, M] f;
  {
    matrix[N, N] Sigma = cov_exp_quad(x, 1.0, rho);
    matrix[N, N] L_Sigma;
    ...
    L_Sigma = cholesky_decompose(Sigma);
		f = L_Sigma * eta * diag_pre_multiply(alpha, L_Omega)';
  }
