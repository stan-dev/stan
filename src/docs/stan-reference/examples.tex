...
In many mixture models, the mixture components are exchangeable in the
model and thus not identifiable.  This arises if the parameters of the
mixture components have exchangeable priors and the mixture ratio gets
a uniform prior so that the parameters of the mixture components are
also exchangeable in the likelihood.
...
undertand, the doubly nested loop used for \code{s} and \code{t} is
...
implementations later in this chapter, is a generalized, 
exponented quadratic function,
\[
k(x)_{i, j}
= \alpha^2 
\exp \left(
...
\subsection{Latent variable GP}

We can explicitly code the latent variable formulation of a GP in Stan. This
will be useful for when the outcome is not normal. We'll need to add a small
positive term, $\delta$ to the diagonal of the covariance matrix in order to
ensure that our covariance matrix remains positive definite.
...
If we want to model count data, we can remove the $\sigma$ parameter,
and switch our likelihood from \texttt{normal} to \texttt{poisson\_log}.
...
\subsection{Classification with Gaussian Processes}
...
simulate the $z_n$ from the $f_n$ using the sampling distribution
...
straightforward, though it currently requires the user to directly
code the covariance matrix themselves. We'll write a function to generate the
Cholesky of the covariance matrix called \texttt{L\_cov\_exp\_quad\_ARD}.
...
The parameters $\rho$ and $\alpha$ are weakly identified, and the ratio of the two 
is better identified and affects the nonlinearity. However it is usual to use
independent priors.  
...
the length scale has little impact on the likelihood. Thus you should use a
prior which quickly declines for the larger length scale values. If the length
scale is so small that the correlation between data points is zero, then
decreasing the length scale further does not change the model. There is
typically no need to restrict the length scale from going to very small values,
but sometimes it can be helpful.  One reasonable choice of priors is a gamma
prior for the length scale, with the scale of the prior determined to be weakly
informative (or more informative if information is available).
...
corresponds to input vector \code{x2}, is declared in the generated quantites
...
\code{y2} by sampling \code{N2} univariate normals with means from 
the last \code{N2} elements of \code{f}.
...
functions {
  vector gp_pred_rng(real[] x_pred,
...

    {
      ...
      matrix[N, N] Sigma;
      Sigma = cov_exp_quad(x_is, alpha, rho);
      for (n in 1:N)
      ...
      k_x_is_x_pred = cov_exp_quad(x_is, x_pred, alpha, rho);
      f_pred_mu = (k_x_is_x_pred' * K_div_y_is); 
      v_pred = mdivide_left_tri_low(L_Sigma, k_x_is_x_pred);
      cov_f_pred = cov_exp_quad(x_pred, alpha, rho) - v_pred' * v_pred;
      diag_delta = diag_matrix(rep_vector(delta,N_pred));

      f_pred = multi_normal_rng(f_pred_mu, cov_f_pred + diag_delta);
    }
    return f_pred;
  }
}
...
\subsection{Predictive Inference in non-Gaussian GPs}

For hyperparameter estimation and predictive inference applications,
the $f_n$ are typically latent parameters (i.e., not observed).
...
Joint hyperparameter estimation and predictive inference are easily
