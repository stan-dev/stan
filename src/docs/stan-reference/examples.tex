...
length-scales. Inverse gamma priors are zero-avoiding because the density is
zero at zero, so the posterior for length-scale will be pushed away from zero,
leading to very little posterior mass on functions that perfectly interpolate
the observed data.
