...
\[
  ...
  \end{aligned}
\]
where $y$ is $n \times T$ matrix where rows are variables and $T$ are observations.
The log-likelihood of this system is calculated using the the Kalman Filter.
If $V$ is diagonal, then a more numerically stable algorithm that processes each observation sequentially can be used.

\subsubsection{Stan Functions}

\begin{description}
%
\fitem{real}{gaussian\_dlm\_log}{vector \farg{y}, matrix
  \farg{F}, matrix \farg{G}, matrix \farg{V}, matrix \farg{W}}
  {The log of the density of the Gaussian Dynamic Linear model with
    observation matrix \farg{y} in which rows are variables and columns are observations, design matrix \farg{F}, transition matrix \farg{G},
    obsrvation covariance matrix \farg{V}, and system covariance matrix \farg{W}.}
%
%
\fitem{real}{gaussian\_dlm\_log}{vector \farg{y}, matrix
  \farg{F}, matrix \farg{G}, vector \farg{V}, matrix \farg{W}}
  {The log of the density of the Gaussian Dynamic Linear model with
    observation matrix \farg{y} in which rows are variables and columns are observations, design matrix \farg{F}, transition matrix \farg{G},
    diagonal of the obsrvation covariance matrix \farg{V}, and system covariance matrix \farg{W}.}
%
\end{description}
