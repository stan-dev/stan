...
If $\eta \in \reals$ and $\phi \in \posreals$, then for $y \in
\nats$,
\[
...
\]
\[
\mbox{With mean } E[Y] = \mu = e^\eta \mbox{ and variance } Var[Y] =
\mu + \frac{\mu^2}{\phi}
\]

\pitem{y}{neg\_2\_binomial}{mu, phi}

\pitem{y}{neg\_2\_binomial\_log}{eta, phi}
...
%
 \fitem{real}{neg\_binomial\_2\_log}{ints \farg{y}, reals
   ...
   mass of \farg{n} given location \farg{mu} and inverse scale
   \farg{phi}.}
%
 \fitem{real}{neg\_binomial\_2\_log\_log}{ints \farg{y}, reals
   \farg{eta}, reals \farg{phi}}{The log negative binomial probability
   mass of \farg{n} given log-location \farg{eta} and inverse scale
   \farg{phi}. This is useful for log-linear negative
   binomial regressions.}
%
\fitem{int}{neg\_binomial\_2\_rng}{real \farg{mu}, real \farg{phi}}{Generate a
  negative binomial variate with location \farg{mu} and inverse scale
  \farg{phi}; may only be used in generated quantities block}
%
\fitem{int}{neg\_binomial\_2\_log\_rng}{real \farg{eta}, real \farg{phi}}{Generate a
  negative binomial variate with log-location \farg{eta} and inverse scale
  \farg{phi}; may only be used in generated quantities block}
\end{description}
...
\refsection{betafun}.  If $\theta = 0$ or $\theta = 1$, then
the log probability is $-\infty$.
...
\fitem{real}{multi\_normal\_log}{vector \farg{y}, vector \farg{mu},
  matrix \farg{Sigma}}{The log of the multivariate normal density of vector \farg{y}
 given location \farg{mu} and covariance matrix \farg{Sigma}}
%
\end{description}
%
\begin{description}
...
\pitem{y}{multi\_norm\_prec}{mu,Omega}
...
\fitem{real}{multi\_normal\_prec\_log}{vector \farg{y}, vector
  \farg{mu}, matrix \farg{Omega}}{The log of the multivariate normal density of vector
...
\fitem{real}{multi\_student\_t\_log}{vector \farg{y}, real \farg{nu},
  vector \farg{mu}, matrix \farg{Sigma}}{The log of the multivariate Student-$t$
  density of vector \farg{y} given degrees of freedom \farg{nu},
  location \farg{mu}, and scale matrix \farg{Sigma}}
%
\end{description}
%
\begin{description}
...
\begin{description}
%
\fitemtwolines{real}{gaussian\_dlm\_obs\_log}{vector \farg{y}, matrix \farg{F},
...
\fitem{vector}{dirichlet\_rng}{vector \farg{alpha}}{Generate a
  Dirichlet variate with prior counts2 (plus one) \farg{alpha}; 
may only be used in generated quantities block}
\end{description}
...
\item for $0 < \eta < 1$, the density has a trough at the identity
  matrix.
\item if $\eta$ were an unknown parameter, the Jeffrey's prior is
  proportional to $\sqrt{\prod_{k=1}^{K-1}\left(
  2\psi_1\left(\eta+\frac{K-k-1}{2}\right) - 
  4\psi_1\left(2\eta+K-k-1 \right)\right)}$, where $\psi_1()$ is the
  trigamma function
\end{itemize}
...
densities in terms of their Cholesky factors, which you should use rather
...
\fitem{real}{wishart\_log}{matrix \farg{W}, real \farg{nu}, matrix
 \farg{Sigma}}{The log of the Wishart density for positive-definite matrix
...
\fitem{real}{inv\_wishart\_log}{matrix \farg{W}, real \farg{nu}, matrix
 \farg{Sigma}}{The log of the inverse Wishart density for positive-definite matrix
 \farg{W} given degrees of freedom \farg{nu} and scale matrix
 \farg{Sigma}}
%
\end{description}
...
\pitem{W}{lkj\_cov\_log}{mu,sigma,eta}
