...
%
  \fitem{real}{exp\_mod\_normal\_cdf}{reals \farg{y}, reals \farg{mu},
    reals \farg{sigma}, reals \farg{lambda}}{The cumulative
    exponentially modified normal distribution of \farg{y} given
    location \farg{mu}, scale \farg{sigma}, and shape \farg{lambda}}
\end{description}
%
\begin{description}
...
\fitem{real}{skew\_normal\_cdf}{reals \farg{y}, reals \farg{mu}, reals
  \farg{sigma}, reals \farg{alpha}}{The cumulative skew normal
  distribution of \farg{y} given location \farg{mu}, scale
...
\section{Guassian Dynamic Linear Models}
...
are observations. The log-likelihood of this system is calculated
using the the Kalman Filter. If $V$ is diagonal, then a more efficient
algorithm which sequentially processes observations can be used
\citep[Sec~6.4]{DurbinKoopman:2001}.

\subsubsection{Stan Functions}

\begin{description}
%
\fitem{real}{gaussian\_dlm\_log}{vector \farg{y}, matrix \farg{F},
...
%
\fitem{real}{gaussian\_dlm\_log}{vector \farg{y}, matrix \farg{F},
  matrix \farg{G}, matrix \farg{V}, matrix \farg{W}}{The log of the
  density of the Gaussian Dynamic Linear model with observation matrix
  \farg{y} in which rows are variables and columns are observations,
  design matrix \farg{F}, transition matrix \farg{G}, observation
  covariance matrix \farg{V}, system covariance matrix \farg{W}. The
  initial state is distributed with mean 0 and a diagonal covariance
  matrix in which the diagonal elements are 1e7.}
%
%
\fitem{real}{gaussian\_dlm\_log}{vector \farg{y}, matrix \farg{F},
  matrix \farg{G}, vector \farg{V}, matrix \farg{W}}{The log of the density of the Gaussian Dynamic
  Linear model with observation matrix \farg{y} in which rows are
  variables and columns are observations, design matrix \farg{F},
  transition matrix \farg{G}, observation covariance matrix with
  diagonal \farg{V}, system covariance matrix \farg{W}. The initial
  state is distributed with mean 0 and a diagonal covariance matrix
  in which the diagonal elements are 1e7.}
%
%
