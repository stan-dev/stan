...
\section{Negative Binomial Distribution, Log Parameterization}

Stan also provides a parameterization of the Negative Binomial
Distribution using the log-location and inverse dispersion as a
parameters.  This is useful for log-linear Negative
...
\nats$,
\[
\distro{NegativeBinomial}(y|\eta,\phi)
 =
\binom{y + \phi - 1}{y}
\,
\left( \frac{e^\eta}{e^\eta+\phi} \right)^{\!y}
\,
\left( \frac{\phi}{e^\eta+\phi} \right)^{\!\phi} \!.
\]
\[
\mbox{With mean } E[Y] = e^\eta \mbox{ and variance } Var[Y] = \left( \frac{e^\eta(\phi + e^\eta)}{\phi}  \right)
\]


\pitem{y}{neg\_binomial\_log}{eta, phi}

\subsubsection{Stan Functions}

\begin{description}
%
 \fitem{real}{neg\_binomial\_log\_log}{ints \farg{y}, reals
   \farg{eta}, reals \farg{phi}}{The log negative binomial probability
   mass of \farg{n} given log-location \farg{eta} and inverse scale \farg{phi}}
%
\fitem{int}{neg\_binomial\_rng}{real \farg{eta}, real \farg{phi}}{Generate a
