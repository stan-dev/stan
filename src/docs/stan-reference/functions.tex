...
\begin{description}
  ...
  \fitem{real}{inv\_logit}{real \farg{y}}{
    ...
    \[
    \mbox{\code{inv\_logit}}(y) = \frac{1}{1 + \exp(-y)}
    \]}
  %
  \fitem{real}{inv\_cloglog}{real \farg{y}}{
    ...
    applied to \farg{y}.
    \[
    \mbox{\code{inv\_cloglog}}(y) = 1 - \exp \! \left( - \exp(y) \right)
    \]}
  %
\end{description}
...
\begin{description}
  ...
  \fitemindexsort{real}{Phi}{real \farg{x}}{
    ...
    \]}{Phi}{phi}
  %
  \fitemindexsort{real}{inv\_Phi}{real \farg{p}}{
    Returns the standard normal inverse cumulative distribution function of 
    \farg{p}, otherwise known as the quantile function, which satisfies
    \[
...
\begin{description}
  ...
  \fitem{real}{log1p\_exp}{real \farg{x}}{ 
    ...
    \]}
  %
  \fitem{real}{log1m\_exp}{real \farg{x}}{ Returns the natural
    logarithm of one minus the natural exponentiation of \farg{x}.
    \[
...
matrix, returning a result of the same shape as the argument.
...
\chapter{Sparse Matrix Operations}\label{sparse.chapter}
...
it to a sparse matrix before multiplying it by a vector.
...
\end{bmatrix} \! ,
\]
and an array of integer indices indicating where a given row's values
start,
\[
u(A) = 
\begin{bmatrix}
...
$1$, and $3$).  The values $(w(A), \, v(A), \, u(A))$ are sufficient for
reconstructing $A$.
...
%
  \fitem{int[]}{csr\_extract\_v}{matrix \farg{a}}{Return colum
...
%
\begin{description}
\fitem{matrix}{csr\_to\_dense\_matrix}{int \farg{m}, int \farg{n}, int
  \farg{w}, int \farg{v}, int \farg{u}}{Return dense $m \times n$
  matrix with specified values, column indices, and row starting
  indices;  see \refsection{CSR}.}
\end{description}
...
$A$ and a dense vector $b$ to produce a dense vector $A\,b$.  This
allows dense row vector $b$ times a sparse matrices $A$ to be coded
using transposition as
\[
v \, A = (A^{\top} \, v^{\top})^{\top},
\]
...
\begin{description}
\fitemtwolines{vector}{csr\_matrix\_times\_vector}{int \farg{m}, int \farg{n},
vector \farg{w}, int[] \farg{v}}{int[] \farg{u}, vector \farg{b}}{
Multiply the matrix represented by (\farg{w}, \farg{v}, \farg{u}) by
the vector \farg{b}; see \refsection{CSR}.}
\end{description}
