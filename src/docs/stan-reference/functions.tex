...
Currently, Stan's logical abilities are very limited.  Like C++, BUGS,
and R, Stan uses 0 to encode false, and 1 to encode true.  The only
conditional-like operator is \Verb|if_else()| and the only logical
output operator is \Verb|step()|.

Until Stan gets comparison and boolean operations, these must be
hacked with various flavors of arithmetic.  To test if a variable
\code{x} is greater than or equal to \code{y}, use \code{step(x - y)}.
To test if \code{x} equals \code{y}, use \code{step(x-y) * step(y-x)};
to test for zero, use \code{step(x) * step(-x)}.  

For conjunction, use multiplication; for disjunction, use use addition
of absolute values.  Negation is the same as testing for zero.
...
\begin{description}
  \fitem{matrix}{diag\_pre\_multiply}{vector \farg{v}, matrix
    ...
    the vector \code{v} and the matrix \code{m}, i.e.,
    \code{diag\_matrix(v) * m}.}
%
  \fitem{matrix}{diag\_pre\_multiply}{row\_vector rv, matrix m}{Return
    the product of the diagonal matrix formed from the vector
    \code{rv} and the matrix \code{m}, i.e., \code{diag\_matrix(rv) *
      m}.}
%
  \fitem{matrix}{diag\_post\_multiply}{matrix m, vector v}{Return the
    product of the matrix \code{m} and the diagonal matrix formed from
    the vector \code{v}, i.e., \code{m * diag\_matrix(v)}.}
%
\fitem{matrix}{diag\_post\_multiply}{matrix m, row\_vector rv}{Return the
  product of the matrix \code{m} and the diagonal matrix fromed from
  the the row vector \code{rv}, i.e., \code{m * diag\_matrix(rv)}.}
\end{description}
