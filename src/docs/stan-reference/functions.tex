...
Arrays of integers, reals, vectors, row\_vectors, and matrices can
be concatenated together using the following operations.

\begin{description}
\fitem{int[]}{append\_array}{int[] \farg{x}, int[] \farg{y}}{
  Return the concatenation of two arrays of integers in the order
  of the arguments. }
%
\fitem{real[]}{append\_array}{real[] \farg{x}, real[] \farg{y}}{
  Return the concatenation of two arrays of reals in the order
  of the arguments. }
%
\fitem{vector[]}{append\_array}{vector[] \farg{x}, vector[]
  \farg{y}}{ Return the concatenation of two arrays of vectors
  in the order of the arguments. The vectors must all have the same
  number of rows.}
%
\fitem{row\_vector[]}{append\_array}{row\_vector[] \farg{x}, row\_vector[]
  \farg{y}}{ Return the concatenation of two arrays of row\_vectors
  in the order of the arguments. The row\_vectors must all have the same
  number of columns.}
%
\fitem{matrix[]}{append\_array}{matrix[] \farg{x}, matrix[]
  \farg{y}}{ Return the concatenation of two arrays of matrices
  in the order of the arguments. The matrices must all have the same
  dimensions.}
\end{description}
%
...
The compound arithmetic and assignment operations are coded directly
as statements, as described in
\refsection{compound-arithmetic-assignment}.  They allow statements of
the form 
%
\begin{stancode}
x = x op y;
...
\chapter{Algebraic Equation Solver}
\label{functions-algebraic-solver.chapter}

Stan provides a built-in algebraic equation solver. Although it 
looks like other function applications, the algebraic solver is
special in two ways.
...
solvers (see section~\ref{functions-ode-solver.chapter}). Ordinary Stan
...
\section{Specifying an Algebraic Equation as a Function}
\label{functions-algebra-function.section}
...
\section{Call to the Algebraic Solver}
...
\subsection{Arguments to the Algebraic Solver}

The arguments to the algebraic solver are as follows:
%
\begin{enumerate}
\item \farg{algebra\_system}: function literal referring to a function
  specifying the system of algebraic equations with signature described
  in \refsection{functions-algebra-function}:
  \begin{quote}
...
\item \farg{y\_guess}: initia guess for the solution, type \code{vector},
\item \farg{theta}: parameters, type \code{vector},
\item \farg{x\_r}: real data, type \code{real[]}, data only, and
\item \farg{x\_i}: parameters, type \code{int[]}, data only.
\end{enumerate}
...
The algebraic solver uses the Powell hybrid method \citep{Powell:1970}. The
