...
The exponentiated quadratic kernel relates the 
covariance between $f(x_i) \in \reals$ and $f(x_j) \in \reals$
to a function of the squared Euclidian distance between 
$x_i \in \reals^D$ and $x_j \in \reals^D$:
...
with $\sigma$ and $l$ constrained to be positive.

The exponentiated quadratic covariance function builds the covariance matrix, $K \in
\reals^{N \times N}$ for $f(x_1), \dots, f(x_N)$, which is symmetric
and positive semi definite by construction. There is a second variant
of the exponentiated quadratic covariance function that builds
the $K \in \reals^{N \times M}$ covariance matrix using the exponentiated
quadratic kernel for $f(x_{1,1}), \dots, f(x_{1,N})$ and $f(x_{2,1}), \dots,
f(x_{2,M})$, where $x_{1,i} \in \reals^D$ and $x_{2,i} \in \reals^D$.
