...
\begin{description}
  ...
  \fitem{real}{if\_else}{int \farg{cond}, real \farg{x}, real \farg{y}}{
    ...
    \end{cases}
    \]
    {\bfseries Warning:} This function should not be used.  Rather,
    \code{if\_else(a,~b,~c)} should be replaced with the conditional
    operator \code{a~?~b~:~c}.  The conditional operator also allows
    more flexiblity in the types of \code{b} and \code{c}.  See
    \refsection{conditional-operator} for full details of the
...
which can be used with the conditional operator
for efficient evaluation of the 
log probability function and gradient:
%
\begin{stancode}
y = a>b?x1:x2;
\end{stancode}
