...
%
\begin{stancode}
real[] algebra_system(real[] unknown, real[] theta,
                      real[] x_r, int[] x_i)
\end{stancode}
%
...
\begin{itemize}
\item \farg{unknown}, the unknowns we wish to solve for
...
\begin{description}
%
   \fitemthreelines{real[~]}{algebra\_solver}%
                  {function \farg{algebra\_system}}%
                  {real[] \farg{initial\_guess}}%
                  {real \farg{theta}, real[] \farg{x\_r}, int[] \farg{x\_i}}%
                  {Solves the algebraic system, given an initial guess,
                    using the Powell Hybrid algorithm with the unsupported
                    implementation from Eigen.}
                   
\fitemfourlines{real[~]}{algebra\_solver}%
                  {function \farg{algebra\_system}}%
                  {real[] \farg{initial\_guess}}%
                  {real \farg{theta}, real[] \farg{x\_r}, int[] \farg{x\_i}}%
                  ...
                  {Solves the algebraic system, given an initial guess,
                    using the Powell Hybrid algorithm with the unsupported
                    implementation from Eigen with additional control
...
%
\begin{enumerate}
\item \farg{algebra\_system}: function literal refering to a function
  ...
  \end{quote}
  The arguments represent (1) unknown, (2) parameters, (3) real data, and
...
\item \farg{initial\_guess}: initia guess for the solution, type \code{real[]},
...
The return value for the algebraic solver is an array of type \code{real[]}, with values
which, when plugged in as the \code{unknowns} make the algebraic function go to 0.

\subsection{Sizes and Parallel Arrays}
Certain sizes must match. In particular, the following groups are of the same size:
%
\begin{enumerate}
  \item unknowns passed into the system function, algebraic function returned by the system
  function, initial guess passed into the solver, and return value of the solver,
  \item parameters, real data, and integer data passed to the solver will be passed to the system
  function.
\end{enumerate}
...
in \refchapter{algebra-solver.chapter}.
