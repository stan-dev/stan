...
The cumulative unit normal density function of \farg{x}, defined as $\Phi(x) \equiv \frac{1}{\sqrt{2\pi}} \int_{0}^{x} e^{-t^2/2} dt$.
%
\[
\textrm{Phi}(x) = 
\begin{cases}
  \Phi(x) & : -37.5 \leq x \leq 8.25 \\
  0 & : x < -37.5 \\
  1 & : x > 8.25 \\
  \textrm{throws exception} & : x = \textrm{NaN}
\end{cases}
\]
%
\code{Phi(\farg{x})} derivatives will underflow to 0 below
