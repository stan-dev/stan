...
\code{real[]}) of size 5 with all values set to 1.0; if \code{v} is a
vector, then \code{rep\_array(v,27)} is a size 27 array consisting of
27 copies of the vector \code{v}.  They are copies in the sense that
...
\subsection{Exponential Normal Distribution}

if $\mu \in \reals$, $\sigma \in \posreals$, and $\lambda \in \posreals$, then for $y \in \reals$,
\[
\distro{ExpNormal}(y|\mu,\sigma,\lambda)
=
\frac{\lambda}{\sqrt{\pi}}
\
\exp \left(\frac{\lambda}{2} \left(2\mu + \lambda \sigma^2 - 2y\right)\right)
\
\int_{\left(\frac{\mu + \lambda\sigma^2 - y}{\sqrt{2}\sigma}\right)}^\infty
\exp\left(-t^2\right)dt
.
\]

\begin{description}
  \fitem{real}{exp\_normal\_log}{reals \farg{y}, reals \farg{mu}, reals
    \farg{sigma}, reals \farg{lambda}}{The log of the exponential normal density of \farg{y} given location \farg{mu}, scale \farg{sigma}, and shape \farg{lambda}}
%
\fitem{real}{exp\_normal\_cdf}{reals \farg{y}, reals \farg{mu}, reals
  \farg{sigma}, reals \farg{lambda}}{The cumulative skew normal distribution of \farg{y}
...
\exp\left( - \, \frac{1}{2}
    ...
    \right)
\
\int_{-\infty}^{\alpha \left( \frac{y - \mu}{\sigma}\right)}
...
\begin{description}
  \fitem{real}{gumbel\_log}{reals \farg{y}, reals \farg{mu}, reals
    \farg{beta}}{The log of the exponential normal density of \farg{y} given location \farg{mu} and scale \farg{beta}}
%
\fitem{real}{gumbel\_cdf}{reals \farg{y}, reals \farg{mu}, reals
  \farg{beta}}{The cumulative skew normal distribution of \farg{y}
