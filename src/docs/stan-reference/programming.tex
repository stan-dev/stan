...
A function argument which is a real-valued or container of real values
...
built-in ordinary differential equation (ODE) solvers which have
strictly specified signatures where some arguments of are data only
expressions. (See \refchapter{ode-solver} and
\refchapter{functions-ode-solver} for details.)
Arguments to the embedding function which will be used as a
data-only argument to an ODE solver function, either directly
or indirectly, should have the \code{data} prefix qualifier.
