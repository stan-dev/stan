...
bounds.  With \code{N} declared as type \code{int[1,]}, there must be
...
also provides the cumulative distribution function \code{normal\_p},
defined byma
\[
...
values \code{1 - Phi(eta - c(d))} once and storing them for re-use.
...
not as bad as it would be with a raw call to \code{new}.}
%
...
\footnote{\Stan cannot exploit the expression templates in the Eigen
  ...
  would be required for \code{beta' * x[n]}.}
%
...
encoding either the \code{x} or \code{n} as row vectors, as in the
following example.
%
\begin{quote}
\begin{Verbatim} 
...
The probability function documentation in \refpart{built-in-functions} indicates which
...
are the same dimensionality.  In the example above, \code{y} is a
vector of size \code{N}, \code{x * beta} is a vector of size \code{N},
and \code{1} is a scalar.
