...
\, \distro{Exp}(l|r_l)
\, \distro{Unif}(s|1,T)
\, \prod_{t=1}^T \distro{Pois}(D_t | t < s \ ? \ e \ : \ l),
\end{array}
\]
with names of distributions shortened.  The required marginalization
involves noting
%
\[
...
\\[3pt]
\mbox{ } \ \ = \
\mbox{log\_sum\_exp}_{s=1}^T \log \distro{Unif}(s | 1,T) 
+ \sum_{t=1}^T \log \distro{Pois}(D_t | t < s \ ? \ e \ : \ l),
\end{array}
\]
%
where the notation
...
\]
%
allows the model to be coded directly using the function of the same
name in Stan, which provides an arithmetically robust approach to the
arithmetic required to marginalize discrete parameters.
...
\reffigure{change-point-posterior.figure}.  
%
\begin{figure}
\begin{center}
...
\end{quote}
%
A posterior histogram of draws for $s$ is shown on the right side of \reffigure{change-point-posterior.figure}
...
posterior.  These expectations are computed in the variable \code{lp}
at every iteration, so even their correlation with the rates in the
