...
Most of these measurements are taken with some error.  For instance,
measuring height in a doctor's office is rarely accurate to within
more than one inch and weight measurement is even more variable given
its variability.  Other medical measurements are also prone to
measurement error, such as anti-tumor cell counts, biopsy results,
human judgments as to stage of cancer based on image data.  

When the measurement error is small relative to the quantity being
measured, its affects on a model are usually small.  When measurement
error is large relative to the quantity being measured, or when very
precise relations can be estimated being measured quantities used as
predictors and outcomes, it is useful to introduce an explicit model
...
\citep{Clayton:1992, RichardsonGilks:1993}.  The measurement is then
modeled based on the true value and a measurement error model.

\subsection{Regression without Measurement Error}

For example, consider a simple linear regression model where the
observed data for $N$ cases includes a predictor $x_n$ and outcome
$y_n$.  In Stan, a linear regression for $y$ based on $x$ with a slope
and intercept is modeled as follows.
%
\begin{quote}
\begin{Verbatim}
data {
...
\subsection{Adding Measurement Error}

Now suppose that the true values of the predictors $x_n$ are not
known, but for each $n$, a measurement $u_n$ of $x_n$ is available.
If the error in measurement can be modeled, the measured value $u_n$
can be modeled in terms of the true value $x_n$ plus measurement
...
\end{quote}
%
The regression and regression coefficients and overall noise model
are the same as before, but now \code{x} is delcared as a parameter
rather than as data.  Instead, \code{x\_meas} is the data with known
measurement noise \code{tau}.  The model then specifies that the
...
measurement error models may be specified.  A simple generalization of
the above model is to allow the measurement noise term \code{tau} to
vary with item.  This only requires changing its declaration in the
data block to
\begin{quote}
\begin{Verbatim}
...
Meta-analysis models combine the results of multiple (typically
previously carried out) statistical models into a single model.  The
Bayesian framework is particular convenient for meta-analysis, because
each previous study in the meta-analysis can be treated as providing a
noisy measurement of some underlying quantity of interest.  The model
then follows directly from two components, a prior on the underlying
quantities of interest and measurement-error style model for each of
the studies being analyzed.

\subsection{Eight Schools Model}

The following model is not a meta-analysis per se, but has many of the
same components.  What is observed is measured difference in test
performance based on a pre-test tutoring treatment at eight different
schools.  The available data is only provided in the form of a sample
mean treatment effect $y_j$ and standard error $\sigma_j$ for each of
$J$ schools.  The meta-analysis model combines the result of the
studies in each school by modeling an overall mean treatment effect
$\mu$ across schools.
%
\begin{quote}
\begin{Verbatim}
data {
  ...
  real<lower=0> sigma[J];  // std err of effect estimate
}
parameters {
  real mu;
  real theta[J];
...
\end{quote}
%
The variable $\theta_j$ is introduced to model the true treatment
effect at school $j$, with $\tau$ modeling the variability among the
schools.  The observed treatment effect $y_j$ is modeled as having
normal measurement error $\sigma$ around the true effect $\theta_j$.

Meta-anlyses typically focus on a treatment effect such as the effect
of coaching ($\mu$) in the eight schools model.  Unlike the eight
schools example, the studies are not typically homogeneous.
...
$K$-means and latent Dirichlet allocation (LDA).  These models are
...
with a fixed covariance, soft $K$-means is just a multivariate normal
mixture model.
...
the log of the unnormalized cluster assignment probabilities.  To
normalize to probabilities, convert \code{soft\_z} back to a linear
scale by exponentiating, then to a simplex by normalizing.  For
instance, in R, the statement
%
\begin{quote}
\begin{Verbatim}
soft_z_norm[n] <- exp(soft_z[n]) / sum(exp(soft_z[n]));
\end{Verbatim}
\end{quote}
%
defines a properly normalized simplex \code{soft\_z\_norm}.
...
covariance matrix (i.e., in a Riemann manifold). 
...
situation, a hierarchical prior may be used to regularize.



\section{The Difficulty of Bayesian Inference for Clustering}

Two problems make it pretty much impossible to do proper, full
Bayesian inference in clustering models, the lack of parameter
identifiability and the multimodality of the posterior.

\subsection{Non-Identifiability}

A fundamental problem with mixture models is that the cluster
assignments are not identified.  This is easy to see because permuting
the cluster mean vectors \code{mu} leads to a model with identical
likelihoods.  For instance, permuting the first two indexes in
\code{mu} and the first two indexes in each \code{soft\_z[n]} leads to
an identical likelihood (and prior).  

The lack of identifiability means that the parameters cannot be
compared across multiple Markov chains.  Since the only parameter in
soft $K$-means is not identified, this leads to a problem in running
multiple chains and monitoring convergence.  So typically only a
single chain is run.  But even within a single chain, the indexes can
swap if the chain is long enough or if the data is not cleanly
separated.  

\subsection{Multimodality}

The other problem with clustering models is that their posteriors are
highly multimodal.  This multimodality does not simply arise from the
non-identifiability, but is an inherent problem in clustering that
leads to many local maxima of the posterior.  
...
The advice often given in this situation is to try many different
initializations and select the result with the highest overall
probability.
...
w_{m,N[m]} \in 1{:}V$.  Despite the ordered indexing of words ina
...
The parameters are typically given symmetric Dirichlet priors.
...
looks as follows, with the word arrays layed out in a column and each
...
of words in all the documents, the word value array \code{w}, and the
...
function.  The category assignment is often summed out of the
likelihood to compute the marginal probability of one or more documents.
...
to stabilize the numerical calculation and return a result on the log scale.
%
\begin{quote}
\begin{Verbatim}
model {
...
\]
%
Given \code{gamma}, the posterior probability that document
$m$ is assigned category $k$ is
\[
\exp \left( 
\mbox{\code{gamma}}[m,k]  
- \log \sum_{k=1}^K \exp \left( \code{gamma}[m,k] \right)
\right).
\]
%
...
can be implemented in Stan by comining the models for labeled and
unlabeled data.  The estimands include both the model parameters and
the labels for the unlabeled data.  The model is essentially a missing
data model assuming the unknown category labels are missing completely
at random; see \citep{GelmanCarlinSternRubin:2003,GelmanHill:2007} for
more information on missing data imputation.  The model is also an
instance of semisupervised learning because the unlabeled data
...
words, as well as the hyperparameters are shared and only declined
...
\subsection{Cutting Variables out of Estimation}
...
behavior can be implemented by simply removing the contribution of
\code{gamma} to the log probability accumulator \code{lp\_\_}.  The
value of \code{gamma} will still be calculated, but it will no longer
have any affect on the log probabilty and hence no effect on the
parameter estimation.
%
\footnote{BUGS provides a function \code{cut} that can be applied to
  variables in the directed graphical model to remove their
  contribution to the overall log probability function.}
%
At this point, a variable like \code{gamma} can be moved to the
generated quantities block, which will be more efficient.

Partially cut inferences can be defined by including the variable as a
transformed parameter, but only including some of its contribution to
the log probability function.  Anything that does not contribute to
the accumulator \code{lp\_\_} will not affect estimation.
...
clustering model.  Using the typical topic/document terminology common
in discussions of LDA, each document is modeled as having a mixture
of topics, with each word drawn from a topic based on the mixing
proportions.  

\subsection{The LDA Model}

The basic document generation process generates each document
independently.  For document $m$, the first step is to draw a topic
distribtuion simplex $\theta_m$ over the $K$ topics,
%
\[
...
$z_{m,n} \in 1{:}K$ is drawn for the word,
\[
...
\]
%
where $\beta$ is a $V$-vector of positive values.
...
\end{eqnarray*}
%
The inner word-probability term is marginalized by summing out the
topic assignments,
\begin{eqnarray*}
p(w_{m,n}|\theta_m,\phi)
& = &
...
\end{eqnarray*}
%
Putting this back together and converting to the log scale provides a
...
series of $N$ real input values $x_1,\ldots,x_N \in \reals^D$ paired
with outputs $y_1,\ldots,y_N \in \reals$.  The defining feature of
Gaussian processes is that the probability of a finite number of
outputs $y$ conditioned on their inputs $x$ is Gaussian,
\[
...
spaced over some interval in order to plot sample draws from a
Gaussian process in that range.
...
Because the hyperparameters are required to be positive and are all
essentially variance parameters, weakly informative half-Cauchy priors
...
the half-Cauchy priors for the hyperparameters and the multivariate
...
hyperparameter inference in his model \citep{Neal:1997}, and the Stan
...
parameter $\rho_d$ for each dimension,
\[
k(x)_{i,j} = \eta^2 \exp 
\left(
...
hierarchically, for instance taking $\rho_d \sim
\distro{HalfCauchy}(\nu,tau)$, with priors on $\nu$ and $\tau$ such as
$\nu \sim \distro{HalfCauchy}(0,5)$ and $\tau \sim \distro{HalfCauchy}(0,5)$.
...
ingredients, sampling the predictive quantity $\tilde{y}$ is
straightforward, as shown in the Cholesky example above.
...
above.  This cannot be done directly in Stan because it does not (yet)
...
Unfortunately, they can not be easily marginalized out analytically,
...
transformed parameters block.  Ano If their values are not of interest,
