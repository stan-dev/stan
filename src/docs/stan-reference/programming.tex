...
time $t$ and $\delta_t$ represents the shock on volatility.
\[
...
\]
%
Rearranging the first line, $\epsilon_t = y_t \exp(-h_t / 2)$ and
combining it with the sampling of $\epsilon_t$ produces the 
...
\]
This formulation of the model can be directly encoded with the
following Stan model, which is available in the file
...
\end{quote}
%
The model above adds priors for the parameters $\phi$, $\sigma$, and
$\mu$.  Note that it does not explicitly calculate the shock terms
$\epsilon_t$ and $\delta_t$.

The posterior of stochastic volatility models such as these have high
posterior variance and posterior correlation.  For instance,
simulating 500 data points from the above model with $\mu = -1.02$,
$\phi = 0.95$, and $\sigma = 0.25$ leads to 95\% posterior intervals
for $\mu$ of $(-1.23, -0.54)$, for $\phi$ of $(0.82,0.98 )$ and for
$\sigma$ of $(0.16,0.38)$.  Using a non-diagonal mass matrix provides
faster convergence and more effective samples than a diagonal mass
matrix, but will not scale to large values of $T$.  
...
\end{quote}
%
Although the original model can take hundreds and sometimes thousands of
iterations to converge, the reparameterized model reliably converges
in tens of iterations.  Mixing is also dramatically improved, which
results in higher effective sample sizes, particularly for short
chains; with longer changes, the periodic swings mix and the effect of
slower mixing is to some degree mitigated.  Finally, each iteration
