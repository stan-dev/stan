...
outcome \code{y[n]}.  The slope and intercept parameters are
\code{beta} and \code{alpha}.  The model assumes a normally
...
\end{quote}
%
The sofmax function is defined for a $K$-vector $\gamma \in \reals^K$ by
\[
\mbox{softmax}(\gamma) = 
\left(
 \frac{\exp(\gamma_1)}
