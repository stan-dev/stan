...
One application of user-defined functions is to define a systemof
...
data {
  int<lower=1> K;        // system size
  int<lower=0> T;        // number of observations
  int<lower=0> P;        // number of parameters
  int<lower=0> D;        // size of real data
  int<lower=0> D_int;    // size of int data
  
  real t0;               // initial time
  real ts[T];            // observation times
  real x[D];             // real data
  int x_int[D_int];      // integer data
  real y[T,K];           // observations
}
parameters {
  real y0[K];            // initial state
  real theta[P];         // parameters
  real<lower=0> sigma;   // obserbation noise
}
model {
  real y_hat[T,K];
  y_hat <- solve_ode(harm_osc_ode,
...
\end{center}
\caption{\small\itshape Skeletal Stan program for using the ODE solver.}\label{ode-solver.figure}
\end{figure}
