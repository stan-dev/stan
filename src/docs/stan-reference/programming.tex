...
\noindent
Gaussian processes can be encoded in Stan by implementing their
covariance kernel functions and plugging the result into the Gaussian
...
predictive inference.  A more efficient Stan implementation applies a
Cholesky-factor reparameterization of the Gaussian.  

After defining Gaussian processes, this chapter covers the basic
implementations for simulation, hyperparamter estimation, and
posterior predictive inference for univariate regressions,
multivariate regressions, and multivariate logistic regressions.  In
the final chapter, it shows how the parameters underlying these models
may be transformed for improved sampling efficiency.  Gaussian
processes are very general, and by necessity this chapter only touches
on some basic models.  For more information, see \citep{RasmussenWilliams:2006}.
...
\[
y \sim \distro{Normal}(\mu(x),\Sigma(x)),
\] 
where $\mu(x)$ is an $N$-vector and and $\Sigma(x)$ is an $N \times N$
covariance matrix.  The mean function $\mu : \reals^{N \times D}
\rightarrow \reals^{N}$ can be anything, but the kernel
$\Sigma : \reals^{N \times D} \rightarrow \reals^{N \times
  N}$ must produce a positive-definite matrix for any input $x$.

A popular covariance function, which will be used in the
implementations later in this chapter, is
\[
\Sigma_{i,j} 
= \eta^2 
\exp \left(
...
\right)
+ \mbox{\bf I}(i = j) \sigma^2,
\]
where $\eta$, $\rho$, and $\sigma$ are hyperparameters defining the
covariance function and where $\mbox{\bf I}(i=j)$ is the indicator function
with value 1 if $i = j$ and value 0 otherwise.  The only term term in
this kernel involving the inputs $x_i$ and $x_j$ is the squared
Euclidean distance,
\[
d(x_i,x_j) = \sum_{d=1}^D (x_{i,d} - x_{j,d})^2.
\]
A kernel which can be reduced to a function of distance between inputs
is said to be stationary.  Related stationary distributions arise from
changing the distance function, for instance to taxicab ($L_1$ norm)
distance or to a scaled-by-dimension Euclidean distance; other
stationary kernels may be defined using a transform of the distance
other than scaled exponentiated negation.

\section{Simulating from a Gaussian Process}

It is simplest to start with a Stan model that does nothing more than
simulate values of $y$ given values of $x$ as input.  The
Euclidean-distance-based kernel function described in the previous
section will be used with hyperparameters set to $\eta^2 = 1$, $\rho^2
= 1$, and $\sigma^2 = 0.1$, and the mean function $\mu$ will be taken
to be a constant.  The model is then defined by computing the mean and
covariance matrix in a transformed data block functions and then
sampling according to the resulting multivariate Gaussian.  The
following model is included in the Stan distribution in file
...
\end{quote}
%
The input data is just the number of data points \code{N} and the
vector of inputs \code{x}.  Such a model can be used with values of
\code{x} evenly spaced over some interval in order to plot sample
draws from a Gaussian process in that range.
...
\]
where ${\bf 0}$ is an $N$-vector of 0 values and ${\bf 1}$ is an
$N$-vector of 1 values.  Now suppose that the Cholesky decomposition
of $\Sigma(x)$ is the lower-triangular matrix $L$ such that $LL^{\top}
= \Sigma(x)$.  Then the transformed variable $\mu + Lz$ has the
intended target distribution,
\[
\mu + Lz \sim \distro{Normal}(\mu,\Sigma(x)).
\]
