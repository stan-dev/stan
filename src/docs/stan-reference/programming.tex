...
regression uses the cumulative normal density function, which is
typically written as 
\[
\Phi(x) = \int_{-\infty}^x \distro{Normal}(y|0,1) \, dy.
\]
%
The cumulative unit density function $\Phi$ is also known as the
error function and written \code{erf(x)} in programming languages.  
The probit regression model may be coded in \Stan as follows.
%
\begin{quote}
\begin{Verbatim}
