...
\chapter{Multiple and Range Indexing}\label{multi-indexing.chapter}
...
array of integer indexes or range bounds.  This allows hierarchical 1
...
Julian King pointed out that the model may be sped up by noting
\[
\log \distro{PoissonCDF}(0 | \lambda) = \log(1 - exp(-\lambda)).
\]
This allows the truncated Poisson distribution to be coded as
%
\begin{stancode}
if (y[n] > 0) {
  y[n] ~ poisson(lambda);
...
For example, the number of \code{y[n]~==~0} cases can be counted and
stored as a transformed data quantity.  These can then be used to
replace 
%
\begin{stancode}
model {
...
Another speedup can be gained by storing the non-zero count (i.e.,
\code{y[n]~>~0}) cases in a newly defined array and then using a
vectorized Poisson sampling statement and multiplying the increment by
the number of non-zero counts.  This is a little trickier as a
function must be used to compute the number of zero and non-zero cases
here because Stan requires sized declarations.
...
% TODO(carpenter):  leave as is for when we allow overloading
% For example, the function \code{unit\_normal} defined above could be
% named \code{normal}, thus overloading with the built-in function of
% the same name.
...
\code{@throws}.%
%
\footnote{As of Stan 2.8.0, the only way a user-defined
