...
\chapter{Models with Latent Discrete Parameters}
...
\citep[Section 3.1]{PyMC:2014} provide a Poisson model of accident
...
\end{eqnarray*}
%
The ternary operator notation is borrowed from C, where the expression
$(c \ ? \ x_1 \ : \ x_2)$ evalutes to $x_1$ if $c$ is true and $x_2$ otherwise.

\subsection{Marginalizing out the Discrete Parameter}

To code this model in Stan, the discrete parameter $s$ must be
marginalized out to produce a model defining the log of the probability
function $p(e,l,D_t)$.  The full joint probability function is
%
\[
p(e,l,s,D_t)
\mbox{ } \ \ =
...
\end{array}
\]
with names of distributions shortened.  To marginalize, apply the
chain rule
%
\[
p(e,l,D_t) = p(e,l) \, p(D_t | e,l),
\]
%
then reduce
%
...
Because Stan specifies models on the log scale, the final form of the
marginalization needed for the model is
%
\[
\log p(D_t | e,l)
\\[3pt]
...
\code{lp[s]} stores the quantity $\log p(s,C_t|e,l)$.
%
\begin{figure}
\begin{quote}\small
\begin{Verbatim}
...
\end{quote}
\vspace*{-6pt}
\caption{\small\it A change point model in which accident rates
...
in the iteration.  By Bayes's law,
\[ 
p(s|D,e,l) \propto p(s,D|e,l).
\]
In each iteration after convergence, the early and late disaster
rates, $e$ and $l$, are drawn from the posterior $p(e,l|D)$ by MCMC
sampling.  Thus the value of \code{lp} may be used to calculate
$p(s|D,e,l)$ in each iteration, based on on the current values of
$e$ and $l$.  Averaging over iterations provides an unnormalized
probability estimate of the change point being $s$,
%
\[
p(s | D)
\propto
...
sample $m$ for change point $s$.  By averaging over samples, $e$ and
$l$ are themselves marginalized out, and the result has no dependence
on a given iteration's value for $e$ and $l$.  A final normalization
then produces the quantity of interest, the posterior probability of
the change point being $s$ conditioned on
the data $D$,
%
\[
p(s | D)
=
\frac{q(t)}{\sum_{t'=1}^T q(t')}.
\]
%
...
\begin{figure}
\begin{center}
\includegraphics[height=2in]{/Users/carp/temp2/mining-disasters/change-point-posterior.pdf}
\ \ \ \ \
\includegraphics[height=2in]{/Users/carp/temp2/mining-disasters/s-discrete-posterior.pdf}
\end{center}
\vspace*{-12pt}
\caption{\small\it The posterior estimates for the change point.  \
  ...
  the right-hand plot. The posterior mean of $s$ is roughly 1891.}%
...
Although this only plots the posterior marginal probability of $s$,
the quantity \code{lp} is calculated at every iteration, thus allowing
it to be used to calculate covariance with other posterior quantities
of interest.  Such calculations are done in expectation by averaging
over the posterior probabilities.

Alternatively, the discrete samples can be used to calculate
covariance with other parameters.  Although less efficient
statistically (in the sense of requiring more iterations for the same
degree of accuracy), it is conceptually simpler to use the samples.
...
animals, mark them (e.g., by tagging), then release them.  Later,
another round of captures is carried out, with potentially additional
tagging and releasing.  The resulting data may be used to estimate
population size.

The first subsection describes a very simple mark-recapture model that does
not involve any latent discrete parameters.  The following subsection
...
with a recapture probability of $M/N$.  
...
$C$ animals recaptured.  Implementing this lower bound is not strictly
necessary in that values for $N$ below this will cause the binomial
evaluation in the model to reject a sample, but having the lower bound
is more efficient computationally for both MCMC and optimization.
...
\subsection{Collective Cormack-Jolly-Seber Model}
 ...
 This
subsection presents an implementation of the model in terms of counts
for different history profiles for individuals over three capture
times. It assumes exchangeability of the animals in the sense thateach
...
$z_{i,t}$, the Stan models rely on a derivated quantity $\chi_t$ for
...
By defining $\chi$ directly, there is no need for a latent binary
parameter indicating whether an animal is alive at time $t$ or not.
...
The parameters \code{phi[2]} and \code{p[3]} are not identifiable, but
their product \code{beta3} is, so \code{beta3} is defined as a
generated quantity.  The use of unidentified parameters is actually a
...
thus have proper uniform priors, it would be better to replace
parameters \code{phi[2]} and \code{p[3]} with a parameter
\code{beta3}.
...
number of time periods.  Thus the data will consist of the number $T$
of capture events, the number $I$ of individuals, and a boolean flag
$y_{i,t}$ indicating if individual $i$ was observed at time $t$.  In Stan,
%
\begin{quote}
\begin{Verbatim}
data {
...
The individual CJS model is written relying on several function
definitions.  The first two are used in the transformed data block to
comput the first and last time period in which an animal was
captured. 
\begin{quote}
\begin{Verbatim}
functions {
...
\footnote{Both functions return 0 if the individual represented by the
  ...
  would be removed from the data, but the program allows them, but
  they have no impact on the log probability function.}
%
\begin{quote}
\begin{Verbatim}
transformed data {
...
the total number of captures at time \code{t}.  The only readon
\code{n\_captured} is defined as a vector instead of an integer array
is that it is used in an elementwise vector operation in the generated
...
is a function definition for computing the value of \code{chi[t]}, the
...
of $\chi_t$, unrolling the recursion into an iterative algorithm
updating the elemnts of \code{chi} from \code{T} down to 1.

\subsubsection{The Model}

Given a long buildup computation, the model block directly encodes the
CJS model.  All parameters are left with their default uniform priors
and the model simply encodes the log probability of the observations
\code{q} given the parameters \code{p} and \code{phi} as well as the
transformed parameter \code{chi} defined in terms of \code{p} and \code{phi}.
%
\begin{quote}
\begin{Verbatim}
model {
...
\end{quote}
%
The outer loop is over individuals, with the conditional the
conditional skipping individuals \code{i} which are never captured;
this check depends on the convention of returning 0 for \code{first}
if an individual is never captured.

The inner loop for individual \code{i} first increments the Bernoulli
survival, with an outcome of 1 being fixed because the individual must
survive every time between the first and last capture (i.e., no
zombies).  Note that the loop starts after the first capture, because
all information in the CJS model is conditional on the first capture.

Also in the inner loop, the observed capture status \code{y[i,t]} for
...
this model does not identify \code{phi[T-1]} and \code{phi[T]}, but
...
time-series component, in which case \code{p[1]} would be an unknown
intial condition and both \code{phi[T-1]} and \code{p[T]} could be
identified. 

\subsubsection{Population Size Estimates}

The generated quantities block in the previous subsubsection here also
include a calculation of the population at each time \code{t}, which
is estimated as in the simple mark-recapture model as the number of
individuals captured at time \code{t} divided by the probability of
capture at time \code{t}.  This is done with the elementwise division
operation for vectors (\code{./}) in the generated quantities block.
%
\begin{quote}
\begin{Verbatim}
generated quantities {
...
items with categories and diagnostic testing share several
characteristics which allow their statistical properties to modeled
similarly. 

\subsection{Diagnostic Accuracy and Data Coding}

Suppose you have diagnostic tests of varying sensitivity and
specificity.  Sensitivity is the probability a test returns positive
when the patient has the condition and specificity is the probability
that a test returns negative when the patient does not have the
condition.  For example, mammograms and puncture biopsy tests both
test for the presence of breast cancer.  Mammograms have high
...
infection, such as AIDS, in a population.  A test development study
might be interested in the diagnostic accuracy of a new test.  And a
doctor performing tests might be interested in the disease status of a
patient.  
...
negative in overall sentiment, or a dentist looking at an X-ray rates
a patient as having a cavity or not.  In all of these cases, the data
coders play the role of the diagnostic tests and all of the same
estimands are in play --- data coder accuracy and bias, true
categories of items being coded, or the prevalence of various
...
\cite{DawidSkene:1979} introduce a noisy-measruement style model for
...
The data for the model consists of $J$ raters, $I$ items, and $K$
categories to annotate, with $y_{i,j} \in 1{:}K$ being the rating
provided by rater $j$ for item $i$.  In a diagnostic test setting for
a particular condition, the raters are diagnostic procedures and there
are $K=2$ categories, either the condition is present or it isn't.
...
Dawid and Skene provided maximum likelihood estimates.  The parameters
...
in Stan. The joint posterior factors as 
\[
p(y, \theta, \pi) = p(y | \theta,\pi) \, p(\pi) \, p(\theta),
\]
where $p(y | \theta,\pi)$ is derived by marginalizing
%
\[
p(z, y | \theta, \pi)
\ = \
...
\end{array}
\]                            
which can be directly coded as above using \code{log\_sum\_exp}.
