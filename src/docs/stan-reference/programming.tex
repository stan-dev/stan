...
\noindent
Every expression in Stan that denotes a value, such as variables,
constants, function or operator applications, indexing, and so on, has
a type.   The following chapter provides a detailed comparison of the
differences among the container types: arrays, vectors, and matrices.
...
Declarations of data, transformed data, parameter, transformed
parameter, and generated quantities variables may involve constrained
forms of the basic data types.

Constraints provide error checking for data, transformed data,
transformed parameter, and generated quantities types.  

Constraints are critical for variables declared in the
\code{parameters} block.  The constraints on parameters must match the
support on the parameters in the model for sampling or optimization to
...
\begin{Verbatim}
real x[10];
matrix<lower=-1,upper=1>[3,3] m[6,7];
\end{Verbatim}
\end{quote}
delcares \code{x} to be a one-dimensional array of size 10 containing
...
with values falling between -1 and 1.
...
Vectors and matrices are basic types, whereas arrays are collections
of values of some basic types.  Arrays cannot be assigned to vectors
and vice-versa.  Similarly, vectors cannot be assigned to matrices and
vice-versa, even if their dimensions conform.  \refchapter{containers}
provides more information on the distinctions between vectors and
arrays and when each is appropriate.

\subsection{Function Invocation}

Stan functions are only specified for the basic (and array) data type
of their arguments, not for constraints.  Variables with constrained
types may nevertheless be passed to 
...
\caption{\small\it The leftmost column is a list of the unconstrained
  ...
  types \code{cov\_matrix[K]}, \code{corr\_matrix[K]}, and
  \code{cholesky\_factor\_cov[K]} are only assignable to matrices of
...
is used and no error checking is carried out.%
%
\footnote{User-invoked exceptions along with a range of built-in
  validation routines are coming to Stan soon!} 
%
...
\code{@throws}.%
%
\footnote{As of Stan 2.2.0, the only way a user-defined
