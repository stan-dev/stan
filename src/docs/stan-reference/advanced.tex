...
integrals that are not tractable analytically but for which sampling
...
bounds uncertainty in estimates based on number of samples $N$.  Given
dependent samples, the number of independent samples is replaced with
the effective sample size $N_{\mbox{\scriptsize eff}}$, which is the
number of independent samples with the same estimation power as the
...
the resulting Markovian system. This adjustment treats the momentum
term of the Hamiltonian as an auxiliary variable, and the only reason
for rejecting a sample will be discretization error in computing the
Hamiltonian.  In practice, the possibility of rejecting a proposed
update means that one or more duplicate samples may appear in the
chain; this is all accounted for with effective sample size
calculations.

In addition to standard \HMC, \Stan implements an adaptive version of
\HMC, the No-U-Turn Sampler (\NUTS).  \NUTS automatically tunes step
sizes and a diagonal mass matrix during warmup.  During sampling,
\NUTS adapts the number of leapfrog steps (i.e., the simulation time),
using a geometric criterion that stops a trajectory when it begins to
head back in the direction of the initial state.  Once a trajectory is
stopped, \NUTS uses slice sampling to select a state along the
trajectory as the next proposal.

Although \Stan only samples continuous variables, its language qis expressive enough to allow most discrete variables to be
marginalized out.
...
\footnote{For an alternative derivation of the same transform using
  and Jacobians using hyperspherical coordinates, see
  \citep{Betancourt:2010}.}
%
\begin{quote}
...
$x_k$.  Only $K-1$ unconstrained parameters are required, with the
...
\]
%
This definition is recursive, definining $x_k$ in terms of
$x_{1},\ldots,x_{k-1}$.
...
\footnote{An alternative to the transform in this section, which can
 ...
 as a scaled correlation matrix.  If $z$ is a $K \times K$
 correlation matrix and $w$ is a strictly positive $K$-vector of
 scales, then setting $x_{k,k'} = w_k \, w_{k'} \, z_{k,k'}$.}
...
Let $f^{-1}$ be the inverse transform from a $K + {K \choose
  2}$-vector $y$ to the $K \times K$ covariance matrix $x$.  A
$p_X(x)$ is a density defined on $K \times K$ covariance matrices is
...
% \refchapter{stanc}) may be used in contexts other than the \Stan
...
% implements the log probabilty function in a way that allows it to be
