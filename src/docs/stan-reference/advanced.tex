...
Other quantities dependent on the posterior may be computed in the
...
\section{Autocorrelation of Samples}
...
The issue of initial parameter values to start the chain is related to
the issue of determining when the Markov chain has converged to a
stationary sense (i.e., when draws begin to be drawn from the
...
\section{Potential Scale Reduction}
...
\[
\bar{\theta}^{(k)}_{\bullet}
= \frac{1}{N} \sum_{n = 1}^N \theta^{(k)}_n
\ \ \ \ \
\mbox{and}
\ \ \ \ \
\bar{\theta}^{(\bullet)}_{\bullet}
= \frac{1}{K} \, \sum_{k=1}^K \bar{\theta}^{(k)}_{\bullet}.
\]
%
...
Let $N_k$ be the number of samples in chain $k$.  The definitions
above are generalized as follows.
\[
B
= \frac{1}{K-1} \, \sum_{k=1}^K N_k \, (\bar{\theta}^{(\bullet)}_{k} - \bar{\theta}^{(\bullet)}_{\bullet})^2,
\]
%
\[
\bar{\theta}^{(k)}_{\bullet}
...
\]
Or should the second term be the overall mean, as follows?
\[
\bar{\theta}^{(\bullet)}_{\bullet}
= \frac{1}{K} \, \sum_{k=1}^K \bar{\theta}^{(k)}_{\bullet}.
\ \ \ \ \ \mbox{\Large\bf or} \ \ \ \ \
\bar{\theta}^{(k)}_{\bullet}
= \frac{1}{\sum_{k=1}^K N_k} \sum_{k=1}^K \sum_{n=1}^{N_k} \theta^{(k)}_n.
\]
%
\[
W 
= \frac{1}{\sum_{k=1}^K N_k} \, \sum_{k=1}^K N_k \, s_k^2,
\]
Or should that just be as before?
\[ 
W 
= \frac{1}{K} \, \sum_{k=1}^K s_k^2,
\]
