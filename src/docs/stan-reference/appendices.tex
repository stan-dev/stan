...
\end{stancode}
%
with the log probability density function
%
\begin{stancode}
foo_lpdf(y, ...).
\end{stancode}
...
\item[Replacement]
Replace \code{foo\_cdf\_log(...)} with \code{foo\_lcdf(...)}.
\\[4pt]
Replace \code{foo\_ccdf\_log(...)} with \code{foo\_lccdf(...)}.
\end{description}
...
%
\item[Replacement] Use the conditional operator which allows more flexiblity in the types of \code{b} and \code{c}.  See
    \refsection{conditional-operator} for full details of the conditional operator.
    Replace
\begin{stancode}
x = if_else(a,b,c);
\end{stancode}
