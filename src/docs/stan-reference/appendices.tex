...
\begin{quote}
\begin{Verbatim}[fontshape=sl]
> tar -xzf --no-same-owner stan-src.1.m.p.tgz 
\end{Verbatim}
\end{quote}
%
...
specail not-a-number (\code{NaN}) value.  If \code{NaN} is passed to a
...
\end{quote}
%
These can be much harder to read thn the first example Only use this
