...
Download the most recent version of \code{stan-src-1.m.p.tgz}
...
If the Mac OS has not automatically unpacked the \code{.tgz}
file into file \code{stan-src-1.m.p.tar},
double-click the \code{.tgz} file to unpack.

Double click on the \code{.tar} file to unarchive
directory \code{stan-src-1.m.p}.
...
\code{stan-src-1.m.p.tgz}, where \code{m} is the minor version and
...
\end{quote}
%
where \code{<download-dir>} is replaced with the actual path to the diretory.
...
\end{itemize}
%
To verify that the downloaded Stan \code{.tgz} file is there,
...
%
\footnote{Stan still uses the \code{.Rdata} extension in its examples,
  but this is misleading because R itself uses the extension
  \code{.Rdata} for the binary file produced by its \code{save()}
  function, not the ASCII file produced by its \code{dump()} function.}
...
standalone text editors like \code{vi} and \code{emacs}.   

\section{Tools for Emacs}

Stan's emacs mode is defined relative to the directory
\code{<stan-home>} in which Stan was unpacked from source.
%
\begin{quote}
...
Typically, emacs is set up to read a \code{.emacs} file from the
user's home directory.  Add the following two lines to this file
replacing \code{<stan-home>} with the path to where Stan was unpacked.
\begin{quote}
\begin{Verbatim}
...
\code{.emacs} file or in the following preferences file.
%
\begin{quote}
