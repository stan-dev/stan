...
\refchapter{compiling-cpp}.  The model translation step described in
...
with make; see \refchapter{make-models} for details.
...
Use the following call to \code{make}.%
%
%
\begin{quote}
...
optimization level is \code{O=3} (the letter 'O').

\subsection{Windows}

Windows behaves the same way, but executables must be specified
with a \code{.exe} suffix.  For building \stanc, the command is
%
\begin{quote}
\begin{Verbatim}[fontshape=sl]
> make bin/stanc.exe
...
\end{quote}
%
The default compiler options are the same, \code{CC=g++} \code{O=3}.
...
for the parent directory of \code{stanc}.
%
\begin{quote}
\begin{Verbatim}[fontshape=sl]
> bin\stanc --name=binary_normal --o=binorm.cpp binormal.stan 
...
\end{quote}
%
The \code{.exe} suffix on \code{stanc.exe} is optional in Windows.
...
be automated through make; see \refchapter{make-models} for details.
...
\item[\code{-Ilib/eigen\_3.1.0}] specifies the directory for
  the Eigen library,
\item[\code{-Ilib/boost\_1.50.0}] specifies the directory for the 
  Boost library,
...
\chapter{Translating and Compiling through \code{make}}\label{make-models.chapter}

Instead of calling the \Cpp compiler directly, the \code{make} tool
...
targets invoked by \code{make} are broken down step-by-step.

\section{Translating and Compiling Test Models}

There are a number of test models in the directory \code{src/models}
of the distribution.  
...
\end{quote}
%
This will translate the model \code{normal.stan} to a \Cpp file and
...
automatically build the \code{bin/stanc} compiler and
the \code{bin/libstan.a} library.

\subsection{Windows}

For Windows, the make target needs to specify the \code{.exe} suffix.
%
\begin{quote}
\begin{Verbatim}[fontshape=sl]
...
\section{Translating and Compiling User-Defined Models}
