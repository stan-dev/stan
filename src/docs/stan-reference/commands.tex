...
Arguments grouped within supercategories are not ordered with respect
...
\end{quote}
%
provides the top-level options for the \code{sample} method, whereas
%
\begin{quote}
\begin{Verbatim}[fontshape=sl]
...
\\
for arguments with values;  lists value type, legal values, and default value
%
\item \code{\bfseries arg}
\\
for categorical arguments activating subarguments; lists valid subarguments
%
\item \code{\farg{value}}
\\
for values; describes effect of selecting the value
%
\item \code{\farg{\bfseries avalue}}
\\
for values that activate subarguments;  lists valid subarguments
\end{itemize}
...
The value of the argument \code{engine} controls the kind of HMC
sampling, either \code{static} for vanilla HMC or \code{nuts} for the
...
\end{description}
%
The next three options pick out which form of mass matrix is
estimated, and all three options are valid for both basic HMC and
NUTS.
%
\begin{description}
...
\end{description}
%
The remaining arguments apply to all forms of HMC.
%
\begin{description}
%
...
%
\end{description}
The following options are for the BFGS optimizer.
%
\begin{description}
...
require more computation, specifically for $m$ samples of a model with
$n$ parameters, the dense matrix requires $\mathcal{O}(n^3 \log(m) +
n^2 \, m)$ operations, whereas diagonal mass matrices require only
$\mathcal{O}(n \, m)$.  Furthermore, dense mass matrices are difficult
to estimate, because there are $\mathcal{O}(n^2)$ cells with complex
interdependence.
%
\begin{figure}
\begin{quote}
