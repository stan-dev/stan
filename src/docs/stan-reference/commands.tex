...
file \code{my\_data}, both in the current working directory.  Invoking
the Stan executable is done differently on Windows versus Linux and
Mac OS.

To generate samples from a data set using the default settings, use
%
...
\code{my\_data}, runs warmup tuning for 1000 iterations (the valaues
...
To sample four chains using a Bash shell on Mac or Linux, execute
%
\begin{quote}
\begin{Verbatim}[fontshape=sl]
> for i in {1..4}                            \
...
prompt. The ampersand (\code{\&}) at the end pushes each process into
the background, so that the loop can continue without waiting for the
previous chain to finish. This example requires several command-line
options that are explained in further detail in the next subsection.
...
comma-separated values file.  The command is the same on Windows, Mac
OS and Linux.
%
\begin{quote}
\begin{Verbatim}[fontshape=sl]
...
Windows.  A sequence of several such commands, perhaps interpspersed
with \code{bin/print} can be executed from a single script file.
...
model or initial states (or a bug in Stan).
...
which define self-contained categories of arguments.  

\subsection{Unordered}

Categories and arguments within each category are unordered.  For
example, the following four commands all define the same
configuration.%
\footnote{The backslash (\code{\textbackslash}) is used at the end of a line in a command to
indicat that it continues on the next line.  The indentation to
indicate the structure of the command is for pedagogical purposes
only; the same result would be obtained writing each command on one
line with single spaces separating the elements.}
%
\begin{quote}
\begin{Verbatim}[fontshape=sl]
...
\code{diagnostic\_file} and \code{file} go with \code{output}.  No
...
%
\begin{itemize}
\item top-level method indicators (e.g., \code{sample}),
\item indicators for following subarguments (e.g., \code{random}
  and \code{output}), or
\item values of configuration parameters.
\end{itemize}
%
The arguments \code{random} and \code{output} in the above examples
serve as indicators for the following subarguments (e.g.,
\code{nesterov} or \code{bfgs} used without a key to indicate which
optimizer to use).
%
A visual diagram of the available categorical arguments is shown in
\reffigure{hierarchy}.  The mutual exclusivity of these arguments is
shown in \reffigure{configuration}.  Specifying conflicting arguments
...
\put(50, 370) { \makebox(500, 10)[l]{file, append\_sample,
    diagnostic\_file, append\_diagnostic, ... } } % refresh, save\_warmup} }
%
...
\put(50, 370) { \makebox(450, 10)[l]{file, append\_sample,
    diagnostic\_file, append\_diagnostic, ...} }  % refresh, ...
%
\put(10, 340) { \makebox(50, 10)[l]{method} }
\put(20, 117.5) { \line(0, 1){212.5} }
\put(20, 117.5) { \vector(1, 0){15} }
...
implicitly with only \farg{method\_name}.  Available method names in
...
\refsection{command-getting-started}. 
...
If \code{help} is specified without any method, a usage message is
displayed.  From the top-level \code{help\_all} displays the entire
argument hierarchy.

\subsubsection{Context-Sensitive Help}

Specifying \code{help} after any argument displays a description and
valid options for that argument.  The alternative \code{help-all}
provides a verbose presentation of that argument and all arguments
derived from it in the hierarchy.  


\section{Full Argument Hierarchy}
...
enable the subsequent arguments.
...
\subsubsection{Sampling Algorithm-Specific Arguments}

\begin{description}
    \hiercmdarg{\indentarrow\indentarrow}{algorithm}{$$list element$$}
      {Sampling algorithm}
      {Valid values: \  \code{hmc}}
...
\subsection{Diagnostic-Specific Argument}
...
\hierlongcmd{}{output}
  {File output options}
  {Valid subarguments: \code{file, append\_sample, diagnostic\_file,
      \\ \hspace*{24pt} append\_diagnostic, refresh}}
%
  \hiercmdarg{\indentarrow}{file}{$$string$$}
    {Output file}
    {Valid values: \  Path to existing file}
    {Defaults to \code{samples.csv}}
%
...
> ./model method=sample data fle=model.data.R  \
    init=model.init.R random seed=5
\end{Verbatim}
\end{quote}
...
> ./model method=sample data file=model.data.R  \
    init=model.init.R random seed=5 metric=unit
\end{Verbatim}
\end{quote}
%
...
\caption{\small\it Command skeleton for invoking the no-U-turn sampler
  ...
  \refsection{detailed-command-arguments}}\label{nuts-command.figure}
\end{figure}
%
...
Note that both NUTS and HMC may be configured with either a unit,
diagonal or dense mass matrix; a diagonal mass matrix is the default.%
%
\footnote{In Euclidean HMC, a diagonal mass matrix amounts to the same
  thing as using varying step sizes.  Varying step sizes were used in
  Stan 1.3 and before.  \cite{Neal:2011} discusses the equivalence and
  provides a derivation.}
%
A diagonal mass matrix essentially scales each parameter
independently.  A unit mass matrix provides no parameter-by-parameter
scaling.  A dense mass matrix also estimates parameter covariance, so
that correlated parameters may move together.  Although dense mass
matrices offer the hope of superior simulation performance, they
require more computation, specifically $\mathcal{O}(\log(m) n^3 + m \,
n^2)$ compared to $\mathcal{O}(m\,n)$ for diagonal mass matrices.
Furthermore, dense mass matrices are difficult to estimate, because
there are $\mathcal{O}(n^2)$ cells with complex interdependence.
%
\begin{figure}
\begin{quote}
\begin{Verbatim}[fontshape=sl,fontsize=\small]
...
\caption{\small\it Command skeleton for invoking the basic Hamiltonian
  ...
  \refsection{detailed-command-arguments}}\label{hmc-command.figure}
\end{figure}
...
\caption{\small\it Command skeleton for invoking the BFGS optimizer.
  All arguments and their default values are described in detail in
  \refsection{detailed-command-arguments}.}\label{bfgs-command.figure}
\end{figure}
%
...
\caption{\small\it Command skeleton for invoking the Nesterov
  ...
  \refsection{detailed-command-arguments}.}\label{nesterov-command.figure}
\end{figure}
%
...
\caption{\small\it Command skeleton for invoking the Newton optimizer.
  All arguments and their default values are described in detail in
  \refsection{detailed-command-arguments}.}\label{newton-command.figure}
\end{figure}
%
...
\end{Verbatim}
\end{quote}
\caption{\small\it Skeleton of the command for diagnostics.  All
  arguments are described in detail in
  \refsection{detailed-command-arguments}.}\label{diagnostic-command.figure}
\end{figure}
...
\end{quote}
%
The result of \code{bin/print} is displayed in \reffigure{bin-print-eg}.
%
\begin{figure}
...
Section~\refsection{arrays-of-matrices} for more information on
