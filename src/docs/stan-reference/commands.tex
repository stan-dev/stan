...
``fast'' adaptation interval (I), a series of expanding ``slow'' adaptation intervals (A), 
and a final ``fast'' adaptation interval (T).  For HMC both the fast and slow intervals
are used for adapting the step size, while the slow intervals are used for learning
the (co)variance is necessitated by the metric.}
    \label{adaptation.figure}
\end{figure}

When adaptation is engaged the warmup period is split into three
stages (Figure \ref{adaptation.figure}), with two ``fast'' intervals
surrounding a series of growing ``slow'' intervals.  Here ``fast''
