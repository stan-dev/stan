...
Arguments grouped within a category are not ordered with respect
...
\put(100, 20) { \framebox(25, 200)[c]{A} }
\put(125, 20) { \framebox(50, 200)[c]{A} }
\put(175, 20) { \framebox(100, 200)[c]{A} }
\put(275, 20) { \framebox(200, 200)[c]{A} }
\put(475, 20) { \framebox(400, 200)[c]{A} }
\put(875, 20) { \framebox(50, 200)[c]{T} }
\put(0, 20) { \vector(1, 0){1000} }
\put(800, -10) { \makebox(200, 20)[l]{Iteration} }
%
\end{picture}
\caption{\small\it Adaptation during warmup occurs in three stages: initialization (I),
adaption (A), and termination (T).  The step size is adapted in each stage but the
(co)variance is estimated only in the adaptation windows, and only if necessitated
by the specified sampler.}
    \label{adaptation.figure}
\end{figure}

When adaptation is engaged the warmup period is split into three
stages (Figure \ref{adaptation.figure}).  First the chain is allowed 
to converge towards the typical set in an initialization buffer, with 
only the step size adapting.  After this initial stage the (co)variances 
are estimated in a series of adaptation windows, if needed by the sampler.  
Each adaptation window is expanded to twice the size of the previous 
window with estimates and the step size, reset in between.  
Finally the step size is allowed to adapt to the final (co)variance update
in a termination buffer.  The size of the buffers and the adaptation
window are all customizable, although user-specified values may be
...
%
      \hiercmdarg{\indentarrow\indentarrow\indentarrow}{init\_buffer}{$$unsigned int$$}
        {Number of warmup iterations before adaptation commences}
        {Valid values: \ All}
        {Defaults to \code{75}}
%
      \hiercmdarg{\indentarrow\indentarrow\indentarrow}{term\_buffer}{$$unsigned int$$}
        {Number of warmup iterations after adaptation terminates}
        {Valid values: \ All}
        {Defaults to \code{50}}
%
      \hiercmdarg{\indentarrow\indentarrow\indentarrow}{window}{$$unsigned int$$}
        {Width of adaptation interval}
        {Valid values: \ All}
        {Defaults to \code{25}}
%
\end{description}
...
The \code{max\_depth} argument for NUTS controls how much deeper than
...
\code{max\_depth} layers, it stops automatically.  If a model is
