...
distribution $p(\theta|y;x)$ of the parameters given the data.
...
so that the \Stan model may define the model through the function
\[
f(\theta,y,x) = \log p(y|\theta;x) + \log p(\theta;x),
\]
with implicit normalizing constant $Z_{y,x} = p(y;x)$.
