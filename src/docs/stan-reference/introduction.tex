...
language for specifying probability models.  Model
specifications are converted to \Cpp code for reading data,
transforming parameters, and calculating the log probability function
and its gradient.  This \Cpp code is then compiled to an executable
for performing full Bayesian inference by sampling from the posterior.
...
%
\begin{itemize}
\item $y$ is a vector of known data values with a probabilistic model,
\item $x$ is a vector of known data values without a probabilistic
  model, such as size constants, predictors, and fixed
...
\end{itemize}
%
A compiled \Stan model is then given fixed values for the data $y$ and
$x$, and returns a sequence of samples from the conditional
distribution $p(\theta|y)$ of the parameters given the data.
...
function $f(\theta,y,x)$ such that for every data point $y$ and $x$,
there is a normalizing constant $Z_{y,x}$ depending only on $y$ and
$x$ such that
%
\[
...
$Z_{y,x}$, so it need not be included in the model.

An equivalent way of viewing a \Stan program is as defining the
conditional distribution $\log p(\theta|y)$ up to a normalizing
constant, because
\[
p(\theta|y) = \frac{1}{p(y)} p(y,\theta).
\]
Consequently, any or all of the normalizing constant may be included
in the \Stan function $f(y,\theta)$.  

\subsection{Bayesian Inference}

\Stan was developed to enable Bayesian inference through posterior
sampling.  Bayes's rule,
%
\[
p(\theta|y;x) 
...
\]
%
derives a formula for the posterior 
probability function $p(\theta|y;x)$ in terms of the sampling 
probability function $p(y|\theta;x)$ and the prior probability 
function $p(\theta;x)$ (which may depend on constants in $x$, but not 
modeled data $y$) by
 
The denominator is a normalizing term depending only on $y$ and $x$,
so that the \Stan model may define a function
\[
f(\theta,y,x) = \log p(y|\theta;x) + \log p(\theta;x).
\]
because the normalizing constant is given by $Z_{y,x} = p(y;x)$.
...
$\distro{Normal}(\mu,\sigma)$.  In fact, the normalizing terms can be
dropped and $f$ defined instead by
\[
...
such as deviations (constrained to be positive), unit simplexes (a
vector constrained to be positive and to sum to 1), and covariance
matrices (symmetric and positive definite).  The result is an
unconstrained sampling (or optimization) problem from the perspective
of the sampler.  From the user's perspective, this transform happens
behind the scenes, driven by the types declared for each of the
parameters.
