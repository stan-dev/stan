...
\refsection{hmc-parameters}). The inverse $\Sigma^{-1}$ is known as
...
completed tree hits \code{max\_depth}. It then draws a sample using
...
estimates; see \refchapter{mle} for background on point estimation.
...
and described in detail in \refappendix{variable-transforms}.
