...
doesn’t always help.  Therefore, Stan estimates these correlations
...
\chapter{Automatic Variational Inference}\label{vi-algorithms.chapter}
...
single parameter that we expose, denoted \texttt{eta\_adagrad}. We recommend a
default value of $0.1$. However, for some models this may be too large. Should
you observe the ELBO to diverge, try reducing \texttt{eta\_adagrad}. In the
future, we hope to adaptively set this parameter using heuristics.
