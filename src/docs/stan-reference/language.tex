...
format (see \refchapter{dump}); for calls from \R, variables may be
read from file or directly from \R's memory.
...
declared as \code{int(0,)}, after \code{N} is read, it will be tested
...
model executable or \R's memory when called from \R).  Any constraints
declared on the parameters are validated for the initial values.  If a
variable's value violates its declared constraint, the program halts
...
current parameter values (a more
...
evaluation of the log probabilty and gradient are used to update the
...
If the user specifies the number of leapfrog steps (i.e., chosen to
...
required because the inverse transform is a total function every point
...
declared on derived quantities variables are validated.   If these
...
parameters).  In the \R version of \Stan, the values may either be
...
\item Strong typing catches programming errors relative to the
  declared intent to be caught sooner (at compile time) rather than
  later (at run time).  The \Stan compiler (see \refchapter{stanc})
...
\end{itemize}
%
Strong typing disallows assigning the same variable to different types
at different points in the program or in different invocations of the
program. 
...
\code{simplex} for unit simplexes and \code{ordered} for ordered
...
simplexes, this is only done up to $\epsilon$ to account for errors
arising from floating-point imprecision.
...
variable declared to be \code{real(0,1)} could be assigned to a
...
\begin{Verbatim}
data {
  int(0,) N_observed;    int(0,) N_missing;
  ...
...
\times 4$ array \code{y} of two dimesions, \code{y[1,1]} is the first
...
array with three rows and four columns.  Then \code{y[3]} is one-dimensional
array of length four.  This means that \code{y[3][1]} may be used instead
...
(see \refchapter{stanc}) will report the location of the problem.
...
\end{quote}
%
or with the names of predefined functions or distributions, but these
should nevertheless be avoided for the sake of program clarity.
...
account for the above log transform.%
%
...
variables used in Bayesian models.  \Stan adds a missing data type to
the list.  \reffigure{variable-kinds} contains Gelman and Hill's
taxonomy along with a missing data type along with the corresponding
locations of declarations and definitions in \Stan.
...
be declared in the \code{transformed data} block.  For example, the
...
Finally, the variable \code{n} is used as a loop index in two for
...
more complex parameter distributions.
...
\noindent
This appendix provides a Backus-Naur form (\BNF) grammar for the \Stan
modeling language.  Literal strings are indicated in single quotes
(\code{'}).  Grammar non-terminals are unquoted strings.  A prefix
question mark (\code{?}) indicates optionality.  A postfixed Kleene
star (\code{*}) indicates zero or more occurrences. 
