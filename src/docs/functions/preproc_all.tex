...
If $\mu \in (0, 1)$ and $\kappa \in \mathbb{R}^+$, then for $\theta \in (0,1)$, \[ \text{Beta_Proportion}(\theta|\mu,\kappa) = \frac{1}{\mathrm{B}(\mu \kappa, (1 - \mu) \kappa)} \, \theta^{\mu\kappa - 1} \, (1 - \theta)^{(1 - \mu)\kappa- 1} , \] where the beta function $\mathrm{B}()$ is as defined in section \@ref(betafun).
