...
generalized linear models.  The function $\Phi$ is also
...
\end{itemize}
%
\section{1D Integrator}%
\label{functions-1d-integrator.section}

Stan provides a built-in mechanism to perform 1D integration of a function via quadrature methods.

It operates similarly to the algebraic solver (\refsection{functions-algebraic-solver}) and ordinary differential equations solver (\refsection{functions-ode-solver}) in that it allows as an argument a function.

Like both of those utilities, some of the arguments are limited
to data only expressions. These expressions must not contain variables
other than those declared in the data or transformed data blocks.

\subsection{Specifying an Integrand as a Function}
...
The argument of this function are:
\begin{itemize}
\item \farg{x}, the independent variable being integrated over
\item \farg{xc}, a high precision version of the distance from x to the nearest endpoint in a definite integral (for more into see \refsection{precision-loss})
\item \farg{theta}, parameter values used to evaluate the integral
\item \farg{x\_r}, data values used to evaluate the integral
\item \farg{x\_i}, integer data used to evaluate the integral
\end{itemize}

Like algebraic solver and the differential equations solver, the 1D
integrator separates parameter values, \farg{theta}, from data values, \farg{x\_r}.

\subsection{Call to the 1D Integrator}

\begin{description}
%
   \fitemthreelines{vector}{integrate\_1d}%
                  {function \farg{integrand}}%
                  {real \farg{a}, real \farg{b}}%
                  {real[] \farg{theta}, real[] \farg{x\_r}, int[] \farg{x\_i}}%
                  {Integrates the integrand from \farg{a} to \farg{b}.}

\fitemfourlines{vector}{integrate\_1d}%
                  {function \farg{integrand}}%
                  {real \farg{a}, real \farg{b}}%
                  {real[] \farg{theta}, real[] \farg{x\_r}, int[] \farg{x\_i}}%
                  {real \farg{relative\_tolerance}}%
                  {Integrates the integrand from \farg{a} to \farg{b} with the given relative tolerance.}
%
\end{description}


\subsubsection{Arguments to the 1D Integrator}

The arguments to the 1D integrator are as follows:
%
\begin{enumerate}
\item \farg{integrand}: function literal referring to a function
  specifying the integrand with signature
  \begin{quote}
    (real, real, real[], real[], int[]):real
  \end{quote}
  The arguments represent (1) where integrand is evaluated, (2) distance from evaluation point to integration limit for definite integrals, (3) parameters, (4) real data, and
  (5) integer data, and the return value is the integrand evaluated at the given point,
\item \farg{a}: left limit of integration, may be negative infinity, type \code{real},
\item \farg{b}: right limit of integration, may be positive infinity, type \code{real},
\item \farg{theta}: parameters only, type \code{real[]},
\item \farg{x\_r}: real data only, type \code{real[]}, and
\item \farg{x\_i}: integer data only, type \code{int[]}.
\end{enumerate}

A \farg{relative\_tolerance} argument can also be provided for more control over the algorithm:
%
\begin{enumerate}
  \addtocounter{enumi}{6}
  \item \farg{relative\_tolerance}: relative tolerance for the 1d integrator, type \code{real}, data only.
\end{enumerate}

\subsubsection{Return value}

The return value for the 1D integrator is a \code{real}, the value of the integral.

\subsubsection{Zero-crossing integrals}\label{zero-crossing.section}

For numeric stability, integrals on the (possibly infinite) interval $(a, b)$ that cross zero are split into two integrals, one from $(a, 0)$ and one from $(0, b)$. Each integral is separately integrated to the given \farg{relative\_tolerance}.

\subsubsection{Precision loss near limits of integration in definite integrals}\label{precision-loss.section}

When integrating certain definite integrals, there can be significant precision loss in evaluating the integrand near the endpoints. This has to do with the breakdown in precision of double precision floating point values when adding or subtracting a small number from a number much larger than it in magnitude (for instance, \code{1.0 - x}). \farg{xc} (as passed to the integrand) is a high-precision version of the distance between \farg{x} and the definite integral endpoints and can be used to address this issue. More information (and an example where this is useful) is given in the User's Guide. For zero crossing integrals, \farg{xc} will be a high precision version of the distance to the endpoints of the two smaller integrals. For any integral with an endpoint at negative infinity or positive infinity, \farg{xc} is set to \code{NaN}.

\subsubsection{Algorithmic details}\label{algorithmic-details.section}
Internally the 1D integrator uses the double-exponential methods in the Boost 1D quadrature library. Boost in turn makes use of quadrature methods developed in \citep{Takahasi:1974}, \citep{Mori:1978}, \citep{Bailey:2005}, and \citep{Tanaka:2009}.
