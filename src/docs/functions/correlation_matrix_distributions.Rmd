...
Because Stan requires models to have support on all valid constrained parameters, `L` will almost always[^fnlkj] be a parameter declared with the type of a Cholesky factor for a correlation matrix; for example,

[^fnlkj]: It is possible to build up a valid `L` within Stan, but that would then require Jacobian adjustments to imply the intended posterior.
...
`matrix` **`lkj_corr_cholesky_rng`**(`int K, real eta`)
 Generate a random Cholesky factor of a correlation matrix of order K that is distributed LKJ with shape eta; may only be used in generated quantities block
