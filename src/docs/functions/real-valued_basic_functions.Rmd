...
`real` **`operator^`**(`real x, real y`)
 Return x raised to the power of y. \[ (x\text{^}y) =
\text{operator^}(x,y) = x^y \]
...
\text{binary_log_loss}(y,\hat{y}) = \begin{cases} -\log \hat{y} &
...
variables. \[ \text{owens_t}(h,a) = \frac{1}{2\pi} \int_0^a
...
defined for positive a and nonnegative z. \[ \text{gamma_p}(a,z) =
...
defined for positive a and nonnegative z. \[ \text{gamma_q}(a,z) =
...
\text{binomial_coefficient_log}(x,y) = \log\Gamma(x+1) -
...
x. \[ \text{bessel_first_kind}(v,x) = J_v(x), \] where \[
...
\text{bessel_second_kind}(v,x) = \begin{cases} Y_v(x) & \text{if } x >
0 \\ \textrm{error} & \text{otherwise} \end{cases} \] where \[
...
\text{modified_bessel_first_kind}(v,z) = I_v(z) \] where \[ {I_v}(z) =
...
\text{modified_bessel_second_kind}(v,z) = \begin{cases} K_v(z) &
...
x and real n. \[ \text{falling_factorial}(x,n) = \begin{cases} (x)_n &
...
For $0 \leq y \leq x$, \[ \text{binomial_coefficient_log}(x,y) =
...
for positive x and real n. \[ \text{log_falling_factorial}(x,n) =
...
and real n. \[ \text{rising_factorial}(x,n) = \begin{cases} x^{(n)} &
\text{if } x > 0 \\ \textrm{error} & \text{if } x \leq 0 \end{cases}
...
positive x and real n. \[ \text{log_rising_factorial}(x,n) =
...
of y. \[ \text{multiply_log}(x,y) = \begin{cases} 0 & \text{if } x = y
...
\text{log_diff_exp}(x,y) = \begin{cases} \log(\exp(x)-\exp(y)) &
...
proportion theta, defined by \begin{eqnarray*} \text{log_mix}(\theta,
\lambda_1, \lambda_2) & = & \log \!\left( \theta \exp(\lambda_1) +
\left( 1 - \theta \right) \exp(\lambda_2) \right) \\[3pt] & = &
\text{log_sum_exp}\!\left(\log(\theta) + \lambda_1, \ \log(1 - \theta)
+ \lambda_2\right). \end{eqnarray*}
