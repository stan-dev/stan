...
Return column indices for values in `csr_extract_w(a)`; see \@ref(CSR).

`int[]` **`csr_extract_u`**(`matrix a`)
Return array of row starting indices for entries in `csr_extract_w(a)` followed by the size of `csr_extract_w(a)` plus one; see section \@ref(CSR).
...
Return dense $\text{m} \times \text{n}$ matrix with non-zero matrix entries w, column indices v, and row starting indices u; the vector w and arrays v and u must all be the same size, and the arrays v and u must have index values bounded by m and n. see section \@ref(CSR) for more details.
...
`vector` **`csr_matrix_times_vector`**(`int m, int n, vector w, int[] v, int[] u, vector b`)
 Multiply the $\text{m} \times \text{n}$ matrix represented by values w, column indices v, and row start indices u by the vector b; see \@ref(CSR).
