...
If the variable $u$ has a $\distro{Uniform}(0, 1)$ distribution, then
...
\footnote{The problem is the (extremely!) light tails of the triangle
  ...
  distribution on the logit-transformed \code{y} is well behaved.}
...
or indirectly, should have the \code{data} prefix qualifier.  This
...
\section{Truncated Random Number Generation}

\subsection{Generation with Inverse CDFs}
...
were not already built in as \code{logistic\_rng(0, 1)}, it could be
...
finite and that \code{sigma} is non-negative, e.g.,
%
\begin{stancode}
...
\subsection{Truncated variate generation}
...
\subsubsection{Truncating below}

For example, the following code generates a
$\mathsf{Weibull}(\alpha, \sigma)$ variate truncated below at a time $t$,%
%
\footnote{The original code and impetus for including this in the
  manual came from the Stan forums post
  \url{http://discourse.mc-stan.org/t/rng-for-truncated-distributions/3122/7};
  by user \code{lcomm}, who also explained truncation above and below.}
%
\begin{stancode}
...
\subsubsection{Truncating above and below}
...
finiteness, \code{sigma} should be tested for positiveness, and
\code{lb} and \code{ub} should be tested to ensure the upper bound is
...
\end{stancode}
%
for readability.  The names like \code{p} indicate probabilities, and
\code{p\_lb} and \code{p\_ub} indicate the probabilities of the
bounds.  The variable \code{u} is clearly named as a uniform variate,
and \code{y} is used to denote the variate being generated itself.
...
\end{stancode}
%
The arrays \code{thetas} of parametrs, \code{x\_rs} of real data, and
\code{x\_is} of integer data have the suffix ``\code{s}'' to indicate they
are arrays.  These arrays must all be the same size, as they will be
mapped in parallel by the function \code{f}.  The value of \code{phi}
is reused in each mapped operation.

The \code{\_rect} suffix in the name arises because the data
...
data values.  These argument types are marked with the \code{data}
...
The notation \code{(vector, vector, real[], int[]):vector} indicates
that the function argument \code{f} must have the following signature.
%
\begin{stancode}
...
\end{stancode}
%
Although \code{f} will often return a vector of size one, the built-in
flexiblity allows general multivariate functions to be mapped, even
raggedly.

\subsection{Map Function Semantics}

Stan's map function applies the function \code{f} to the shared
parameters along with one element each of the job parameters, real
data, and integer data arrays.  Each of the arguments \code{theta},
\code{x\_r}, and \code{x\_i} must be arrays of the same size.  If the
arrays are all size \code{N}, the result is defined as follows.
%
\begin{stancode}
...
concatenation (implemented as \code{append\_row} in Stan).  The output
of each application of \code{f} is a vector, and the sequence of
\code{N} vectors is concatenated together to return a single vector.
...
used individually.  The \code{bernoulli\_logit} distribution is used
...
\subsection{Mapped Logistic Regression}
...
logistic regression.  The argument \code{beta} will contain the
regression coefficients (intercept and slope), as before.  The second
argument \code{theta} of job-specific parameters is not used, but
nevertheless must be present.  The modeled data \code{y} is passed as
an array of integers and the predictors \code{x} as an array of real
values.  The function body then computes the log probability mass of \code{y} and
assigns it to the local variable \code{lp}.  This variable is then
used in \code{[lp]'} to construct a row vector and then transpose it
...
\footnote{The term ``shard'' is borrowed from databases, where it
  ...
  dataframe.}
%
The value \code{3} is also hard coded; a more practical program would
...
of shards.  The array \code{ys} contains the modeled data variables;
each element of the array \code{ys} is an array of size four.  The
second array \code{xs} is for the predictors, and each element of it
is also of size four.  These contained arrays are the same size
because the predictors \code{x} stand in a one-to-one relationship
with the modeled data \code{y}. The final array \code{theta} is also
...
coded using map-reduce.  The function \code{lr} to compute the log
probability mass is mapped over the data \code{xs} and \code{ys},
which contain the original predictors and outcomes broken into shards.
The parameters \code{beta} are in the first argument because they are
shared across shards.  There are no shard-specific parameters, so
the array of job-specific parameters \code{theta} contains only empty
vectors.
...
\end{stancode}
%
The priors for \code{mu} and \code{sigma} are vectorized.  The priors
on the two components of \code{beta} (intercept and slope,
respectively) are vectorized over the groups.  The likelihood is also
vectorized using multi-indexing with index \code{kk} for the states
and elementwise multiplication (\code{.*}) for the income \code{x}.
...
\subsection{Mapped Implementation}

The mapped version of the model will map over the states \code{K}.
...
(the name \code{bl\_glm} derives from the fact that it's a generalized
...
\end{stancode}
%
The shared parameter \code{mu\_sigma} contains the locations
(\code{mu\_sigma[1:2]}) and scales (\code{mu\_sigma[3:4]}) of the
priors, which are extracted in the first two lines of the program.
The variable \code{lp} is assigned the log density of the prior on
\code{beta}.  The vector \code{beta} is of size two, as are the
vectors \code{mu} and \code{sigma}, so everything lines up for the
vectorization.  Next, the variable \code{ll} is assigned to the log
likelihood contribution for the group.  Here \code{beta[1]} is the
intercept of the regression and \code{beta[2]} the slope.  The
predictor array \code{x} needs to be converted to a vector allow the
multiplication.
...
\end{stancode}
%
The integer \code{J} is set to the number of observations per group.%
%
\footnote{This makes the very strong assumption that each group has
  the same number of observations!}
%
The real data array \code{x\_r} holds the predictors and the integer
data array \code{x\_i} holds the outcomes.  The grouped data arrays
are constructed by slicing the predictor vector \code{x} (and
converting it to an array) and slicing the outcome array \code{y}.
...
hyperparameters \code{mu} and \code{sigma}, but moves the prior for
\code{beta} and the likelihood to the mapped function.
%
\begin{stancode}
parameters {
  vector[2] beta[K];
...
output of \code{map\_rect}.  When every shard returns a singleton
...
arrays, they can still be structured using the \code{to\_matrix}
