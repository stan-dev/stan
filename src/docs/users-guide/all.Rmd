...
# {-}
# <i style="font-size: 110%; color:#990017;">I. Example Models</i></span> {- #example-models.part}
...
usually does not matter very much what prior is used on on the
...
`quad_form_diag` is defined so that
`quad_form_diag(Sigma, tau)` is equivalent to
`diag_matrix(tau) * Sigma * diag_matrix(tau)`, where
`diag_matrix(tau)` returns the matrix with `tau` on the
diagonal and zeroes off diagonal; the version using
`quad_form_diag` should be faster.  See
...
vector `y_obs`) and the missing data (coded as a parameter
vector `y_mis`).  See \refchapter{time-series} for further
...
for cluster $k$.  The transformed parameters `soft_z[n]` contain
the log of the unnormalized cluster assignment probabilities.  The
vector `soft_z[n]` can be converted back to a normalized simplex
...
definition of `sigma_squared`, the more commonly supplied
