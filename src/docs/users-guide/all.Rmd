...
## Comments \label{comments-programming.section}
...
`normal` to be vectors.  If any of the other arguments are
...
would be possible to include a column of 1 values in `x` and
...
## The QR Reparameterization \label{QR-reparameterization.section}
...
for more information on the QR decomposition but note that `qr\_Q` and
`qr\_R` implement the fat QR decomposition so here we thin it by including
...
## Priors for Coefficients and Scales\label{regression-priors.section}
...
## Logistic and Probit Regression\label{logistic-probit-regression.section}
...
=
\mathsf{Bernoulli}(y | \mbox{logit}^{-1}(\alpha)).
$$
...
$\Phi$ is implemented in Stan as the function `Phi\_approx`.  The
...
## Multi-Logit Regression\label{multi-logit.section}
...
all of the predictors.   Then the prior on `beta` is coded in
...
which can then be appended to `beta\_row` to produce the
...
`rep\_row\_vector` and \refsection{matrix-concatenation} for a
definition of `append\_row`.
...
Placing a prior on `beta\_raw` in this parameterization leads to
...
component of `beta\_raw` produces different results than putting
...
that `beta\_raw` sums to 1 because it is a simplex, the
...
`beta\_raw`, perhaps uniform, and another prior put on
`beta\_scale`, typically for "shrinkage."
...
## Ordered Logistic and Probit Regression\label{ordered-logistic.section}
...
`ordered\_logistic` distribution.
...
swapping the cumulative logistic (`inv\_logit`) for the cumulative
normal (`Phi`).
...
`Phi` with `inv\_logit`, though the built-in encoding based
...
expression `beta[l]` denotes a vector.  Although `beta`
...
`x\_beta\_ll`; alternatively, the variable may be declared at the
...
## Hierarchical Priors\label{hierarchical-priors.section}
...
### Boundary-Avoiding Priors for MLE in Hierarchical Models {-}\label{bound-avoid-priors.subsection}
...
## Item-Response Theory Models\label{item-response-models.section}
...
$$
`bernoulli\_logit`(y|\alpha) =
`bernoulli`(y|\mbox{logit}^{-1}(\alpha)).
$$

The key to understanding it is the term inside the
`bernoulli\_logit` distribution, from which it follows that
$$
...
discrimination parameters `beta` and `gamma` then have
...
parameterization, with parameter `mu\_beta` serving as the mean
...
The intercept term `mu\_beta` can't itself be modeled
hierarchically, so it is given a weakly informative
$\mathsf{Cauchy}(0,5)$ prior.  Similarly, the scale terms,
`sigma\_beta`, and `sigma\_gamma`, are given half-Cauchy
...
## Priors for Identifiability\label{priors-for-identification.section}
...
## Multivariate Priors for Hierarchical Models\label{multivariate-hierarchical-priors.section}
...
$$
\beta_j \sim \mathsf{MultiNormal}(\mu, \, \Sigma)
\mbox{ for } j \in 1{:}J.
$$
...
$$
\beta_j \sim \mathsf{MultiNormal}(u_j \, \gamma, \Sigma)
$$
...
because the correlation matrix `Omega` and scale vector
`tau` are more natural to inspect in the output; to output
`Sigma`, define it as a transformed parameter.  The function
`quad\_form\_diag` is defined so that
`quad\_form\_diag(Sigma,~tau)` is equivalent to
`diag\_matrix(tau) * Sigma * diag\_matrix(tau)`, where
`diag\_matrix(tau)` returns the matrix with `tau` on the
diagonal and zeroes off diagonal; the version using
`quad\_form\_diag` should be faster.  See
...
variable `x\_beta\_jj`, which is then filled in a loop and used
...
loop above, because `rows\_dot\_product` cannot do much
...
In this example, the covariance matrix `Sigma\_beta` is defined
...
use it in matrix arithmetic.  The new parameter `L\_Omega` is
...
which are given independent standard normal priors; the `to\_vector`
...
This model also reparameterizes the prior scale `tau` to avoid potential problems with the heavy tails of the Cauchy distribution. The statement `tau\_unif ~ uniform(0,pi()/2)` can be omitted from the model block because stan increments the log posterior for parameters with uniform priors without it.
...
`N\_new`, and their predictor matrix, `x\_new`.  The
predictions themselves are modeled as a parameter `y\_new`.  The
model statement for the predictions is exactly the same as for the
observations, with the new outcome vector `y\_new` and prediction
matrix `x\_new`.
...
Now the data is just as before, but the parameter `y\_new` is now
...
((((([4pt]
 \epsilon_n & \sim & \mathsf{MultiNormal}(0, \Sigma)
\end{eqnarray*}
...
The variables `N\_pos` and `N\_neg` are set to the number of
...
factor of the correlation matrix, `L\_Omega`.  This time there is
...
Given observed predictors $x$ and outcomes $y$, `y\_tilde` will
be drawn according to $p(\tilde{y} \, | \, \tilde{x}, y, x)$.  This
means that, for example, the posterior mean for `y\_tilde` is the
...
# Time-Series Models \label{time-series.chapter}
...
## Autoregressive Models\label{autoregressive.section}
...
rescaling.  This requires a standardized parameter `h\_std` to be
...
The first assignment rescales `h\_std` to have a
...
## Hidden Markov Models\label{hmms.section}
...
`log\_sum\_exp` operation just multiplies the probabilities for
...
sequence `y\_star[1], ..., y\_star[T\_unsup]` underlying the
array of observations `u[1], ..., u[T\_unsup]`.  Because this
...
`back\_ptr`, `best\_logp`, and `best\_total\_logp`
local so they will not be output.  The variable `y\_star` will
...
the maximum probability `best\_logp[t,~k]` for the sequence up to
...
non-negative integer variables `N\_obs` and `N\_mis`.  The
observed data is provided as an array data variable `y\_obs`.
The missing data is coded as an array parameter, `y\_mis`.  The
...
## Partially Known Parameters\label{partially-known-parameters.section}
...
The index arrays `ii\_obs` and `ii\_mis` contain the indexes
into the final array `y` of the observed data (coded as a data
vector `y\_obs`) and the missing data (coded as a parameter
vector `y\_mis`).  See \refchapter{time-series} for further
discussion of time-series model and specifically
\refsection{autoregressive} for an explanation of the vectorization
for `y` as well as an explanation of how to convert this example
to a full AR(1) model.  To ensure `y[1]` has a proper posterior
...
It is most convenient to place a prior directly on `L\_lower`.
An alternative would be a prior for the full Cholesky factor `L`,
because the transform from `L\_lower` to `L` is just the
...
outcome are missin^[Note that this is not the same as missing components of a multivariate predictor in a regression problem; in that case, you will need to represent the missing data as a parameter and impute missing values in order to feed them into the regression.]
...
`y\_observed` to indicate which values were observed (others can
...
## Truncated Distributions\label{truncation.section}
...
## Truncated Data\label{truncated-data.section}
...
Note that for pairs of real numbers, the function `fmax` is used
rather than `max`.
...
Because the censored data array `y\_cens` is declared to be a parameter, it
will be sampled along with the location and scale parameters `mu`
and `sigma`.  Because the censored data array `y\_cens` is
...
= M \, `normal\_lccdf`(y | \mu, \sigma),
$$

where `normal\_lccdf` is the log of complementary CDF
...
For the observed values in `y\_obs`, the normal sampling model is
...
For the left-censored data the CDF (`normal\_lcdf`) has to be
...
# Finite Mixtures \label{mixture-modeling.chapter}
...
## Relation to Clustering\label{clustering-mixture.section}
...
For example, the mixture of $`Normal`(-1,2)$ and
$`Normal`(3,1)$ with mixing proportion $\lambda =
...
## Inferences Supported by Mixtures\label{mixture-inference.section}
...
do is to apply `log\_sum\_exp` of the posterior draws of
`log\_p\_y\_tilde`.  The average log predictive density is then
given by subtracting `log(N\_new)`.
...
## Zero-Inflated and Hurdle Models\label{zero-inflated.section}
...
The `log\_sum\_exp(lp1,lp2)` function adds the log probabilities
...
`poisson\_lpmf(0 | lambda)` every time the first condition
...
`y\_nonzero`, so it must be computed in a function.
...
appropriately;  in the vectorized case, if `y\_nonzero` is empty,
`N\_nonzero` will be zero, and the last two target increment
...
and $\log (1 - \theta)$ to the log density.  The `T[1,]` after
...
Now suppose that the true values of the predictors $x_n$ are not known, but for each $n$, a measurement $x^{\mathrm meas}_n$ of $x_n$ is available.  If the error in measurement can be modeled, the measured value $x^{\mathrm{meas}}_n$ can be modeled in terms of the true value $x_n$ plus measurement noise.  The true value $x_n$ is treated as missing data and estimated along with other quantities in the model.  A very simple approach is to assume the measurement error is normal with known deviation $\tau$.  This leads to the following regression model with constant measurement error.
...
now `x\_meas`, which is a measurement of the true `x` value
with noise scale `tau`.  The model then specifies that the
measurement error for `x\_meas[n]` given true value `x[n]`
...
# Latent Discrete Parameters \label{latent-discrete.chapter}
...
## The Benefits of Marginalization\label{rao-blackwell.section}
...
## Change Point Models\label{change-point.section}
...
directly in Stan using the built-in function `log\_sum\_exp`,
...
see that `head(lp\_e)` holds the forward values, whereas
`lp\_l[T + 1] - head(lp\_l, T)` holds the backward values; the
clever use of subtraction allows `lp\_l` to be accumulated
...
`to\_vector` before being passed to `log\_sum\_exp`.
...
The transformed data block also defines `n\_captured[t]`, which is
the total number of captures at time `t`.  The variable
`n\_captured` is defined as a vector instead of an integer array
...
The definition of `prob\_uncaptured`, from the functions block,
is
...
\end{array}
$$
which can be directly coded using Stan's built-in `log\_sum\_exp`
function.
...
\caption{\small\it Stan program for the rating (or diagnostic
  ...
  probability $\log q(z_i=k|\theta,\pi)$ in\ `log\_q\_z[i,~k]`.}
...
The quantity `log\_q\_z[i]` is defined as a transformed
...
function to `log\_q\_z[i]` provides a simplex corresponding to
...
# Sparse and Ragged Data Structures \label{sparse-ragged.chapter}
...
## Ragged Data Structures\label{ragged-data-structs.section}
...
# Clustering Models \label{clustering.chapter}
...
The only parameter is `mu`, where `mu[k]` is the centroid
for cluster $k$.  The transformed parameters `soft\_z[n]` contain
the log of the unnormalized cluster assignment probabilities.  The
vector `soft\_z[n]` can be converted back to a normalized simplex
...
two indexes in each `soft\_z[n]` leads to an identical likelihood
(and prior).
...
The last step shows how the `log\_sum\_exp` function can be used
...
# Gaussian Processes \label{gaussian-processes.chapter}
...
$$
y \sim \mathsf{MultiNormal}(m(x), K(x | \theta)),
$$
...
target distribution,
$$
  \mu + L\eta \sim \mathsf{MultiNormal}(\mu(x), K(x | \theta)).
$$
...
## Fitting a Gaussian Process\label{fit-gp.section}
...
\begin{align*}
  ...
  f & \sim \mathsf{MultiNormal}\left(0, K(x | \alpha, \rho)\right) \\
...
We have used the Cholesky parameterized \mathsf{MultiNormal} rather than the
standard \mathsf{MultiNormal} because it allows us to the
`cholesky\_decompose` function which has been optimized for both small and
...
`poisson\_log`, which implements a log link, for our likelihood rather
...
covariance matrix called `L\_cov\_exp\_quad\_ARD`.
...
### Priors for Gaussian Process Parameters {-}\label{priors-gp.section}
...
inverse gamma, `inv\_gamma\_lpdf` in Stan’s language, will work
...
\begin{align*}
  f & \sim \mathsf{MultiNormal}\left(0, K(x | \alpha, \rho)\right) \\ y_i &
...
example, but we've defined a function called `gp\_pred\_rng` which will
...
\begin{align*}
  y_i & \sim \mathsf{MultiNormal}(f(x_i), \mathbf{I}_M \sigma^2) \\
...
\begin{align*} f_{[n,]} \sim \mathsf{MultiNormal}(m(x)_{[n,]}, K(x | \alpha,
\rho)_{[n,n]} C(\phi)) \end{align*} and that the columns of the matrix $f$ are
distributed: \begin{align*} f_{[,m]} \sim \mathsf{MultiNormal}(m(x)_{[,m]}, K(x
...
density output (`lp\_\_` in some interfaces).  The underlying
...
@Marsaglia:1972.  A point $y \in \reals^K$ is transformed to a
point $x \in S^{K-1}$ by
...
# Solving Algebraic Equations \label{algebra-solver.chapter}
...
data in `x\_r` (a real array) and the integer data in `x\_i`
...
`1e-10` ($10^{-10}$), `1e-6` ($10^{-6}$), and `1e3` ($10^3$). Their
...
The arguments for the real data `x\_r` and
the integer data `x\_i` must be expressions that only involve data or
...
`y\_guess`, which may be a data or a parameter vector.
...
## Control Parameters for the Algebraic Solver\label{algebra-control.section}
...
# Solving Differential Equations \label{ode-solver.chapter}
...
along with real data in variable `x\_r` (a real array) and
integer data in variable `x\_i` (an integer array).  The system
...
`integrate\_ode` function to accept a parameter for the initial
...
operations, we declare `y0` and `y\_hat` as vectors, instead of using arrays,
...
\caption{\small\it Stan program to simulate noisy measurements from a
  ...
  `y\_hat`. Because the ODEs are linear, we can use the `matrix\_exp`
...
\caption{\small\it Stan program to simulate noisy measurements from a
  ...
  random measurement error, producing observations `y\_hat`.
  Because the system is not stiff, the `rk45` solver is used.}\label{sho-sim.figure}
\end{figure}
...
time `t`, real data `x`, and integer data `x\_int` must
...
parameters uses the `integrate\_ode` function in the model block
...
array `y\_hat`, which is then used as the location in the
...
## Stiff ODEs\label{stiff-ode.section}
...
# Reparameterization and Change of Variables \label{change-of-variables.chapter}
...
In this model, an unconstrained parameter `theta\_raw` gets a
logistic prior, and then the transformed parameter `theta` is
defined to be the inverse logit of `theta\_raw`.  In this
parameterization, `inv\_logit(mu)` is the mean of the implied
...
through `L\_Sigma` at the expense of having on the order of $K^2$
...
directly in Stan as follows.^[This example is for illustrative purposes only; the recommended way to implement the lognormal distribution in Stan is with the built-in `lognormal` probability function (see \refsection{lognormal}).]
...
#### Gamma and Inverse Gamma Distribution {-}\label{jacobian-adjustment.section}
...
The transform based approach to sampling `y\_inv` with an inverse
...
The change-of-variables approach to sampling `y\_inv` with an
...
The expression `U - L` is multiplied by `alpha\_raw`
...
# Custom Probability Functions \label{custom-probability-functions.chapter}
...
$\reals$ as follows by defining the probability to be `log(0.0)`,
...
C++ code that is very similar to that generated by the following
sampling statement.
...
# User-Defined Functions \label{functions-programming.chapter}
...
## Basic Functions\label{basic-functions.section}
...
The function is named `relative\_diff`, and is declared to have
...
of `relative\_diff` above, are only allowed in the bodies of
...
`(x~<~0~||~is\_nan(x))`.  The positive infinite case is allowed
...
consumer of `dbl\_sqrt(-2)` would be likely to raise an error, at
...
`row\_vector`
& `row\_vector[M]`
& `row\_vector<lower=L>[M]`
\\
& &  `row\_vector<upper=U>[M]`
\\
& &  `row\_vector<lower=L,upper=U>[M]`
...
\caption{\small\it The leftmost column is a list of the unconstrained
  ...
  types `cov\_matrix[K]`, `corr\_matrix[K]`,
  `cholesky\_factor\_cov[K]`, and
  `cholesky\_factor\_corr[K]` are only assignable to matrices of
...
\refsection{functions-ode-solver}, and
\refsection{functions-algebraic-solver},for details.)  When writing a
...
or indirectly, should have the `data` prefix qualifier.  This
...
multivariate normal draws for `x\_sim`.
...
The ability to use the `unit\_normal` function as a density is
keyed off its name ending in `\_lpdf` (names ending in
`\_lpmf` for probability mass functions work the same way).

In general, if `foo\_lpdf` is defined to consume $N + 1$
arguments, then
...
## Documenting Functions\label{documenting-functions.section}
...
is necessary so that the embedded use of `matrix\_pow` is
...
were not already built in as `logistic\_rng(0, 1)`, it could be
...
## Collinearity of Predictors in Regressions\label{collinearity.section}
...
## Label Switching in Mixture Models\label{label-switching-problematic.section}
...
$\mathsf{Beta}(\phi|0.5,0.5)$, which can arise if very "weak" beta
...
* As indicated by `R\_hat` column, all parameters have
...
three basic linear algebra types, `vector`, `row\_vector`,
...
## Data Type and Indexing Efficiency\label{indexingefficiency.section}
...
`std::vector` template class are implemented very similarly as
...
# Multiple Indexing and Range Indexing \label{multi-indexing.chapter}
...
# Map-Reduce \label{map-reduce.chapter}
...
The arrays `thetas` of parametrs, `x\_rs` of real data, and
`x\_is` of integer data have the suffix "`s`" to indicate they
...
`x\_r`, and `x\_i` must be arrays of the same size.  If the
...
concatenation (implemented as `append\_row` in Stan).  The output
...
used individually.  The `bernoulli\_logit` distribution is used
...
(the name `bl\_glm` derives from the fact that it's a generalized
...
The shared parameter `mu\_sigma` contains the locations
(`mu\_sigma[1:2]`) and scales (`mu\_sigma[3:4]`) of the
...
The integer `J` is set to the number of observations per group.^[This makes the very strong assumption that each group has the same number of observations!]

The real data array `x\_r` holds the predictors and the integer
data array `x\_i` holds the outcomes.  The grouped data arrays
...
output of `map\_rect`.  When every shard returns a singleton
...
arrays, they can still be structured using the `to\_matrix`
...
# Optimizing Stan Code for Efficiency \label{optimization.chapter}
...
## Reparameterization\label{reparameterization.section}
...
\caption{\small\it Neal's Funnel.  (Left) The marginal density of
  ...
  \mathsf{Normal}(y|0,3)$, which has mean 0 and standard deviation
  3.}\label{funnel.figure}
\end{figure}

@Neal:2003 defines a distribution that exemplifies the
...
uniform-distributed parameter `beta\_unif`.
...
The uniform distribution on `beta\_unif` is defined explicitly in
...
some time checking that that `beta\_unif` is between
`-pi()/2` and `pi()/2`, but this condition is guaranteed by
the constraints in the declaration of `beta\_unif`.
...
very fat tails and as `nu` approaches infinity, the Student-t
...
above, `y` is a vector of size `N`, `x * beta` is a
...
are small (i.e., do not have many levels), then a table `a\_b` of
...
Then the sum can be replaced with `a\_b[ii[n], jj[n]]`.
...
\refchapter{map-reduce} for more information on implementing
...
# Bayesian Data Analysis \label{bayesian.chapter}


@GelmanEtAl:2013 provide the following
...
@GelmanEtAl:2013 break applied Bayesian modeling
into the following three steps.

1. Set up a full probability model for all observable and
  unobservable quantities.  This model should be consistent with
  existing knowledge of the data being modeled and how it was
  collected.
1. Calculate the posterior probability of unknown quantities
...
1. Evaluate the model fit to the data.  This includes evaluating
...
# Penalized Maximum Likelihood Point Estimation \label{mle.chapter}
...
## Maximum Likelihood Estimation\label{mle.section}
...
By replacing `N` with `N-1` in the denominator of the
definition of `sigma\_squared`, the more commonly supplied
...
### Examples {-}\label{penalized-mle-examples}
...
The lasso @Tibshirani:1996 is an alternative to ridge
...
[@EfronMorris:1975; @Efron:2012].  This latter approach is similar
to the hierarchical models commonly employed in Bayesian statistics.


## Estimation Error, Bias, and Variance\label{estimation-bias.section}
...
# Variational Inference \label{vi-advanced.chapter}
...
Sometimes expressions are too long to fit on a single line.  In that case, the recommended form is to break \emph{before} an operator,^[This is the usual convention in both typesetting and other programming languages. Neither R nor BUGS allows breaks before an operator because they allow newlines to signal the end of an expression or statement.]  aligning the operator to indicate scoping.  For example, use the following form (though not the content; inverting matrices is almost always a bad idea).
...
# Stan for Users of BUGS \label{stan-for-bugs.appendix}
