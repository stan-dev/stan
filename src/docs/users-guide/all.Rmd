...
$$
\Sigma = \mbox{diag_matrix}(\tau) \ \Omega \ \mbox{diag_matrix}(\tau),
$$
...
for (n in 1:N)
  target += log_sum_exp(neg_log_half + normal_lpdf(y[n] | mu[1], sigma[1]),
...
\log \mbox{Pr}(z_n = k \mid y_n, \mu, \sigma, \lambda)
 = 
 \\
 \log p(y_n \mid z_n = k, \mu, \sigma)  + \log \mbox{Pr}(z_n = k \mid \lambda) - 
\mathrm{log\_sum\_exp}_{k' = 1}^K \log p(y_n \mid z_n = k', \mu,
\sigma) + \log p(z_n = k' \mid \lambda). 
$$
...
$$
\mbox{log_sum_exp}_{n=1}^N \, \alpha_n
\ = \
...
$$
\Sigma_k = \mbox{diag_matrix}({\bf 1}),
$$
so that the log multivariate normal can be implemented directly up to a proportion
by
$$
\mbox{normal}\left( y_n | \mu_k, \mbox{diag_matrix}({\bf 1}) \right)
...
$\text{trace}(K(x | \alpha, \rho)) * C$, whereas $\E\left[ff^T\right]$
...
* Stan does not yet support estimation of discrete parameters
  (discrete data are supported).  We may eventually implement a
...
joint density.  Multiplying them gives, $\mathsf{normal}(x|0.8y,\sigma)
* \mathsf{normal}(y|0.8x,\sigma)$, which is what it is (you can
