...
# <span style="padding: 5em 0 0 0"><i style="font-size: 110%; font-weight: 500; color:#990017;">I. Example Models</i></span> {- #example-models.part}
...
for more information on the QR decomposition but note that `qr_Q` and
`qr_R` implement the fat QR decomposition so here we thin it by including
...
not have an informative prior on the *location* of the regression
...
$\Phi$ is implemented in Stan as the function `Phi_approx`.  The
...
`rep_row_vector` and \refsection{matrix-concatenation} for a
definition of `append_row`.
...
$$
\mathtt{bernoulli\_logit}(y|\alpha) =
\mathtt{bernoulli}(y|\mbox{logit}^{-1}(\alpha)).
$$
...
\begin{eqnarray} {#algebra.equation}
  \begin{aligned}
  z_1 &= y_1 - \theta_1 \\
  z_2 &= y_1 y_2 + \theta_2 \\
  \end{aligned}
...
# <i style="font-size: 110%; color:#990017;">II. Programming Techniques</i> {-} {#programming-techniques.part}
...
## Data Type and Indexing Efficiency {#indexingefficiency.section}
...
# <i style="font-size: 110%; color:#990017;">III. Inference</i> {-}

This section of the model provides an overview of statistical inference
supported by Stan, including full Bayes, maximum posterior modes, and
variational approximations.  It also covers the posterior analysis
calculations such as $\hat{R}$ for convergence monitoring and effective
sample size calculations.

# Bayesian Data Analysis  {#bayesian.chapter}
