...
The term $\textrm{sq\_foot}$ is called an exposure term. If we log the term, we can 
put it in $\eta_{b,t}$:
...
`mu_raw` that is given a $\mathrm{normal}(0,1)$ prior in the model
block. We then make $\texttt{mu}$ a transformed parameter:
We can reparameterize the random intercept $\mu_b$, which is distributed:

$$
\mu_b \sim \text{normal}(\alpha + \texttt{building\_data} \, \zeta, \sigma_{\mu})
$$
...
$\text{normal}(\alpha + \texttt{building\_data}\, \zeta, \sigma_\mu)$ 
...
\text{complaints}_{b,t} \sim \text{Neg-Binomial}(\lambda_{b,t}, \phi)  \\
\lambda_{b,t} = \exp{(\eta_{b,t})}\\
\eta_{b,t} = \mu_b + \kappa_b \, \texttt{traps}_{b,t} + \text{log\_sq\_foot}_b \\
\mu_b \sim \text{normal}(\alpha + \texttt{building\_data} \, \zeta, \sigma_{\mu}) \\
\kappa_b \sim \text{normal}(\beta + \texttt{building\_data} \, \gamma, \sigma_{\kappa})
$$

Let's compile the model.

```{r comp-NB-hier-slopes, cache=TRUE, results="hide", message=FALSE}
comp_model_NB_hier_slopes <- stan_model('stan/hier_NB_regression_ncp_slopes_mod.stan')
```
...
$$
\eta_{b,t} = \mu_b + \kappa_b \, \texttt{traps}_{b,t} + \texttt{mo}_t + \text{log\_sq\_foot}_b
$$
...
comp_model_NB_hier_mos <- stan_model('stan/hier_NB_regression_ncp_slopes_mod_mos.stan')
```
...
$\texttt{mo\_raw} \in \mathbb{R}^M$ in which each element is univariate
$\text{normal}(0,1)$
...
The trick is that by specifying each element of $\texttt{mo\_raw}$ to be distributed $\text{normal}(0,1)$ we
are implicitly defining $\texttt{mo\_raw} \sim \text{multivariate \ normal}(0, I_M)$, where $I_M$ is the identity matrix of
dimension $M \times M$. Then we do a linear transformation using a matrix $L$ and assign the result to `mo` like $\texttt{mo} = L*\texttt{mo\_raw}$
...
$$
\texttt{mo}_1 = \frac{\sigma_{\texttt{mo}}}{\sqrt{1 - \rho^2}} *\texttt{mo\_raw}_1 + 0 *\texttt{mo\_raw}_2 + 0 *\texttt{mo\_raw}_3\\
$$
The second element is a bit more complicated:

$$ \begin{aligned}
\texttt{mo}_2 & = \rho \texttt{mo}_1 + \sigma_{\texttt{mo}}\,\texttt{mo\_raw}_2 + 0 * \texttt{mo\_raw}_3  \\
 & = \rho \left(\frac{\sigma_{\texttt{mo}}}{\sqrt{1 - \rho^2}} *\texttt{mo\_raw}_1\right) + \sigma_{\texttt{mo}}\,\texttt{mo\_raw}_2 + 0 * \texttt{mo\_raw}_3  \\[5pt]
 & = \frac{\rho \sigma_{\texttt{mo}}}{\sqrt{1 - \rho^2}} * \texttt{mo\_raw}_1 + \sigma_{\texttt{mo}}\,\texttt{mo\_raw}_2 + 0 * \texttt{mo\_raw}_3  \\
\end{aligned} $$

While the third element will involve all three terms

$$ \begin{aligned}
\texttt{mo}_3 & = \rho \, \texttt{mo}_2 + \sigma_{\texttt{mo}}\,\texttt{mo\_raw}_3 \\
 & = \rho \left(\frac{\rho \sigma_{\texttt{mo}}}{\sqrt{1 - \rho^2}} *\texttt{mo\_raw}_1 + \sigma_{\texttt{mo}}\,\texttt{mo\_raw}_2\right) + \sigma_{\texttt{mo}} \texttt{mo\_raw}_3  \\[5pt]
& = \frac{\rho^2 \sigma_{\texttt{mo}}}{\sqrt{1 - \rho^2}} * \texttt{mo\_raw}_1 + \rho \, \sigma_{\texttt{mo}}\,\texttt{mo\_raw}_2 +  \sigma_{\texttt{mo}}\,\texttt{mo\_raw}_3  \\
\end{aligned} $$

Writing this all together:

$$ \begin{aligned}
\texttt{mo}_1 & = \frac{\sigma_{\texttt{mo}}}{\sqrt{1 - \rho^2}} * \texttt{mo\_raw}_1 + 0 * \texttt{mo\_raw}_2 + 0 * \texttt{mo\_raw}_3\\[3pt]
\texttt{mo}_2 & = \frac{\rho \sigma_{\texttt{mo}}}{\sqrt{1 - \rho^2}} * \texttt{mo\_raw}_1 + \sigma_{\texttt{mo}}\,\texttt{mo\_raw}_2 + 0 * \texttt{mo\_raw}_3  \\[3pt]
\texttt{mo}_3 & = \frac{\rho^2 \sigma_{\texttt{mo}}}{\sqrt{1 - \rho^2}} * \texttt{mo\_raw}_1 + \rho \, \sigma_{\texttt{mo}}\,\texttt{mo\_raw}_2 +  \sigma_{\texttt{mo}}\,\texttt{mo\_raw}_3  \\
\end{aligned} $$
...
Writing this in Stan code to replace lines 59 to 63 in `stan/hier_NB_regression_ncp_slopes_mod_mos.stan` we would get:
...
comp_model_NB_hier_gp <- stan_model('stan/hier_NB_regression_ncp_slopes_mod_mos_gp.stan')
```
