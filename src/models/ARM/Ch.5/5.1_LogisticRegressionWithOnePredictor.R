...
m <- m + geom_point() + scale_y_continuous("Pr(Republican Vote)",limits=c(-.01,1)) + scale_x_continuous("Income",limits=c(-2,8)) + theme_bw() + stat_smooth(method="glm",family="binomial",se=F,size=2,colour="black") + geom_jitter(position=position_jitter(height=.08,width=.4)) + stat_function(fun=function(x) 1.0 / (1 + exp(-fit1.post$beta[1] - fit1.post$beta[2] * x)))

 # Graph figure 5.1 (b)

m2 <- "ggplot(frame1,aes(x=income,y=vote)) + scale_y_continuous('Pr(Republican Vote)',limits=c(-.01,1)) + scale_x_continuous('Income') + theme_bw() + stat_smooth(method='glm',family='binomial',se=F,colour='black') + geom_jitter(position=position_jitter(height=.08,width=.4))"
for (i in 1:20) {
