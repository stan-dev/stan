...
m <- ggplot(frame1,aes(x=x1))  + scale_x_continuous("Democratic Share of the Two-Party Vote") + labs(title="Congressional Elections in 1988")
m + geom_histogram(colour = "black", fill = "white",binwidth=0.05) + theme_bw()
...
m2 <- ggplot()
m2 <- m2 + geom_point(data=frame1,aes(x=x1,y=y1),shape=1)
m2 <- m2 + geom_point(data=frame2,aes(x=x2,y=y2),shape=16)
m2 <- m2 + geom_point(data=frame3,aes(x=x3,y=y3),shape=4)
m2 + scale_y_continuous("Democratic Vote Share in 1988") + scale_x_continuous("Democratic Vote Share in 1986") + theme_bw() + geom_abline(yintercept=0,slope=1) + labs(title="Raw Data (jittered at 0 and 1)")
...
m3 <- ggplot()
m3 <- m3 + geom_point(data=frame4,aes(x=x1,y=y1),shape=1)
m3 <- m3 + geom_point(data=frame5,aes(x=x2,y=y2),shape=16)
m3 <- m3 + geom_point(data=frame6,aes(x=x3,y=y3),shape=4)
m3 + scale_y_continuous("Democratic Vote Share in 1988") + scale_x_continuous("Democratic Vote Share in 1986") + theme_bw() + geom_abline(yintercept=0,slope=1) + labs(title="Adjusted data (imputing 0's and 1's to .75)")
