...
3. Centering
kidiq_interaction_c.stan: lm(kid_score ~ c_mom_hs + c_mom_iq + c_mom_hs:c_mom_iq)
where:
- c_mom_hs <- mom_hs - mean(mom_hs)
- c_mom_iq <- mom_iq - mean(mom_iq)

4. Centering based on an understandable reference point
kidiq_interaction_c2.stan: lm(kid_score ~ c2_mom_hs + c2_mom_iq + c2_mom_hs:c2_mom_iq)
where:
- c2_mom_hs <- mom_hs - 0.5
- c2_mom_iq <- mom_iq - 100

5. Standardizing
kidiq_interaction_z.stan: lm(kid_score ~ z_mom_hs + z_mom_iq + z_mom_hs:z_mom_iq)
where:
...
mesquite.stan       : lm(weight ~ diam1 + diam2 + canopy_height + total_height
                               + density + group)
mesquite_log.stan   : lm(log(weight) ~ log(diam1) + log(diam2) + log(canopy_height)
...
mesquite_va.stan    : lm(log(weight) ~ log(canopy_volume) + log(canopy_area) + group)
mesquite_vash.stan  : lm(log(weight) ~ log(canopy_volume) + log(canopy_area)
                               + log(canopy_shape) + log(total_height) + group)
