...
# the variance of double exponential distribution is (2 * sigma^2) 
sd <- sqrt(2) * post[, "sigma"]  
...
cat("# sd here is the standard deviation, the `sigma` in BUGS example.\n") 
poi <- cbind(b1, b2, b3, b0, outlier[, c(3, 4, 21)], post[, "sigma"], sd)
colnames(poi) <- c('b1', 'b2', 'b3', 'b0', paste('outlier', c(3, 4, 21), sep = ''), "sigma", "sd") 
summary(as.mcmc(poi))
