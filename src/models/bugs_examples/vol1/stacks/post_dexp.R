...
post <- read.csv(file = "samples_t.csv", header = FALSE)

colnames(post) <- c("beta0", paste("beta", 1:3, sep = ''), "sigmasq")
sigma <- sqrt(post[, "sigmasq"])
...
b1 <- post[, "beta1"] / sd(x[, 1])
b2 <- post[, "beta2"] / sd(x[, 2])
b3 <- post[, "beta3"] / sd(x[, 3])
