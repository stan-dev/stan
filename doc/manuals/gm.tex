...
which if $\alpha \in \reals$ and $\beta \in (\alpha,\infty)$, assigns
the density to $y \in (\alpha,\beta)$ of
\[
...
\]
%
In the case where $\alpha = -1$ and $\beta = 1$, for $y \in (-1,1)$, 
\[
\distro{Triangle}(y,-1,1) = 1 - |y|.
\]
Therefore, the $\distro{Triangle}(-1,1)$ distribution may be coded as
follows.
%
\begin{quote}
\begin{Verbatim}
...
model {
    lp__ <- log(fmax(0.0,1.0 - fabs(y))); 
}
\end{Verbatim}
\end{quote}
%
The single scalar parameter \code{y} is declared as lying in the
interval \code{(-1,1)}.  The model block sets the log probability to
value of $\log \distro{Triangle}(y|-1,1)$.
