...
\renewcommand{\cftchapaftersnum}{.\hspace*{12pt}}
\renewcommand{\cftchapaftersnumb}{\hspace*{8pt}}
%  \fontsize{11}{13}\usefont{OT1}{phv}{bc}{n}\selectfont}

\setcounter{tocdepth}{0}

\titleformat{\chapter}[hang]{\bfseries\LARGE}{\thechapter.}{1.5pc}{}{}
...
\subsection{Custom Distributions within \Stan}
...
set to the joint log probabilty of all parameters, i.e., $\log
\distro{Triangle}(y|-1,1)$.  This value is coded in \Stan as
\code{log1m(fabs(y))}.  The function \code{log1m} is is defined
so that \code{log1m(x)} has the same value as \code{log(1.0-x)}, but
the computation is faster, more accurate, and more stable.

Typically, the log probabilty variable \code{lp\_\_} would be
incremented for the probability of each variable so that at the end of
the program block it was equal to the log joint probability of all
parameters.  The variable \code{lp\_\_} is initialized with the value
0, so an equivalent, though less efficient coding for the above model
...
The constraint declarations are critical for correct run-time
behavior.  If the constraint on \code{y} is removed from the program,
it would compile, but produce arithmetic exceptions at run time when
the sampler explored values of \code{y} outside of $(-1,1)$.
