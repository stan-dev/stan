...
Although Lewandowski et al.\ provided a recursive formula,
the \LKJ transform can be formulated in terms of a Cholesky
factor $w$ of the final correlation matrix, defined for $1 \leq i,j
\leq K$ by
%
\[
w_{i,j} = 
...
1 & \mbox{if } 1 = i = j
\\[12pt]
\sqrt{\prod_{m=1}^{i - 1} \left( i - z_{m,j}^2 \right) }
& \mbox{if } 1 < i = j
\\[12pt]
z_{i,j} & \mbox{if } 1 = i < j
\\[12pt]
z_{i,j} \, \sqrt{\prod_{m=1}^{i-1} \left( 1 - z_{m,j}^2 \right) }
& \mbox{ if } 1 < i < j
%
\end{array}
\right.
...
Covariance matrices are just scaled correlation matrices.  Thus a
scaling transform composed with the correlation matrix transform in
the previous section provides a covariance matrix transform.
